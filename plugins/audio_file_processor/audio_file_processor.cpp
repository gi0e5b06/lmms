...
audioFileProcessor::audioFileProcessor(InstrumentTrack* _instrument_track) :
      ...
      m_nextPlayStartPoint(0), m_nextPlayBackwards(false)
{
    connect(&m_reverseModel, SIGNAL(dataChanged()), this,
    ...
    connect(&m_endPointModel, SIGNAL(dataChanged()), this,
            SLOT(endPointChanged()));
    connect(&m_loopPointModel, SIGNAL(dataChanged()), this,
            SLOT(loopPointChanged()));
    connect(&m_stutterModel, SIGNAL(dataChanged()), this,
                                  ...
                                  sampleFrame*    _working_buffer)
{
    ...
    if(!_n->m_pluginData)
    {
        if(m_stutterModel.value() == true
           && m_nextPlayStartPoint >= m_sampleBuffer.endFrame())
        {
    ...
    if(!_n->isFinished())
    {
        ...
        {
            memset(_working_buffer, 0,
                   (frames + offset) * sizeof(sampleFrame));
            emit isPlaying(0);
        }
    }
    else
...
void audioFileProcessor::loadSettings(const QDomElement& _this)
{
    if(_this.attribute("src") != "")
    {
        ...
        QString absolutePath = m_sampleBuffer.tryToMakeAbsolute(
                m_sampleBuffer.audioFile());
        if(!QFileInfo(absolutePath).exists())
        {
            QString message = tr("Sample not found: %1")
                                      .arg(m_sampleBuffer.audioFile());
...
{
    const real_t freqFactor = BaseFreq / _n->frequency()
...
void audioFileProcessor::startPointChanged(void)
{
    ...
    {
        real_t tmp = m_endPointModel.value();
    ...
    {
        m_loopPointModel.setValue(qMax(m_endPointModel.value() - 0.001, 0.));
    }
    ...
    if(m_startPointModel.value() == m_endPointModel.value())
    {
        m_endPointModel.setValue(qMin(m_endPointModel.value() + 0.001, 1.));
    }
...
void audioFileProcessor::loopPointChanged(void)
{
    ...
    if(m_loopPointModel.value() >= m_endPointModel.value())
    {
        m_endPointModel.setValue(m_loopPointModel.value() + 0.001);
        if(m_endPointModel.value() == 1.)
        {
            m_loopPointModel.setValue(1. - 0.001);
        }
    }
...
void audioFileProcessor::pointChanged()
{
    ...
    const f_cnt_t f_end = static_cast<f_cnt_t>(
            m_endPointModel.value() * (m_sampleBuffer.frames() - 1));
    const f_cnt_t f_loop = static_cast<f_cnt_t>(
            m_loopPointModel.value() * (m_sampleBuffer.frames() - 1));
      ...
      InstrumentView(_instrument, _parent)
{
    if(s_artwork == nullptr)
        s_artwork = new QPixmap(PLUGIN_NAME::getPixmap("artwork"));

    m_openAudioFileButton = new PixmapButton(this);
    m_openAudioFileButton->setCursor(QCursor(Qt::PointingHandCursor));
    m_openAudioFileButton->move(227, 72);
    m_openAudioFileButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("select_file"));
    m_openAudioFileButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("select_file"));
    ...
    m_reverseButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("reverse_on"));
    m_reverseButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("reverse_off"));
    ...
    m_loopOffButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("loop_off_on"));
    m_loopOffButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("loop_off_off"));
    ...
    m_loopOnButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("loop_on_on"));
    m_loopOnButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("loop_on_off"));
    ...
    m_loopPingPongButton->setActiveGraphic(
    ...
    m_stutterButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("stutter_on"));
    m_stutterButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("stutter_off"));
    ...
    m_stutterButton->setWhatsThis(
            tr("Enabling this option makes the sample continue playing "
               ...
               "playback to the start of the sample, insert a note at the "
               "bottom "
...
void AudioFileProcessorView::dragEnterEvent(QDragEnterEvent* _dee)
{
    if(_dee->mimeData()->hasFormat(StringPairDrag::mimeType()))
    {
        ...
        if(txt.section(':', 0, 0)
           == QString("tco_%1").arg(Track::SampleTrack))
        {
...
void AudioFileProcessorView::newWaveView()
{
    ...
    m_waveView = new AudioFileProcessorWaveView(
            this, 245, 75, castModel<audioFileProcessor>()->m_sampleBuffer);
...
void AudioFileProcessorView::dropEvent(QDropEvent* _de)
{
    ...
    else if(type == QString("tco_%1").arg(Track::SampleTrack))
    {
        ...
        castModel<audioFileProcessor>()->setAudioFile(
                dataFile.content().firstChild().toElement().attribute("src"));
        _de->accept();
        return;
    }
...
void AudioFileProcessorWaveView::updateSampleRange()
{
    ...
    {
        const f_cnt_t marging
                = (m_sampleBuffer.endFrame() - m_sampleBuffer.startFrame())
                  * 0.1;
        m_from = qMax(0, m_sampleBuffer.startFrame() - marging);
        m_to   = qMin(m_sampleBuffer.endFrame() + marging,
                    m_sampleBuffer.frames());
    }
}

AudioFileProcessorWaveView::AudioFileProcessorWaveView(QWidget*      _parent,
                                                       int           _w,
                                                       int           _h,
                                                       SampleBuffer& buf) :
      QWidget(_parent),
      m_sampleBuffer(buf),
      m_graph(QPixmap(_w - 2 * s_padding, _h - 2 * s_padding)), m_from(0),
      m_to(m_sampleBuffer.frames()), m_last_from(0), m_last_to(0),
      m_last_amp(0), m_startKnob(0), m_endKnob(0), m_loopKnob(0),
      m_isDragging(false), m_reversed(false), m_framesPlayed(0),
...
void AudioFileProcessorWaveView::updateCursor(QMouseEvent* _me)
{
    ...
    else if(m_isDragging && (m_draggingType == wave))
...
void AudioFileProcessorWaveView::mouseDoubleClickEvent(QMouseEvent* _me)
{
    ...
    switch(m_draggingType)
    {
        case sample_start:
            ...
            qWarning("mouseDoubleClickEvent: start=%d, step=%d", step0,
                     step1);
...
void AudioFileProcessorWaveView::paintEvent(QPaintEvent* _pe)
{
    ...
    const QRect   graph_rect(s_padding, s_padding, width() - 2 * s_padding,
                           height() - 2 * s_padding);
    const f_cnt_t frames = m_to - m_from;
    m_startFrameX        = graph_rect.x()
                    + (m_sampleBuffer.startFrame() - m_from)
                              * real_t(graph_rect.width()) / frames;
    m_endFrameX = graph_rect.x()
                  + (m_sampleBuffer.endFrame() - m_from)
                            * real_t(graph_rect.width()) / frames;
    m_loopFrameX = graph_rect.x()
                   + (m_sampleBuffer.loopStartFrame() - m_from)
                             * real_t(graph_rect.width()) / frames;
    const int played_width_px
            = (m_framesPlayed - m_from) * real_t(graph_rect.width()) / frames;

    // loop point line
    p.setPen(QColor(0x7F, 0xFF, 0xFF));  // TODO: put into a qproperty
    p.drawLine(m_loopFrameX, graph_rect.y(), m_loopFrameX,
               graph_rect.height() + graph_rect.y());

    // start/end lines
    p.setPen(QColor(0xFF, 0xFF, 0xFF));  // TODO: put into a qproperty
    p.drawLine(m_startFrameX, graph_rect.y(), m_startFrameX,
               graph_rect.height() + graph_rect.y());
    p.drawLine(m_endFrameX, graph_rect.y(), m_endFrameX,
               graph_rect.height() + graph_rect.y());

    if(m_endFrameX - m_startFrameX > 2)
    {
        p.fillRect(m_startFrameX + 1, graph_rect.y(),
                   m_endFrameX - m_startFrameX - 1,
                   graph_rect.height() + graph_rect.y(),
                   QColor(95, 175, 255, 50)  // TODO: put into a qproperty
        );
        if(m_endFrameX - m_loopFrameX > 2)
            p.fillRect(m_loopFrameX + 1, graph_rect.y(),
        ...
        if(m_framesPlayed && m_animation)
        {
            ...
            p.drawLine(played_width_px, graph_rect.y(), played_width_px,
                       graph_rect.height() + graph_rect.y());
            m_framesPlayed = 0;
        }
    }
    ...
    p.drawText(s_padding + 2, s_padding + 10,
...
void AudioFileProcessorWaveView::zoom(const bool _out)
{
    ...
    const real_t comp_ratio
            = real_t(qMin(d_from, d_to)) / qMax(1, qMax(d_from, d_to));
    ...
    else
    {
        ...
        new_from = qBound(
                ...
                m_from
                        + f_cnt_t(step_from
                                  * (new_to == m_to ? 1 : comp_ratio)),
                start);
    }

    if(real_t(new_to - new_from) / m_sampleBuffer.sampleRate() > 0.05)
    {
        m_from = new_from;
        m_to   = new_to;
    }
...
{
    const real_t fact = qAbs(real_t(_px) / width());
    ...
    f_cnt_t step_from
            = qBound(0, m_from + step, m_sampleBuffer.frames()) - m_from;
    f_cnt_t step_to
            = qBound(m_from + 1, m_to + step, m_sampleBuffer.frames()) - m_to;
...
void AudioFileProcessorWaveView::setKnobs(knob* _start,
                                          knob* _end,
                                          knob* _loop)
{
    m_startKnob = _start;
...
void AudioFileProcessorWaveView::slideSamplePointByPx(knobType _point,
                                                      int      _px)
{
    slideSamplePointByFrames(
            _point, f_cnt_t((real_t(_px) / width()) * (m_to - m_from)));
}

void AudioFileProcessorWaveView::slideSamplePointByFrames(knobType _point,
                                                          f_cnt_t  _frames,
                                                          bool     _slide_to)
{
    ...
    {
        const real_t v
                = static_cast<real_t>(_frames) / m_sampleBuffer.frames();
        if(_slide_to)
        {
            a_knob->slideTo(v);
        }
...
void AudioFileProcessorWaveView::slideSampleByFrames(f_cnt_t _frames)
{
    ...
    const real_t v = static_cast<real_t>(_frames) / m_sampleBuffer.frames();
    if(m_startKnob)
    {
...
{
    slideSampleByFrames(m_sampleBuffer.frames() - m_sampleBuffer.endFrame()
...
void AudioFileProcessorWaveView::knob::slideTo(real_t _v, bool _check_bound)
{
...
void AudioFileProcessorWaveView::knob::convert(const QPoint& _p,
                                               real_t&       value_,
                                               real_t&       dist_)
{
    const real_t dec_fact
            = !m_waveView ? 1
                          : real_t(m_waveView->m_to - m_waveView->m_from)
                                    / m_waveView->m_sampleBuffer.frames();
    // const real_t inc = ::Knob::getValue( _p ) * dec_fact;
    ::Knob::convert(_p, value_, dist_);
    value_ *= dec_fact;
}
/*
real_t AudioFileProcessorWaveView::knob::getValue( const QPoint & _p )
{
        const real_t dec_fact = ! m_waveView ? 1 :
                real_t( m_waveView->m_to - m_waveView->m_from )
                        / m_waveView->m_sampleBuffer.frames();
        const real_t inc = ::Knob::getValue( _p ) * dec_fact;
...
bool AudioFileProcessorWaveView::knob::checkBound(real_t _v) const
{
    ...
    const real_t d1 = qAbs(m_relatedKnob->model()->value() - model()->value())
                      * (m_waveView->m_sampleBuffer.frames())
                      / m_waveView->m_sampleBuffer.sampleRate();

    const real_t d2 = qAbs(m_relatedKnob->model()->value() - _v)
                      * (m_waveView->m_sampleBuffer.frames())
                      / m_waveView->m_sampleBuffer.sampleRate();
...
extern "C"
{
    ...
    {
        return new audioFileProcessor(static_cast<InstrumentTrack*>(_data));
    }
}
