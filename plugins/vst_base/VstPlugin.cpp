...
VstPlugin::VstPlugin(const QString& _plugin) :
      RemotePlugin(), JournallingObject(), m_plugin(_plugin),
      m_pluginWidget(NULL), m_subWindow(NULL), m_pluginWindowID(0),
      m_badDllFormat(false), m_name(), m_version(0), m_vendorString(),
      m_productString(), m_currentProgramName(), m_allProgramNames(),
      p_name(), m_currentProgram(), m_idleTimer()
{
    setSplittedChannels(true);

    qInfo("VstPlugin::VstPlugin _plugin ='%s'", qPrintable(_plugin));

    /*
    if(_plugin.endsWith(".so", Qt::CaseInsensitive))
    {
#ifdef LMMS_HOST_X86_64
        tryLoad("remote_vst_linux64");
        if(m_badDllFormat)
        {
            m_badDllFormat = false;
            tryLoad("remote_vst_linux32");
        }
#endif
#ifdef LMMS_HOST_X86
        tryLoad("remote_vst_linux32");
    ...
    {
#ifdef LMMS_HOST_X86_64
        tryLoad("RemoteVstWin64");
        if(m_badDllFormat)
        {
            m_badDllFormat = false;
            tryLoad("RemoteVstWin32");
        }
#endif
#ifdef LMMS_HOST_X86
        tryLoad("RemoteVstWin32");
...
{
    qInfo("VstPlugin::tryLoad executable='%s'",
          qPrintable(remoteVstPluginExecutable));
    ...
    static int    k = 0;
    const QString t
            = QString("vst%1%2").arg(GetCurrentProcessId() << 10).arg(++k);
    ...
    if(!failed() && m_pluginWindowID)
    {
        ...
        // VstSubWindow * sw = new VstSubWindow(
...
{
    // qWarning("VstPlugin::showEditor parent=%p
    // isEffect=%d",_parent,isEffect);
    ...
    if(isEffect)
    {
        ...
        if(xe)
        {
            ...
            xe->setGeometry(0, 0, m_pluginGeometry.width(),
                            m_pluginGeometry.height());
            xe->show();
            ...
            if(xq)
            {
                ...
                m_pluginWidget = new QLabel("Error Vst Effect: XQ");
        }
        else
            m_pluginWidget = new QLabel("Error Vst Effect: XE");
#endif
        ...
        // m_pluginWidget->show();
        // qWarning("PluginWidget Size: %d x
        // %d",m_pluginWidget->width(),m_pluginWidget->height());
        ...
        */
...
#if QT_VERSION < 0x050000
        QX11EmbedContainer* xe
                = new QX11EmbedContainer();  // m_pluginWidget );
        xe->embedClient(m_pluginWindowID);
        // xe->setFixedSize( m_pluginGeometry );
        xe->move(4, 24);
        xe->show();
        ...
        if(xe)
        {
            ...
            if(xq)
            {
                ...
                m_pluginWidget = new QLabel("Error Vst Instr: XQ");
        }
        else
            m_pluginWidget = new QLabel("Error Vst Instr: XE");
#endif
        ...
        m_subWindow = SubWindow::putWidgetOnWorkspace(m_pluginWidget, false,
                                                      true, false, false);
        // m_subWindow->show();
    }
#endif
            ...
            for(int i = 0; i < num_params; ++i)
            {
                ...
                m_parameterDump["param" + QString::number(item.index)]
                        = QString::number(item.index) + ":" +
                          /*uncomented*/ /*QString( item.shortLabel )*/
                          QString::fromStdString(item.shortLabel) + ":"
...
void VstPlugin::openPreset()
{
    ...
    if(ofd.exec() == QDialog::Accepted && !ofd.selectedFiles().isEmpty())
    {
        lock();
        sendMessage(
                message(IdLoadPresetFile)
                        .addString(QSTR_TO_STDSTR(QDir::toNativeSeparators(
                                ofd.selectedFiles()[0]))));
...
{
    QString presName = currentProgramName().isEmpty() ? tr(": default")
                                                      : currentProgramName();
    presName.replace(
            tr("\""),
            tr("'"));  // QFileDialog unable to handle double quotes properly

    FileDialog sfd(NULL, tr("Save Preset"),
       ...
       && sfd.selectedFiles()[0] != "")
    {
        ...
        if((fns.toUpper().indexOf(tr(".FXP")) == -1)
           && (fns.toUpper().indexOf(tr(".FXB")) == -1))
            fns = fns + tr(".fxb");
        else
            fns = fns.left(fns.length() - 4) + (fns.right(4)).toLower();
        lock();
        sendMessage(message(IdSavePresetFile)
                            .addString(QSTR_TO_STDSTR(
