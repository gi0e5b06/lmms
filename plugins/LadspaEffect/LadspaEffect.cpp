      ...
      m_key(LadspaSubPluginFeatures::subPluginKeyToLadspaKey(_key))
{
    ...
    if(manager->getDescription(m_key) == nullptr)
    {
        Engine::getSong()->collectError(
                tr("Unknown LADSPA plugin %1 requested.").arg(m_key.second));
        setOkay(false);
        return;
    }
        ...
        for(int port = 0; port < m_portCount; ++port)
        {
            ...
            switch(pp->rate)
            {
                ...
                case CONTROL_RATE_INPUT:
                    ...
                    pp->value = static_cast<LADSPA_Data>(pp->control->value()
                                                         / pp->scale);
...
void LadspaEffect::pluginInstantiation()
{
    ...
    if(m_descriptor == nullptr)
    {
        QMessageBox::warning(0, "Effect",
                             "Can't get LADSPA descriptor function: "
                                     + m_key.second,
    ...
    for(ch_cnt_t proc = 0; proc < processorCount(); proc++)
    {
        ...
        {
            port_desc_t* pp = m_ports.at(proc).at(port);
            if(!manager->connectPort(m_key, m_handles[proc], port,
                                     pp->buffer))
            {
                QMessageBox::warning(0, "Effect",
