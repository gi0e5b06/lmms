...
LadspaControls::LadspaControls(LadspaEffect* _eff) :
      ...
      m_processors(_eff->processorCount()), m_noLink(false),
      m_stereoLinkModel(true, this)
{

    connect(&m_stereoLinkModel, SIGNAL(dataChanged()), this,
            SLOT(updateLinkStatesFromGlobal()));
...
void LadspaControls::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
    ...
    for(multi_proc_t::Iterator it = controls.begin(); it != controls.end();
        it++)
    {
