      ...
      Instrument(_instrumentTrack, &vibedstrings_plugin_descriptor)
{
    ...
    {
        knob = new FloatModel(DefaultVolume, MinVolume, MaxVolume, 1.0f, this,
                              tr("String %1 volume").arg(harm + 1));
        ...
        led = new BoolModel(false, this, tr("Impulse %1").arg(harm + 1));
        m_impulses.append(led);

        led = new BoolModel(harm == 0, this, tr("Octave %1").arg(harm + 1));
    ...
    for(int i = 0; i < 9; i++)
    {
        ...
        if(m_powerButtons[i]->value())
        {
            ...
            _this.setAttribute(name,
                               base64::encodeFloats(m_graphs[i]->samples(),
                                                    __sampleLength));
        }
    }
}
...
void vibed::loadSettings(const QDomElement& _this)
{
    ...
    for(int i = 0; i < 9; i++)
    {
           ...
           && _this.hasAttribute("volume" + QString::number(i)))
        {
            ...
            FLOAT* shp = nullptr;
            base64::decodeFloats(
                    _this.attribute("graph" + QString::number(i)), &shp);
    ...
    for(fpp_t i = offset; i < frames + offset; ++i)
    {
        ...
        for(int string = 0; string < 9; ++string)
        {
            if(ps->exists(string))
            {
                ...
                const sample_t sample = ps->getStringSample(s)
                                        * m_volumeKnobs[string]->value()
                                        / 100.0f;
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    m_stiffnessKnob->setWhatsThis(
            tr("The 'S' knob sets the stiffness of the selected string.  The "
               "stiffness "
               "of the string affects how long the string will ring out.  "
               "The lower "
    ...
    m_pickKnob->setWhatsThis(tr(
            "The 'P' knob sets the position where the selected string will "
            "be 'picked'.  "
    ...
    m_pickupKnob->setWhatsThis(tr(
            "The 'PU' knob sets the position where the vibrations will be "
            "monitored "
    ...
    m_panKnob->setWhatsThis(
            tr("The Pan knob determines the location of the selected string "
               "in the stereo "
               "field."));
    ...
    m_detuneKnob->setWhatsThis(
            tr("The Detune knob modifies the pitch of the selected string.  "
               "Settings less "
               "than zero will cause the string to sound flat.  Settings "
               "greater than zero "
    ...
    m_randomKnob->setWhatsThis(
            tr("The Slap knob adds a bit of fuzz to the selected string "
               "which is most "
               "apparent during the attack, though it can also be used to "
               "make the string "
               "sound more 'metallic'."));
    ...
    m_lengthKnob->setWhatsThis(
            tr("The Length knob sets the length of the selected string.  "
               "Longer strings "
               "will both ring longer and sound brighter, however, they will "
               "also eat up "
               "more CPU cycles."));

    m_impulse = new LedCheckBox("", this);
    m_impulse->move(23, 94);
    ToolTip::add(m_impulse, tr("Impulse or initial state"));
    m_impulse->setWhatsThis(
            tr("The 'Imp' selector determines whether the waveform in the "
               "graph is to be "
               "treated as an impulse imparted to the string by the pick or "
               "the initial "
               "state of the string."));
    ...
    m_harmonic->setWhatsThis(
            tr("The Octave selector is used to choose which harmonic of the "
               "note the "
               "string will ring at.  For example, '-2' means the string "
               "will ring two "
               "octaves below the fundamental, 'F' means the string will "
               "ring at the "
               "fundamental, and '6' means the string will ring six octaves "
               "above the "
               "fundamental."));
    ...
    m_stringSelector = new nineButtonSelector(
            ...
            PLUGIN_NAME::getIconPixmap("button_9_off"), 0, 21, 39, this);
    ...
    m_graph->setWhatsThis(
            tr("The waveform editor provides control over the initial state "
               "or impulse "
               "that is used to start the string vibrating.  The buttons to "
               "the right of "
               "the graph will initialize the waveform to the selected type. "
               " The '?' "
               "button will load a waveform from a file--only the first 128 "
               "samples "
               "will be loaded.\n\n"
    ...
    setWhatsThis(
            tr("Vibed models up to nine independently vibrating strings.  "
               "The 'String' "
               "selector allows you to choose which string is being edited.  "
               "The 'Imp' "
               "selector chooses whether the graph represents an impulse or "
               "the initial "
               "state of the string.  The 'Octave' selector chooses which "
               "harmonic the "
               "string should vibrate at.\n\n"

               "The graph allows you to control the initial state or impulse "
               "used to set the "
               "string in motion.\n\n"

               "The 'V' knob controls the volume.  The 'S' knob controls the "
               "string's "
               "stiffness.  The 'P' knob controls the pick position.  The "
               "'PU' knob "
               "controls the pickup position.\n\n"

               "'Pan' and 'Detune' hopefully don't need explanation.  The "
               "'Slap' knob "
               "adds a bit of fuzz to the sound of the string.\n\n"

               "The 'Length' knob controls the length of the string.\n\n"

               "The LED in the lower right corner of the waveform editor "
               "determines "
    ...
    ToolTip::add(m_power, tr("Click here to enable/disable waveform."));

    // String selector is not a part of the model
    m_stringSelector->setWindowTitle(tr("String"));
    m_stringSelector->setWhatsThis(
            tr("The String selector is used to choose which string the "
               "controls are "
               "editing.  A Vibed instrument can contain up to nine "
               "independently "
               "vibrating strings.  The LED in the lower right corner of the "
               "waveform editor indicates whether the selected string is "
               "active."));

    connect(m_stringSelector, SIGNAL(nineButtonSelection(int)), this,
            SLOT(showString(int)));
    ...
    ToolTip::add(m_sinWaveBtn, tr("Use a sine-wave for "
                                  "current oscillator."));
    ...
    ToolTip::add(m_triangleWaveBtn, tr("Use a triangle-wave "
    ...
    ToolTip::add(m_sawWaveBtn, tr("Use a saw-wave for "
                                  "current oscillator."));
    ...
    ToolTip::add(m_sqrWaveBtn, tr("Use a square-wave for "
                                  "current oscillator."));
    ...
    ToolTip::add(m_whiteNoiseWaveBtn, tr("Use white-noise for "
                                         "current oscillator."));
    ...
    ToolTip::add(m_usrWaveBtn, tr("Use a user-defined "
    ...
    m_smoothBtn->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("smooth_active"));
    m_smoothBtn->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("smooth_inactive"));
    m_smoothBtn->setChecked(false);
    ToolTip::add(m_smoothBtn, tr("Click here to smooth waveform."));
    ...
    m_normalizeBtn->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("normalize_active"));
    m_normalizeBtn->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("normalize_inactive"));
    m_normalizeBtn->setChecked(false);
    ToolTip::add(m_normalizeBtn, tr("Click here to normalize waveform."));

    connect(m_normalizeBtn, SIGNAL(clicked()), this,
