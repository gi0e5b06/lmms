...
ZynAddSubFxRemotePlugin::ZynAddSubFxRemotePlugin() : RemotePlugin()
{
...
ZynAddSubFxInstrument::ZynAddSubFxInstrument(
      ...
      m_hasGUI(false), m_plugin(nullptr), m_remotePlugin(nullptr),
      ...
      m_resCenterFreqModel(
              64, 0, 127, 1, this, tr("Resonance Center Frequency")),
      m_resBandwidthModel(64, 0, 127, 1, this, tr("Resonance Bandwidth")),
      m_forwardMidiCcModel(
              true, this, tr("Forward MIDI Control Change Events"))
{
    initPlugin();

    connect(&m_portamentoModel, SIGNAL(dataChanged()), this,
            SLOT(updatePortamento()));
    connect(&m_filterFreqModel, SIGNAL(dataChanged()), this,
            SLOT(updateFilterFreq()));
    connect(&m_filterQModel, SIGNAL(dataChanged()), this,
            SLOT(updateFilterQ()));
    connect(&m_bandwidthModel, SIGNAL(dataChanged()), this,
            SLOT(updateBandwidth()));
    connect(&m_fmGainModel, SIGNAL(dataChanged()), this,
            SLOT(updateFmGain()));
    connect(&m_resCenterFreqModel, SIGNAL(dataChanged()), this,
            SLOT(updateResCenterFreq()));
    connect(&m_resBandwidthModel, SIGNAL(dataChanged()), this,
            SLOT(updateResBandwidth()));
...
void ZynAddSubFxInstrument::loadFile(const QString& _file)
{
    ...
    setDisplayName(QFileInfo(_file).baseName().replace(QRegExp("^[0-9]{4}-"),
                                                       QString()));
    instrumentTrack()->setName(displayName());
...
bool ZynAddSubFxInstrument::handleMidiEvent(const MidiEvent& event,
                                            const MidiTime&  time,
                                            f_cnt_t          offset)
{
    // do not forward external MIDI Control Change events if the according
    // LED is not checked
    if(event.type() == MidiControlChange && event.sourcePort() != this
...
void ZynAddSubFxInstrument::updateBendingRange()
{
    ...
    if(m_remotePlugin)
    {
        // qInfo("ZynAddSubFxInstrument::updateBendingRange #1
        // %d",instrumentTrack()->midiBendingRange());
        m_remotePlugin->sendMessage(
                RemotePlugin::message(IdZasfSetPitchWheelBendRange)
                        .addInt(instrumentTrack()->midiBendingRange()));
    }
    else
    {
        // qInfo("ZynAddSubFxInstrument::updateBendingRange #2
        // %d",instrumentTrack()->midiBendingRange());
        m_plugin->setPitchWheelBendRange(
                instrumentTrack()->midiBendingRange());
    }
    m_pluginMutex.unlock();
}
    ...
    if(m_hasGUI)
    {
        ...
        m_remotePlugin->sendMessage(
                RemotePlugin::message(IdZasfLmmsWorkingDirectory)
                        .addString(QSTR_TO_STDSTR(QString(
        ...
        m_remotePlugin->updateSampleRate(
                Engine::mixer()->processingSampleRate());

        // temporary workaround until the VST synchronization feature gets
        // stripped out of the RemotePluginClient class causing not to send
        // buffer size information requests
        m_remotePlugin->sendMessage(
                RemotePlugin::message(IdBufferSizeInformation)
                        .addInt(Engine::mixer()->framesPerPeriod()));
...
void ZynAddSubFxInstrument::sendControlChange(MidiControllers midiCtl,
                                              float           value)
{
    handleMidiEvent(
            MidiEvent(MidiControlChange,
                      instrumentTrack()->midiPort()->outputChannel() - 1,
                      midiCtl, (int)value, this));
}
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    m_forwardMidiCC
            = new LedCheckBox(tr("Forward MIDI Control Changes"), this);
...
extern "C"
{
    ...
    Plugin* PLUGIN_EXPORT lmms_plugin_main(Model*, void* _data)
    {
        ...
        return new ZynAddSubFxInstrument(
