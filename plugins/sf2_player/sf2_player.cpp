...
sf2Instrument::sf2Instrument(InstrumentTrack* _instrument_track) :
      ...
      m_reverbRoomSize(FLUID_REVERB_DEFAULT_ROOMSIZE,
                       ...
                       this,
                       tr("Reverb Roomsize")),
      m_reverbDamping(FLUID_REVERB_DEFAULT_DAMP,
                      0,
                      1.0,
                      0.01,
                      this,
                      tr("Reverb Damping")),
      m_reverbWidth(FLUID_REVERB_DEFAULT_WIDTH,
                    0,
                    1.0,
                    0.01f,
                    this,
                    tr("Reverb Width")),
      m_reverbLevel(FLUID_REVERB_DEFAULT_LEVEL,
                    0,
                    1.0,
                    0.01f,
                    this,
      ...
      m_chorusNum(
              FLUID_CHORUS_DEFAULT_N, 0, 10.0, 1.0, this, tr("Chorus Lines")),
      m_chorusLevel(FLUID_CHORUS_DEFAULT_LEVEL,
                    0,
                    10.0,
                    0.01,
                    this,
      ...
      m_chorusSpeed(FLUID_CHORUS_DEFAULT_SPEED,
                    ...
                    this,
                    tr("Chorus Speed")),
      m_chorusDepth(FLUID_CHORUS_DEFAULT_DEPTH,
                    0,
                    46.0,
                    0.05,
                    this,
                    tr("Chorus Depth"))
{
    ...
    connect(&m_reverbRoomSize, SIGNAL(dataChanged()), this,
            SLOT(updateReverb()));
    connect(&m_reverbDamping, SIGNAL(dataChanged()), this,
            SLOT(updateReverb()));
    connect(&m_reverbWidth, SIGNAL(dataChanged()), this,
            SLOT(updateReverb()));
    connect(&m_reverbLevel, SIGNAL(dataChanged()), this,
            SLOT(updateReverb()));
    ...
    connect(&m_chorusLevel, SIGNAL(dataChanged()), this,
            SLOT(updateChorus()));
    connect(&m_chorusSpeed, SIGNAL(dataChanged()), this,
            SLOT(updateChorus()));
    connect(&m_chorusDepth, SIGNAL(dataChanged()), this,
            SLOT(updateChorus()));
...
void sf2Instrument::openFile(const QString& _sf2File, bool updateTrackName)
{
    ...
    char* sf2Ascii
            = qstrdup(qPrintable(SampleBuffer::tryToMakeAbsolute(_sf2File)));
    ...
    else
    {
        ...
        {
            collectErrorForUI(
                    sf2Instrument::tr("A soundfont %1 could not be loaded.")
...
void sf2Instrument::updateSampleRate()
{
    ...
    if(m_internalSampleRate < Engine::mixer()->processingSampleRate())
    {
        ...
        if(m_srcState == NULL || error)
        {
            qCritical(
                    ...
                    "Sf2Instrument::updateSampleRate()");
        }
        m_synthMutex.unlock();
    }
    updateReverb();
    ...
    // Reset last MIDI pitch properties, which will be set to the correct
    // values upon playing the next note
    m_lastMidiPitch      = -1;
        ...
        m_playingNotesMutex.unlock();
            ...
            && !_n->instrumentTrack()
                        ->isSustainPedalPressed())  // note is released during
                                                    // this period
    {
        SF2PluginData* pluginData
                = static_cast<SF2PluginData*>(_n->m_pluginData);
        if(pluginData)
        {
...
void sf2Instrument::play(sampleFrame* _working_buffer)
{
    ...
    if(m_lastMidiPitchRange != currentMidiPitchRange)
    {
        ...
        m_synthMutex.lock();
        fluid_synth_pitch_wheel_sens(m_synth, m_channel,
                                     m_lastMidiPitchRange);
        m_synthMutex.unlock();
    }
    // if we have no new noteons/noteoffs, just render a period and call it a
    // day
    ...
    while(!m_playingNotes.isEmpty())
    {
        ...
        {
            SF2PluginData* currentData
                    = static_cast<SF2PluginData*>(currentNote->m_pluginData);
            SF2PluginData* iData = static_cast<SF2PluginData*>(
                    m_playingNotes[i]->m_pluginData);
        ...
        {
            renderFrames(currentData->offset - currentFrame,
                         _working_buffer + currentFrame);
            currentFrame = currentData->offset;
        }
        if(currentData->isNew)
        {
            noteOn(currentData);
            if(currentNote->isReleased())  // if the note is released during
                                           // the same period, we have to
                                           // process it again for noteoff
            {
...
void sf2Instrument::renderFrames(f_cnt_t frames, sampleFrame* _buf)
{
       ...
       && m_srcState != NULL)
    {
...
#else
        FLOAT* tmp = MM_ALLOC(
                FLOAT, 2 * f);  // sampleFrame * tmp = new sampleFrame[f];
#endif
        ...
        if(error)
        {
            qCritical("sf2Instrument: error while resampling: %s",
                      src_strerror(error));
        }
        if(src_data.output_frames_gen > frames)
        {
            qCritical("sf2Instrument: not enough frames: %ld / %d",
...
void sf2Instrument::deleteNotePluginData(NotePlayHandle* _n)
{
    ...
    if(!pluginData
                ->noteOffSent)  // if we for some reason haven't noteoffed the
                                // note before it gets deleted, do it here
    {
        noteOff(pluginData);
        m_playingNotesMutex.lock();
...
{
  public:
    sf2Knob(QWidget* _parent) : Knob(knobStyled, _parent)
    {
        setFixedSize(31, 38);
    }
};
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    m_reverbButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("reverb_off"));
    ...
    */
    ...
    m_chorusButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("chorus_off"));
...
void sf2InstrumentView::showPatchDialog()
{
    ...
    pd.setup(k->m_synth, 1, k->instrumentTrack()->name(), &k->m_bankNum,
