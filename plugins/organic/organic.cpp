...
organicInstrument::organicInstrument(InstrumentTrack* _instrument_track) :
      ...
      m_modulationAlgo(Oscillator::SignalMix,
                       Oscillator::SignalMix,
                       Oscillator::SignalMix),
      m_fx1Model(0., 0., 0.99, 0.01, this, tr("Distortion")),
      m_volModel(100., 0., 200., 1., this, tr("Volume"))
{
    ...
    for(int i = 0; i < m_numOscillators; i++)
    {
        ...
        // Connect events
        connect(&m_osc[i]->m_oscModel, SIGNAL(dataChanged()), m_osc[i],
                SLOT(oscButtonChanged()));
        connect(&m_osc[i]->m_harmModel, SIGNAL(dataChanged()), m_osc[i],
                SLOT(updateDetuning()));
        connect(&m_osc[i]->m_volModel, SIGNAL(dataChanged()), m_osc[i],
                SLOT(updateVolume()));
        connect(&m_osc[i]->m_panModel, SIGNAL(dataChanged()), m_osc[i],
                SLOT(updateVolume()));
        connect(&m_osc[i]->m_detuneModel, SIGNAL(dataChanged()), m_osc[i],
                SLOT(updateDetuning()));

        m_osc[i]->updateVolume();
    }

    /*	m_osc[0]->m_harmonic = log2( 0.5 );	// one octave below
            m_osc[1]->m_harmonic = log2( 0.75 );	// a fifth below
            m_osc[2]->m_harmonic = log2( 1. );	// base freq
            m_osc[3]->m_harmonic = log2( 2. );	// first overtone
            m_osc[4]->m_harmonic = log2( 3. );	// second overtone
            m_osc[5]->m_harmonic = log2( 4. );	// .
    ...
    if(s_harmonics == nullptr)
    {
        ...
        s_harmonics[1]  = log2(0.75);
        s_harmonics[2]  = log2(1.);
        s_harmonics[3]  = log2(2.);
        s_harmonics[4]  = log2(3.);
        s_harmonics[5]  = log2(4.);
        s_harmonics[6]  = log2(5.);
        s_harmonics[7]  = log2(6.);
        s_harmonics[8]  = log2(7.);
        s_harmonics[9]  = log2(8.);
        s_harmonics[10] = log2(9.);
        s_harmonics[11] = log2(10.);
        s_harmonics[12] = log2(11.);
        s_harmonics[13] = log2(12.);
        s_harmonics[14] = log2(13.);
...
void organicInstrument::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
    ...
    for(int i = 0; i < m_numOscillators; ++i)
    {
        ...
        m_osc[i]->m_detuneModel.saveSettings(_doc, _this, "newdetune" + is);
        m_osc[i]->m_oscModel.saveSettings(_doc, _this, "wavetype" + is);
    }
}
...
void organicInstrument::loadSettings(const QDomElement& _this)
{
    ...
    for(int i = 0; i < m_numOscillators; ++i)
    {
        ...
        {
            m_osc[i]->m_detuneModel.setValue(_this.attribute("detune").toInt()
                                             * 12);
        }
        else
        {
        ...
        {
            m_osc[i]->m_harmModel.setValue(static_cast<real_t>(i));
        }
    }
        ...
        for(int i = m_numOscillators - 1; i >= 0; --i)
        {
            ...
            if(i == m_numOscillators - 1)
            {
                ...
                oscs_l[i] = new Oscillator(
                        &m_osc[i]->m_waveShape, &m_modulationAlgo,
                        _n->frequency(), m_osc[i]->m_detuningLeft,
                        static_cast<oscPtr*>(_n->m_pluginData)
                                ->phaseOffsetLeft[i],
                        m_osc[i]->m_volumeLeft);
                // create right oscillator
                oscs_r[i] = new Oscillator(
                        &m_osc[i]->m_waveShape, &m_modulationAlgo,
                        _n->frequency(), m_osc[i]->m_detuningRight,
                        static_cast<oscPtr*>(_n->m_pluginData)
                                ->phaseOffsetRight[i],
            ...
            else
            {
                ...
                oscs_r[i] = new Oscillator(
                        ...
                        static_cast<oscPtr*>(_n->m_pluginData)
                                ->phaseOffsetRight[i],
                        m_osc[i]->m_volumeRight, oscs_r[i + 1]);
            }
        }
...
/*real_t inline organicInstrument::foldback(sample_t in, real_t threshold)
{
  if (in>threshold || in<-threshold)
  {
    in= abs(abs(fmod(in - threshold, threshold*4.)) - threshold*2.) -
threshold;
  }
  return in;
}
...
real_t inline organicInstrument::waveshape(sample_t in, real_t amount)
{
    real_t k = 2. * amount / (1. - amount);
...
{
  public:
    organicKnob(QWidget* _parent) : Knob(knobStyled, _parent)
    {
        setFixedSize(21, 21);
    }
};
      ...
      m_oscKnobs(nullptr)
{
    ...
    m_fx1Knob->setWhatsThis(
            tr("The distortion knob adds distortion to the output of the "
               "instrument. "));
    ...
    m_randBtn->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("randomise_pressed"));
    m_randBtn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("randomise"));
    m_randBtn->setWhatsThis(tr(
            "The randomize button randomizes all knobs except the harmonics,"
...
void organicInstrumentView::modelChanged()
{
    ...
    const real_t y         = 91.;
    const real_t rowHeight = 26.;
    const real_t x         = 53.;
    const real_t colWidth  = 24.;
    ...
    for(int i = 0; i < m_numOscillators; ++i)
    {
        ...
        connect(&oi->m_osc[i]->m_harmModel, SIGNAL(dataChanged()), this,
                SLOT(updateKnobHint()));

        // setup waveform-knob
        Knob* oscKnob = new organicKnob(this);
        oscKnob->move(x + i * colWidth, y);
        connect(&oi->m_osc[i]->m_oscModel, SIGNAL(dataChanged()), this,
                SLOT(updateKnobHint()));
        ...
        panKnob->setHintText(tr("Osc %1 panning:").arg(i + 1), "");
        ...
        m_oscKnobs[i] = OscillatorKnobs(harmKnob, volKnob, oscKnob, panKnob,
                                        detuneKnob);
...
void organicInstrumentView::updateKnobHint()
{
    ...
    for(int i = 0; i < m_numOscillators; ++i)
    {
        const real_t harm = oi->m_osc[i]->m_harmModel.value();
        const real_t wave = oi->m_osc[i]->m_oscModel.value();

        m_oscKnobs[i].m_harmKnob->setHintText(
                tr("Osc %1 harmonic:").arg(i + 1),
                " (" + HARMONIC_NAMES[static_cast<int>(harm)] + ")");
        m_oscKnobs[i].m_oscKnob->setHintText(
                tr("Osc %1 waveform:").arg(i + 1),
...
OscillatorObject::OscillatorObject(Model* _parent, int _index) :
      ...
      m_harmModel(static_cast<real_t>(_index),
                  0.,
                  17.,
                  1.,
                  this,
                  tr("Osc %1 harmonic").arg(_index + 1)),
      m_volModel(
              100., 0., 100., 1., this, tr("Osc %1 volume").arg(_index + 1)),
      m_panModel(DefaultPanning,
                 PanningLeft,
                 PanningRight,
                 1.,
                 this,
                 tr("Osc %1 panning").arg(_index + 1)),
      m_detuneModel(0.,
                    -1200.,
                    1200.,
                    1.,
                    this,
                    tr("Osc %1 fine detuning left").arg(_index + 1))
{
}
...
void OscillatorObject::oscButtonChanged()
{
    ...
    static Oscillator::WaveShapes shapes[]
            = {Oscillator::SineWave,    Oscillator::SawWave,
               Oscillator::SquareWave,  Oscillator::TriangleWave,
               Oscillator::MoogSawWave, Oscillator::ExponentialWave};

    m_waveShape.setValue(shapes[(int)round(m_oscModel.value())]);
}

void OscillatorObject::updateVolume()
{
    m_volumeLeft = (1. - m_panModel.value() / (real_t)PanningRight)
                   * m_volModel.value() / m_numOscillators / 100.;
    m_volumeRight = (1. + m_panModel.value() / (real_t)PanningRight)
                    * m_volModel.value() / m_numOscillators / 100.;
}

void OscillatorObject::updateDetuning()
{
    m_detuningLeft = exp2(organicInstrument::s_harmonics[static_cast<int>(
                                  m_harmModel.value())]
                          + (real_t)m_detuneModel.value() * CENT)
                     / Engine::mixer()->processingSampleRate();
    m_detuningRight = exp2(organicInstrument::s_harmonics[static_cast<int>(
                                   m_harmModel.value())]
                           - (real_t)m_detuneModel.value() * CENT)
                      / Engine::mixer()->processingSampleRate();
}
 ...
 *
 m_osc[i].m_oscModel->setInitValue( 0. );
 * - randomize preset
 */
