...
extern "C"
{
    ...
    Plugin::Descriptor PLUGIN_EXPORT malletsstk_plugin_descriptor = {
            ...
            "Mallets",
            QT_TRANSLATE_NOOP("pluginBrowser", "Tuneful things to bang on"),
...
malletsInstrument::malletsInstrument(InstrumentTrack* _instrument_track) :
      ...
      m_versionModel(MALLETS_PRESET_VERSION,
                     ...
                     this,
                     "[mallets version]"),
      m_isOldVersionModel(false, this, "[is old version]"),
      m_filesMissing(
              !QDir(ConfigManager::inst()->stkDir()).exists()
              || !QFileInfo(ConfigManager::inst()->stkDir() + "/sinewave.raw")
                          .exists())
{
    // ModalBar
                                 ...
                                 sampleFrame*    _working_buffer)
{
    ...
    if(_n->totalFramesPlayed() == 0 || _n->m_pluginData == nullptr)
    {
        ...
        if(p < 9)
        {
            _n->m_pluginData = new malletsSynth(
                    freq, vel, m_stickModel.value(), m_hardnessModel.value(),
                    m_positionModel.value(), m_vibratoGainModel.value(),
                    m_vibratoFreqModel.value(), p,
        ...
        {
            _n->m_pluginData = new malletsSynth(
                    freq, vel, p, m_lfoDepthModel.value(),
                    m_modulatorModel.value(), m_crossfadeModel.value(),
                    m_lfoSpeedModel.value(), m_adsrModel.value(),
                    (uint8_t)m_spreadModel.value(),
                    Engine::mixer()->processingSampleRate());
        }
        else
        {
            _n->m_pluginData = new malletsSynth(
                    freq, vel, m_pressureModel.value(), m_motionModel.value(),
                    m_vibratoModel.value(), p - 10,
    ...
    {
        add_scale
                = static_cast<sample_t>(m_strikeModel.value()) * freq * 2.5f;
    }
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    connect(&_instrument->m_presetsModel, SIGNAL(dataChanged()), this,
            SLOT(changePreset()));
...
void malletsInstrumentView::setWidgetBackground(QWidget*       _widget,
                                                const QString& _pic)
{
