...
MultitapEchoControls::MultitapEchoControls(MultitapEchoEffect* eff) :
      EffectControls(eff), m_effect(eff), m_steps(16, 4, 32, this, "Steps"),
      ...
      m_ampGraph(-60.0f, 0.0f, 16, this), m_lpGraph(0.0f, 3.0f, 16, this)
{
    m_stages.setStrictStepSize(true);
    connect(&m_ampGraph, SIGNAL(samplesChanged(int, int)), this,
            SLOT(ampSamplesChanged(int, int)));
    connect(&m_lpGraph, SIGNAL(samplesChanged(int, int)), this,
            SLOT(lpSamplesChanged(int, int)));
...
void MultitapEchoControls::saveSettings(QDomDocument& doc,
                                        QDomElement&  parent)
{
    ...
    parent.setAttribute(
            "ampsteps",
            base64::encodeFloats(m_ampGraph.samples(), m_ampGraph.length()));
    parent.setAttribute("lpsteps", base64::encodeFloats(m_lpGraph.samples(),
                                                        m_lpGraph.length()));
}
...
void MultitapEchoControls::loadSettings(const QDomElement& elem)
{
    ...
    base64::decodeFloats(elem.attribute("ampsteps"), &dst);
    m_ampGraph.setSamples(dst);
    delete[] dst;

    base64::decodeFloats(elem.attribute("lpsteps"), &dst);
    m_lpGraph.setSamples(dst);
    delete[] dst;
}
