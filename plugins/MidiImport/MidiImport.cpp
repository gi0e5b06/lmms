...
MidiImport::MidiImport(const QString& _file) :
      ImportFilter(_file, &midiimport_plugin_descriptor), m_events(),
      m_timingDivision(0)
{
}
...
bool MidiImport::tryImport(TrackContainer* tc)
{
    ...
    if(gui != NULL && ConfigManager::inst()->defaultSoundfont().isEmpty())
    {
        QMessageBox::information(
                gui->mainWindow(), tr("Setup incomplete"),
    ...
    {
        case makeID('M', 'T', 'h', 'd'):
            qInfo("MidiImport::tryImport(): found MThd");
            return readSMF(tc);

        case makeID('R', 'I', 'F', 'F'):
            qInfo("MidiImport::tryImport(): found RIFF");
            return readRIFF(tc);

        default:
            qInfo("MidiImport::tryImport(): not a Standard MIDI file");
            return false;
    }
}
...
class smfMidiCC
{
  ...
  public:
    smfMidiCC() : at(NULL), ap(NULL), lastPos(0)
    {
    }
    ...
    smfMidiCC&
            putValue(MidiTime time, AutomatableModel* objModel, float value)
    {
...
class smfMidiChannel
{
  ...
  public:
    smfMidiChannel() :
          it(NULL), p(NULL), it_inst(NULL), isSF2(false), hasNotes(false),
          lastEnd(0)
    {
    }
    ...
    smfMidiChannel* create(TrackContainer* tc, QString tn)
    {
        if(!it)
        {
            ...
            it->bendingRangeModel()->setInitValue(2);
        }
        return this;
    }
...
bool MidiImport::readSMF(TrackContainer* tc)
{
    ...
    QProgressDialog pd(TrackContainer::tr("Importing MIDI-file..."),
                       TrackContainer::tr("Cancel"), 0, preTrackSteps,
                       gui->mainWindow());
    ...
    for(int s = 0; s < timeSigs->length(); ++s)
    {
        ...
        if(/* timeSig.beat == 0*/ true)
        {
            ...
            qInfo("Another timesig at %f", timeSig.beat);
            timeSigNumeratorPat->putValue(timeSig.beat * ticksPerBeat,
                                          timeSig.num);
            timeSigDenominatorPat->putValue(timeSig.beat * ticksPerBeat,
                                            timeSig.den);
        }
        else
        {
    ...
    for(int e = 0; e < seq->length(); ++e)
    {
        ...
        if(evt->is_update())
        {
            qWarning("Unhandled SONG update: %d %f %s", evt->get_type_code(),
                     evt->time, evt->get_attribute());
        }
    }
        ...
        for(int e = 0; e < trk->length(); ++e)
        {
            ...
            if(evt->chan == -1)
            {
                ...
                if(!handled)
                {
                    ...
                    if(evt->is_update())
                    {
                        s.append(", Update Type: ");
                        s.append(evt->get_attribute());
                        if(evt->get_update_type() == 'a')
                        {
                            s.append(", Atom: ");
                            s.append(evt->get_atom_value());
                        }
                    }
                    qWarning(
                            "MISSING GLOBAL HANDLER\n"
                            "     Chn: %d, Type Code: %d, Time: %f %s",
            ...
            else if(evt->is_note() && evt->chan < 256)
            {
                ...
                Note n((ticks < 1 ? 1 : ticks),
                       ...
                       noteEvt->get_identifier() - 12, noteEvt->get_loud());
                ch->addNote(n);
            }
            ...
            else if(evt->is_update())
            {
                ...
                if(update == "programi")
                {
                    ...
                    else
                    {
                        ...
                        const QString filter
                                = QString().fill('0', 3 - num.length()) + num
                                  + "*.pat";
                        const QString dir
                                = "/usr/share/midi/"
                                  "freepats/Tone_000/";
                        const QStringList files
                                = QDir(dir).entryList(QStringList(filter));
                    ...
                    if(ccid <= 128)
                    {
                        ...
                        switch(ccid)
                        {
                            case 0:
                                ...
                                {
                                    objModel
                                            = ch->it_inst->childModel("bank");
                                    qInfo("BANK SELECT %f %d", cc,
                                          (int)(cc * 127.0));
                                    cc *= 127.0f;
                                }
                                break;
                ...
                {
                    qWarning("Unhandled update: %d %d %f %s", (int)evt->chan,
    ...
    for(int c = 0; c < 256; ++c)
    {
        ...
        {
            qWarning("Should remove empty track\n");
...
{
    qWarning("MidiImport::readTrack(): invalid MIDI data (offset %#x)",
