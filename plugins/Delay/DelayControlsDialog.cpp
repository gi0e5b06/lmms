      ...
      EffectControlDialog(controls)
{
    ...
    sampleDelayKnob->setText(tr("DELAY"));
    sampleDelayKnob->setHintText(tr("Delay Time") + " ", " s");

    VolumeKnob* feedbackKnob = new VolumeKnob(knobBright_26, this);
    ...
    XyPad* pad = new XyPad(this, &controls->m_feedbackModel,
                           &controls->m_delayTimeModel);
    pad->resize(200, 200);
    pad->move(50, 5);
}

XyPad::XyPad(QWidget* parent, FloatModel* xModel, FloatModel* yModel) :
      QWidget(parent), m_xModel(xModel), m_yModel(yModel),
      m_acceptInput(false)
{
...
void XyPad::paintEvent(QPaintEvent* event)
{
    ...
    painter.setPen(QPen(QColor(200, 200, 200, 200), 8, Qt::SolidLine,
