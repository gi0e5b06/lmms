...
CompressorControlDialog::CompressorControlDialog(
      ...
      m_controls(controls)
{
    ...
    ToolTip::add(m_thresholdKnob,
                 tr("Volume at which the compression begins to take place"));

    m_ratioKnob = new Knob(knobStyled, this);
    makeLargeKnob(m_ratioKnob, tr("Ratio:"), ":1");
    m_ratioKnob->setModel(&controls->m_ratioModel);
    ToolTip::add(m_ratioKnob, tr("How far the compressor must turn the "
                                 "volume down after crossing the threshold"));
    ...
    ToolTip::add(
            m_attackKnob,
            tr("Speed at which the compressor starts to compress the audio"));
    ...
    ToolTip::add(
            m_releaseKnob,
            tr("Speed at which the compressor ceases to compress the audio"));
    ...
    ToolTip::add(
            m_kneeKnob,
            tr("Smooth out the gain reduction curve around the threshold"));
    ...
    ToolTip::add(m_lookaheadLengthKnob,
                 tr("How long the compressor has to react to the sidechain "
    ...
    ToolTip::add(m_inBalanceKnob,
                 tr("Bias the input audio to the left/right or mid/side"));
    ...
    ToolTip::add(m_outBalanceKnob,
                 tr("Bias the output audio to the left/right or mid/side"));
    ...
    ToolTip::add(
            m_stereoBalanceKnob,
            tr("Bias the sidechain signal to the left/right or mid/side"));
    ...
    ToolTip::add(m_blendKnob,
                 tr("Blend between unlinked/maximum/average/minimum stereo "
                    "linking modes"));
    ...
    ToolTip::add(m_tiltKnob,
                 tr("Bias the sidechain signal to the low or high "
                    "frequencies.  -6 db is lowpass, 6 db is highpass."));
    ...
    ToolTip::add(m_tiltFreqKnob,
                 tr("Center frequency of sidechain tilt filter"));
    ...
    ToolTip::add(m_autoAttackKnob, tr("Automatically control attack value "
                                      "depending on crest factor"));
    ...
    ToolTip::add(m_autoReleaseKnob, tr("Automatically control release value "
                                       "depending on crest factor"));

    m_outFader
            = new EqFader(&controls->m_outGainModel, tr("Output gain"), this,
    ...
    m_inFader = new EqFader(&controls->m_inGainModel, tr("Input gain"), this,
    ...
    leftRightButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("leftright_sel"));
    leftRightButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("leftright_unsel"));
    ToolTip::add(leftRightButton, tr("Compress left and right audio"));

    midSideButton = new PixmapButton(this, tr("Mid/Side"));
    midSideButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("midside_sel"));
    midSideButton->setInactiveGraphic(
    ...
    compressButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("compressor_sel"));
    compressButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("compressor_unsel"));
    ...
    limitButton->setInactiveGraphic(
    ...
    unlinkedButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("unlinked_sel"));
    unlinkedButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("unlinked_unsel"));
    ToolTip::add(unlinkedButton, tr("Compress each channel separately"));

    maximumButton = new PixmapButton(this, tr("Maximum"));
    maximumButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("maximum_sel"));
    maximumButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("maximum_unsel"));
    ToolTip::add(maximumButton, tr("Compress based on the loudest channel"));

    averageButton = new PixmapButton(this, tr("Average"));
    averageButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("average_sel"));
    averageButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("average_unsel"));
    ToolTip::add(averageButton,
                 tr("Compress based on the averaged channel volume"));

    minimumButton = new PixmapButton(this, tr("Minimum"));
    minimumButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("minimum_sel"));
    minimumButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("minimum_unsel"));
    ...
    blendButton->setInactiveGraphic(
    ...
    autoMakeupButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("autogain_sel"));
    autoMakeupButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("autogain_unsel"));
    ToolTip::add(autoMakeupButton,
                 tr("Automatically change makeup gain depending on "
                    "threshold, knee, and ratio settings"));
    ...
    auditionButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("audition_sel"));
    auditionButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("audition_unsel"));
    ...
    feedbackButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("feedback_sel"));
    feedbackButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("feedback_unsel"));
    ToolTip::add(feedbackButton,
                 tr("Use the compressor's output as the sidechain input"));
    ...
    lookaheadButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("lookahead_sel"));
    lookaheadButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("lookahead_unsel"));
    ToolTip::add(lookaheadButton, tr("Enable Lookahead, which introduces 20 "
    ...
    connect(&m_controls->m_peakmodeModel, SIGNAL(dataChanged()), this,
            SLOT(peakmodeChanged()));
    connect(&m_controls->m_stereoLinkModel, SIGNAL(dataChanged()), this,
            SLOT(stereoLinkChanged()));
    connect(&m_controls->m_lookaheadModel, SIGNAL(dataChanged()), this,
            SLOT(lookaheadChanged()));
    connect(&m_controls->m_limiterModel, SIGNAL(dataChanged()), this,
            SLOT(limiterChanged()));
...
void CompressorControlDialog::makeLargeKnob(Knob*   knob,
                                            QString hint,
                                            QString unit)
{
...
void CompressorControlDialog::makeSmallKnob(Knob*   knob,
                                            QString hint,
                                            QString unit)
{
    knob->setHintText(hint, unit);
...
void CompressorControlDialog::stereoLinkChanged()
{
    ...
    m_blendEnabledLabel->setVisible(m_controls->m_stereoLinkModel.value()
                                    == 4);
}
...
void CompressorControlDialog::updateDisplay()
{
    ...
    if(!m_controls->m_effect->isEnabled()
       || !m_controls->m_effect->isRunning())
    {
    ...
    const real_t peakAvg = (m_controls->m_effect->m_displayPeak[0]
                            + m_controls->m_effect->m_displayPeak[1])
                           * 0.5;
    const real_t gainAvg = (m_controls->m_effect->m_displayGain[0]
                            + m_controls->m_effect->m_displayGain[1])
                           * 0.5;
    ...
    p.fillRect(m_windowSizeX - compPixelMovement, 0, m_windowSizeX,
               m_windowSizeY, QColor("transparent"));
    ...
    for(int i = 0; i < compPixelMovement; ++i)
    {
        const int temp = linearInterpolate(
                m_lastPoint, yPoint, FLOAT(i) / FLOAT(compPixelMovement));
        p.drawLine(m_windowSizeX - compPixelMovement + i, temp,
                   m_windowSizeX - compPixelMovement + i, m_windowSizeY);
    }

    // Draw input volume line
    p.setPen(QPen(m_inVolColor, 1));
    p.drawLine(m_windowSizeX - compPixelMovement - 1, m_lastPoint,
               m_windowSizeX, yPoint);
    ...
    {
        const int temp = linearInterpolate(
                m_lastPoint + m_lastGainPoint, yPoint + yGainPoint,
                FLOAT(i) / FLOAT(compPixelMovement));
        p.drawLine(m_windowSizeX - compPixelMovement + i, temp,
                   m_windowSizeX - compPixelMovement + i, m_windowSizeY);
    }
    ...
    p.drawLine(m_windowSizeX - compPixelMovement - 1,
               m_lastPoint + m_lastGainPoint, m_windowSizeX,
               yPoint + yGainPoint);

    // Draw gain reduction line
    p.setPen(QPen(m_gainReductionColor, 2));
    p.drawLine(m_windowSizeX - compPixelMovement - 1, m_lastGainPoint,
               m_windowSizeX, yGainPoint);
    ...
    if(m_controls->m_effect->m_redrawKnee)
    {
        ...
        // Clear display
        p.setCompositionMode(QPainter::CompositionMode_Source);
        p.fillRect(0, 0, m_windowSizeX, m_kneeWindowSizeY,
                   QColor("transparent"));
        p.setCompositionMode(QPainter::CompositionMode_SourceOver);
        ...
        real_t actualRatio = m_controls->m_limiterModel.value()
                                     ? 0
                                     : m_controls->m_effect->m_ratioVal;

        // Calculate endpoints for the two straight lines
        real_t kneePoint1 = m_controls->m_effect->m_thresholdVal
                            - m_controls->m_effect->m_kneeVal;
        real_t kneePoint2X = m_controls->m_effect->m_thresholdVal
                             + m_controls->m_effect->m_kneeVal;
        real_t kneePoint2Y
        ...
        real_t ratioPoint
                = m_controls->m_effect->m_thresholdVal
                  + (-m_controls->m_effect->m_thresholdVal * actualRatio);

        // Draw two straight lines
        p.drawLine(0, m_kneeWindowSizeY, dbfsToXPoint(kneePoint1),
                   dbfsToYPoint(kneePoint1));
        if(dbfsToXPoint(kneePoint2X) < m_kneeWindowSizeY)
        {
            p.drawLine(dbfsToXPoint(kneePoint2X), dbfsToYPoint(kneePoint2Y),
                       m_kneeWindowSizeY, dbfsToYPoint(ratioPoint));
        }
        ...
        if(m_controls->m_effect->m_kneeVal)
        {
            ...
            real_t prevPoint[2] = {kneePoint1, kneePoint1};
            real_t newPoint[2]  = {0, 0};

            // Draw knee curve using many straight lines.
            for(int i = 0; i < COMP_KNEE_LINES; ++i)
            {
                newPoint[0] = linearInterpolate(
                        kneePoint1, kneePoint2X,
                        (i + 1) / (real_t)COMP_KNEE_LINES);

                const real_t temp = newPoint[0]
                                    - m_controls->m_effect->m_thresholdVal
                                    + m_controls->m_effect->m_kneeVal;
                newPoint[1]
                        = (newPoint[0]
                           + (actualRatio - 1) * temp * temp
                                     / (4 * m_controls->m_effect->m_kneeVal));

                p.drawLine(dbfsToXPoint(prevPoint[0]),
                           dbfsToYPoint(prevPoint[1]),
                           dbfsToXPoint(newPoint[0]),
        ...
        // Erase right portion
        p.setCompositionMode(QPainter::CompositionMode_Source);
        p.fillRect(m_kneeWindowSizeX + 1, 0, m_windowSizeX, m_kneeWindowSizeY,
                   QColor("transparent"));
        p.setCompositionMode(QPainter::CompositionMode_SourceOver);
    ...
    {
        QRectF knee2Rect
                = QRect(m_lastKneePoint, 0, kneePoint - m_lastKneePoint,
                        m_kneeWindowSizeY);
    ...
    {
        p.setCompositionMode(QPainter::CompositionMode_Source);
        p.fillRect(kneePoint, 0, m_lastKneePoint, m_kneeWindowSizeY,
                   QColor("transparent"));
        p.setCompositionMode(QPainter::CompositionMode_SourceOver);
    }
...
inline int CompressorControlDialog::dbfsToYPoint(real_t inDbfs)
{
    return (-((inDbfs + m_dbRange) / m_dbRange) + 1) * m_windowSizeY;
}

inline int CompressorControlDialog::dbfsToXPoint(real_t inDbfs)
{
...
{
    const real_t temp = m_dbRange;
    m_dbRange = round(qBound(3., m_dbRange - event->delta() / 20., 96.) / 3.)
                * 3.;
...
void CompressorControlDialog::resetGraph()
{
    ...
    p.setFont(QFont("Arial", qMax((m_windowSizeY * 24) / 1080, 12)));

    // Redraw graph
    p.setPen(QPen(m_graphColor, 1));
    for(int i = 1; i < m_dbRange / 3. + 1.; ++i)
    {
        p.drawLine(0, dbfsToYPoint(-3 * i), m_windowSizeX,
                   dbfsToYPoint(-3 * i));
        p.drawLine(dbfsToXPoint(-3 * i), 0, dbfsToXPoint(-3 * i),
                   m_kneeWindowSizeY);
        p.drawText(QRectF(m_windowSizeX - 50, dbfsToYPoint(-3 * i), 50, 50),
                   Qt::AlignRight | Qt::AlignTop, QString::number(i * -3));
    }
...
QColor const& CompressorControlDialog::inVolAreaColor() const
{
    return m_inVolAreaColor;
}
QColor const& CompressorControlDialog::inVolColor() const
{
    return m_inVolColor;
}
QColor const& CompressorControlDialog::outVolAreaColor() const
{
    return m_outVolAreaColor;
}
QColor const& CompressorControlDialog::outVolColor() const
{
    return m_outVolColor;
}
QColor const& CompressorControlDialog::gainReductionColor() const
{
    return m_gainReductionColor;
}
QColor const& CompressorControlDialog::kneeColor() const
{
    return m_kneeColor;
}
QColor const& CompressorControlDialog::kneeColor2() const
{
    return m_kneeColor2;
}
QColor const& CompressorControlDialog::threshColor() const
{
    return m_threshColor;
}
QColor const& CompressorControlDialog::textColor() const
{
    return m_textColor;
}
QColor const& CompressorControlDialog::graphColor() const
{
    return m_graphColor;
}
QColor const& CompressorControlDialog::resetColor() const
{
    return m_resetColor;
}

void CompressorControlDialog::setInVolAreaColor(const QColor& c)
{
    m_inVolAreaColor = c;
}
void CompressorControlDialog::setInVolColor(const QColor& c)
{
    m_inVolColor = c;
}
void CompressorControlDialog::setOutVolAreaColor(const QColor& c)
{
    m_outVolAreaColor = c;
}
void CompressorControlDialog::setOutVolColor(const QColor& c)
{
    m_outVolColor = c;
}
void CompressorControlDialog::setGainReductionColor(const QColor& c)
{
    m_gainReductionColor = c;
}
void CompressorControlDialog::setKneeColor(const QColor& c)
{
    m_kneeColor = c;
}
void CompressorControlDialog::setKneeColor2(const QColor& c)
{
    m_kneeColor2 = c;
}
void CompressorControlDialog::setThreshColor(const QColor& c)
{
    m_threshColor = c;
}
void CompressorControlDialog::setTextColor(const QColor& c)
{
    m_textColor = c;
}
void CompressorControlDialog::setGraphColor(const QColor& c)
{
    m_graphColor = c;
}
void CompressorControlDialog::setResetColor(const QColor& c)
{
    m_resetColor = c;
}
