...
CsoundInstrObject::CsoundInstrObject( float * _A1wave, float * _A2wave,
	...
	//m_sco()
{
	...
	if(m_csSession->CompileOrc(orc1.c_str()) == 0)
	{
		...
		cout << m_csSession->GetNchnls() << ", " << m_csSession->Get0dBFS() << endl;
...
void CsoundInstrObject::noMessageCallback(CSOUND* cs, int attr, const char *format, va_list valist)
{
...
void CsoundInstrObject::renderOutput( fpp_t _frames )
{
	...
	if(m_csSession->PerformKsmps()==0)
	{
		...
		while((frame<_frames)&&(i<m_csSession->GetKsmps()))
		{
			...
			m_abuf[frame][0]=v;
			m_abuf[frame][1]=(frame<50 ? sin((float)frame/_frames) : 0.5f);
			//m_bbuf[frame][0]=v;
			//m_bbuf[frame][1]=v;
			frame++;
			i++;
...
CsoundInstrInstrument::CsoundInstrInstrument( InstrumentTrack * _instrument_track ) :
						...
						sampleFrame * _working_buffer )
{
	...
	*/
	float pos=0; //now
	float len=1.f*_n->length().getTicks()/Engine::getSong()->ticksPerTact();
...
CsoundInstrView::CsoundInstrView( Instrument * _instrument,
					QWidget * _parent ) :
...
{
	return( new CsoundInstrInstrument( static_cast<InstrumentTrack *>( _data ) ) );
}
