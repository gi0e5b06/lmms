...
VstEffect::VstEffect(Model*                                    _parent,
      ...
      m_plugin(NULL), m_pluginMutex(), m_key(*_key), m_vstControls(this)
{
...
bool VstEffect::handleMidiEvent(const MidiEvent& event,
                                const MidiTime&,
                                f_cnt_t offset)
{
    qInfo("VstEffect::handleMidiEvent");
...
void VstEffect::openPlugin(const QString& _plugin)
{
    ...
    VstPlugin::connect(Engine::getSong(), SIGNAL(tempoChanged(bpm_t)),
