...
bSynth::bSynth(const FLOAT*        _shape,
               int                 _length,
               NotePlayHandle*     _nph,
               bool                _interpolation,
               real_t              _factor,
      ...
      sample_index(0),
      sample_realindex(0), nph(_nph), sample_length(_length),
      sample_rate(_sample_rate), interpolation(_interpolation)
{
    sample_shape = new real_t[sample_length];
...
{
    real_t sample_step
            = real_t(sample_length / (sample_rate / nph->frequency()));
    ...
    if(interpolation)
    {
        ...
        // Nachkommaanteil
        const real_t frac = fraction(sample_realindex);
...
bitInvader::bitInvader(InstrumentTrack* _instrument_track) :
      ...
      m_graph(-1.0f, 1.0f, 128, this), m_interpolation(false, this),
      m_normalize(false, this)
{

    m_graph.setWaveToSine();

    connect(&m_sampleLength, SIGNAL(dataChanged()), this,
            SLOT(lengthChanged()));

    connect(&m_graph, SIGNAL(samplesChanged(int, int)), this,
...
void bitInvader::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
    ...
    _this.setAttribute("sampleShape",
                       base64::encodeFloats(m_graph.samples(), m_graph.length()));
...
void bitInvader::playNote(NotePlayHandle* _n, sampleFrame* _working_buffer)
{
    if(_n->totalFramesPlayed() == 0 || _n->m_pluginData == NULL)
    {
        ...
        _n->m_pluginData
                = new bSynth(m_graph.samples(), m_graph.length(), _n,
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    m_interpolationToggle = new LedCheckBox(
            "Interpolation", this, tr("Interpolation"), LedCheckBox::Yellow);
    m_interpolationToggle->move(131, 221);

    m_normalizeToggle = new LedCheckBox("Normalize", this, tr("Normalize"),
                                        LedCheckBox::Green);
    ...
    connect(m_interpolationToggle, SIGNAL(toggled(bool)), this,
            SLOT(interpolationToggled(bool)));
...
void bitInvaderView::usrWaveClicked()
{
    ...
    if ( af != "" )
    {
            ...
            for ( int i = 0; i < sample_length; i++ )
            {
                    sample_shape[i] = (real_t)*buffer.data()[i];
            }
    }
