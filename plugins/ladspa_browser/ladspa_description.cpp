      ...
      QWidget(_parent)
{
    ...
    GroupBox*    pluginsBox = new GroupBox(tr("Plugins"), this, false, false);
    ...
    // pluginsLayout->addWidget(pluginList);
    pluginsBox->setContentWidget(pluginList);

    GroupBox* descriptionBox
            = new GroupBox(tr("Description"), this, false, false);
    ...
    descriptionBox->setContentWidget(m_scrollArea);
...
void ladspaDescription::update(const ladspa_key_t& _key)
{
    ...
    channelsIn->setText(
            QWidget::tr("Channels In: ")
            + QString::number(manager->getDescription(_key)->inputChannels));
    layout->addWidget(channelsIn);

    QLabel* channelsOut = new QLabel(description);
    channelsOut->setText(
            QWidget::tr("Channels Out: ")
            + QString::number(manager->getDescription(_key)->outputChannels));
