                                ...
                                sampleFrame*    _working_buffer)
{
    ...
    // ... mix envelope is inactive or we've past the end of envelope, so use
    // a faster calculation to save cpu
    else
    {
        // get knob values
...
void WatsynInstrument::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
    ...
    // save graphs
    _this.setAttribute("a1_wave", base64::encodeFloats(a1_graph.samples(),
                                                       a1_graph.length()));
    _this.setAttribute("a2_wave", base64::encodeFloats(a2_graph.samples(),
                                                       a2_graph.length()));
    _this.setAttribute("b1_wave", base64::encodeFloats(b1_graph.samples(),
                                                       b1_graph.length()));
    _this.setAttribute("b2_wave", base64::encodeFloats(b2_graph.samples(),
                                                       b2_graph.length()));
...
void WatsynInstrument::loadSettings(const QDomElement& _this)
{
    ...
    base64::decodeFloats(_this.attribute("a1_wave"), &dst);
    a1_graph.setSamples(dst);
    delete[] dst;

    base64::decodeFloats(_this.attribute("a2_wave"), &dst);
    a2_graph.setSamples(dst);
    delete[] dst;

    base64::decodeFloats(_this.attribute("b1_wave"), &dst);
    b1_graph.setSamples(dst);
    delete[] dst;

    base64::decodeFloats(_this.attribute("b2_wave"), &dst);
    b2_graph.setSamples(dst);
    delete[] dst;
