      ...
      m_parent(_i), m_nph(_nph)
{
    ...
    m_lfo_next[0] = Oscillator::noiseSample(0.);
    m_lfo_next[1] = Oscillator::noiseSample(0.);
...
{
    real_t modtmp;  // temp variable for freq modulation
// macros for modulating with env/lfos
#define modulatefreq(car, mod)          \
    modtmp = 0.;                      \
    if(mod##_e1 != 0.)                \
        modtmp += env[0][f] * mod##_e1; \
    if(mod##_e2 != 0.)                \
        modtmp += env[1][f] * mod##_e2; \
    if(mod##_l1 != 0.)                \
        modtmp += lfo[0][f] * mod##_l1; \
    if(mod##_l2 != 0.)                \
        modtmp += lfo[1][f] * mod##_l2; \
    car = bound(MIN_FREQ, car * fastpow(2., modtmp), MAX_FREQ);
...
#define modulatevol(car, mod)                            \
    if(mod##_e1 > 0.)                                  \
        car *= (1. - mod##_e1 + mod##_e1 * env[0][f]); \
    if(mod##_e1 < 0.)                                  \
        car *= (1. + mod##_e1 * env[0][f]);            \
    if(mod##_e2 > 0.)                                  \
        car *= (1. - mod##_e2 + mod##_e2 * env[1][f]); \
    if(mod##_e2 < 0.)                                  \
        car *= (1. + mod##_e2 * env[1][f]);            \
    if(mod##_l1 != 0.)                                 \
        car *= (1. + mod##_l1 * lfo[0][f]);            \
    if(mod##_l2 != 0.)                                 \
        car *= (1. + mod##_l2 * lfo[1][f]);            \
    car = bound(-MODCLIP, car, MODCLIP);
    ...
    // LFO phase offsets
    const real_t lfo1_po = m_parent->m_lfo1Phs.value() / 360.;
    const real_t lfo2_po = m_parent->m_lfo2Phs.value() / 360.;
    ...
    m_lfo_rate[0] = (m_parent->m_lfo1Rate.value() * 0.001
                     * m_parent->m_samplerate);
    m_lfo_rate[1] = (m_parent->m_lfo2Rate.value() * 0.001
                     * m_parent->m_samplerate);
    m_lfo_inc[0]  = 1. / m_lfo_rate[0];
    m_lfo_inc[1]  = 1. / m_lfo_rate[1];
    ...
    // get pulse width
    const real_t pw       = (m_parent->m_osc1Pw.value() * 0.01);
    const real_t o1pw_e1  = (m_parent->m_pw1env1.value());
    const real_t o1pw_e2  = (m_parent->m_pw1env2.value());
    const real_t o1pw_l1  = (m_parent->m_pw1lfo1.value() * 0.5);
    const real_t o1pw_l2  = (m_parent->m_pw1lfo2.value() * 0.5);
    const bool   o1pw_mod = o1pw_e1 != 0. || o1pw_e2 != 0.
                          || o1pw_l1 != 0. || o1pw_l2 != 0.;

    // get phases
    const real_t o1lpo   = m_parent->m_osc1l_po;
    const real_t o1rpo   = m_parent->m_osc1r_po;
    const real_t o1p_e1  = (m_parent->m_phs1env1.value());
    const real_t o1p_e2  = (m_parent->m_phs1env2.value());
    const real_t o1p_l1  = (m_parent->m_phs1lfo1.value() * 0.5);
    const real_t o1p_l2  = (m_parent->m_phs1lfo2.value() * 0.5);
    const bool   o1p_mod = o1p_e1 != 0. || o1p_e2 != 0. || o1p_l1 != 0.
                         || o1p_l2 != 0.;

    // get pitch
    const real_t o1lfb   = (m_parent->m_osc1l_freq * m_nph->frequency());
    const real_t o1rfb   = (m_parent->m_osc1r_freq * m_nph->frequency());
    const real_t o1f_e1  = (m_parent->m_pit1env1.value() * 2.);
    const real_t o1f_e2  = (m_parent->m_pit1env2.value() * 2.);
    const real_t o1f_l1  = (m_parent->m_pit1lfo1.value());
    const real_t o1f_l2  = (m_parent->m_pit1lfo2.value());
    const bool   o1f_mod = o1f_e1 != 0. || o1f_e2 != 0. || o1f_l1 != 0.
                         || o1f_l2 != 0.;

    // get volumes
    const real_t o1lv    = m_parent->m_osc1l_vol;
    const real_t o1rv    = m_parent->m_osc1r_vol;
    const real_t o1v_e1  = (m_parent->m_vol1env1.value());
    const real_t o1v_e2  = (m_parent->m_vol1env2.value());
    const real_t o1v_l1  = (m_parent->m_vol1lfo1.value());
    const real_t o1v_l2  = (m_parent->m_vol1lfo2.value());
    const bool   o1v_mod = o1v_e1 != 0. || o1v_e2 != 0. || o1v_l1 != 0.
                         || o1v_l2 != 0.;
    ...
    // get phases
    const real_t o2lpo   = m_parent->m_osc2l_po;
    const real_t o2rpo   = m_parent->m_osc2r_po;
    const real_t o2p_e1  = (m_parent->m_phs2env1.value());
    const real_t o2p_e2  = (m_parent->m_phs2env2.value());
    const real_t o2p_l1  = (m_parent->m_phs2lfo1.value() * 0.5);
    const real_t o2p_l2  = (m_parent->m_phs2lfo2.value() * 0.5);
    const bool   o2p_mod = o2p_e1 != 0. || o2p_e2 != 0. || o2p_l1 != 0.
                         || o2p_l2 != 0.;

    // get pitch
    const real_t o2lfb   = (m_parent->m_osc2l_freq * m_nph->frequency());
    const real_t o2rfb   = (m_parent->m_osc2r_freq * m_nph->frequency());
    const real_t o2f_e1  = (m_parent->m_pit2env1.value() * 2.);
    const real_t o2f_e2  = (m_parent->m_pit2env2.value() * 2.);
    const real_t o2f_l1  = (m_parent->m_pit2lfo1.value());
    const real_t o2f_l2  = (m_parent->m_pit2lfo2.value());
    const bool   o2f_mod = o2f_e1 != 0. || o2f_e2 != 0. || o2f_l1 != 0.
                         || o2f_l2 != 0.;

    // get volumes
    const real_t o2lv    = m_parent->m_osc2l_vol;
    const real_t o2rv    = m_parent->m_osc2r_vol;
    const real_t o2v_e1  = (m_parent->m_vol2env1.value());
    const real_t o2v_e2  = (m_parent->m_vol2env2.value());
    const real_t o2v_l1  = (m_parent->m_vol2lfo1.value());
    const real_t o2v_l2  = (m_parent->m_vol2lfo2.value());
    const bool   o2v_mod = o2v_e1 != 0. || o2v_e2 != 0. || o2v_l1 != 0.
                         || o2v_l2 != 0.;
    ...
    // get phases
    const real_t o3lpo   = m_parent->m_osc3l_po;
    const real_t o3rpo   = m_parent->m_osc3r_po;
    const real_t o3p_e1  = (m_parent->m_phs3env1.value());
    const real_t o3p_e2  = (m_parent->m_phs3env2.value());
    const real_t o3p_l1  = (m_parent->m_phs3lfo1.value() * 0.5);
    const real_t o3p_l2  = (m_parent->m_phs3lfo2.value() * 0.5);
    const bool   o3p_mod = o3p_e1 != 0. || o3p_e2 != 0. || o3p_l1 != 0.
                         || o3p_l2 != 0.;

    // get pitch modulators
    const real_t o3fb    = (m_parent->m_osc3_freq * m_nph->frequency());
    const real_t o3f_e1  = (m_parent->m_pit3env1.value() * 2.);
    const real_t o3f_e2  = (m_parent->m_pit3env2.value() * 2.);
    const real_t o3f_l1  = (m_parent->m_pit3lfo1.value());
    const real_t o3f_l2  = (m_parent->m_pit3lfo2.value());
    const bool   o3f_mod = o3f_e1 != 0. || o3f_e2 != 0. || o3f_l1 != 0.
                         || o3f_l2 != 0.;

    // get volumes
    const real_t o3lv    = m_parent->m_osc3l_vol;
    const real_t o3rv    = m_parent->m_osc3r_vol;
    const real_t o3v_e1  = (m_parent->m_vol3env1.value());
    const real_t o3v_e2  = (m_parent->m_vol3env2.value());
    const real_t o3v_l1  = (m_parent->m_vol3lfo1.value());
    const real_t o3v_l2  = (m_parent->m_vol3lfo2.value());
    const bool   o3v_mod = o3v_e1 != 0. || o3v_e2 != 0. || o3v_l1 != 0.
                         || o3v_l2 != 0.;

    // get sub
    const real_t o3sub   = (m_parent->m_osc3Sub.value() + 100.) / 200.;
    const real_t o3s_e1  = (m_parent->m_sub3env1.value());
    const real_t o3s_e2  = (m_parent->m_sub3env2.value());
    const real_t o3s_l1  = (m_parent->m_sub3lfo1.value() * 0.5);
    const real_t o3s_l2  = (m_parent->m_sub3lfo2.value() * 0.5);
    const bool   o3s_mod = o3s_e1 != 0. || o3s_e2 != 0. || o3s_l1 != 0.
                         || o3s_l2 != 0.;
    ...
    real_t o1l_f;
    real_t o1r_f;
    real_t o1l_p = m_osc1l_phase + o1lpo;  // we add phase offset here so we
                                           // don't have to do it every frame
    real_t o1r_p
            = m_osc1r_phase + o1rpo;  // then substract it again after loop...
    real_t o1_pw;

    // osc2 vars
    real_t o2l_f;
    real_t o2r_f;
    real_t o2l_p = m_osc2l_phase + o2lpo;
    real_t o2r_p = m_osc2r_phase + o2rpo;

    // osc3 vars
    real_t o3l_f;
    real_t o3r_f;
    real_t o3l_p = m_osc3l_phase + o3lpo;
    real_t o3r_p = m_osc3r_phase + o3rpo;
    real_t sub;

    // modulators
    real_t lfo[2][m_parent->m_fpp];
    real_t env[2][m_parent->m_fpp];
    ...
    for(f_cnt_t f = 0; f < _frames; ++f)
    {
        /*	// debug code
                        if( f % 10 == 0 ) {
                                qDebug( "env1 %f -- env1 phase %f",
           m_env1_buf[f], m_env1_phase ); qDebug( "env1 pre %f att %f dec %f
           rel %f ", m_parent->m_env1_pre, m_parent->m_env1_att,
                                        m_parent->m_env1_dec,
           m_parent->m_env1_rel );
                        }*/
        ...
        if(o1f_mod)
        {
            modulatefreq(o1l_f, o1f) modulatefreq(o1r_f, o1f)
        }
        // calc and modulate pulse
        o1_pw = pw;
        if(o1pw_mod)
        {
            modulateabs(o1_pw, o1pw) o1_pw = qBound(PW_MIN, o1_pw, PW_MAX);
        }
        ...
        if(o1p_mod)
        {
            modulatephs(leftph, o1p) modulatephs(rightph, o1p)
        }

        // pulse wave osc
        sample_t O1L = (positivefraction(leftph) < o1_pw) ? 1. : -1.;
        sample_t O1R = (positivefraction(rightph) < o1_pw) ? 1. : -1.;
        ...
        if(o1v_mod)
        {
            modulatevol(O1L, o1v) modulatevol(O1R, o1v)
        }

        // update osc1 phase working variable
        o1l_p += 1. / (static_cast<real_t>(m_parent->m_samplerate) / o1l_f);
        o1r_p += 1. / (static_cast<real_t>(m_parent->m_samplerate) / o1r_f);
        ...
        if(o2f_mod)
        {
            modulatefreq(o2l_f, o2f) modulatefreq(o2r_f, o2f)
        }
        ...
        if(o2p_mod)
        {
            modulatephs(leftph, o2p) modulatephs(rightph, o2p)
        }
        ...
        if(o2v_mod)
        {
            modulatevol(O2L, o2v) modulatevol(O2R, o2v)
        }
        ...
        m_ph2l_last = leftph;
        m_ph2r_last = rightph;
        o2l_p += 1. / (static_cast<real_t>(m_parent->m_samplerate) / o2l_f);
        o2r_p += 1. / (static_cast<real_t>(m_parent->m_samplerate) / o2r_f);
        ...
        if(o3f_mod)
        {
            modulatefreq(o3l_f, o3f) modulatefreq(o3r_f, o3f)
        }
        // calc and modulate phase
        leftph  = o3l_p;
        rightph = o3r_p;
        if(o3p_mod)
        {
            modulatephs(leftph, o3p) modulatephs(rightph, o3p)
        }
        ...
        if(o3s_mod)
        {
            modulateabs(sub, o3s) sub = qBound(0., sub, 1.);
        }
        ...
        if(o3v_mod)
        {
            modulatevol(O3L, o3v) modulatevol(O3R, o3v)
        }
        // o2 modulation?
        if(omod == MOD_AM)
        {
            O3L = qBound<sample_t>(-MODCLIP, O3L * qMax(0., 1. + O2L),
                                   MODCLIP);
            O3R = qBound<sample_t>(-MODCLIP, O3R * qMax(0., 1. + O2R),
                                   MODCLIP);
        }
        ...
        m_ph3l_last = leftph;
        m_ph3r_last = rightph;
        len_l = 1. / (static_cast<real_t>(m_parent->m_samplerate) / o3l_f);
        len_r = 1. / (static_cast<real_t>(m_parent->m_samplerate) / o3r_f);
        // handle FM as PM
        if(omod == MOD_FM)
        {
        ...
        sample_t L = O1L + O3L + (omod == MOD_MIX ? O2L : 0.);
        sample_t R = O1R + O3R + (omod == MOD_MIX ? O2R : 0.);
...
inline void MonstroSynth::updateModulators(
        real_t* env1, real_t* env2, real_t* lfo1, real_t* lfo2, int frames)
{
    ...
    for(int i = 0; i < 2; ++i)
    {
        switch(m_lfovalue[i])
        {
            ...
            case WAVE_RAMP:
                for(f_cnt_t f = 0; f < frames; ++f)
                {
                    lfo[i][f] = Oscillator::sawSample(m_lfo_phase[i]) * -1.;
                    m_lfo_phase[i] += m_lfo_inc[i];
                }
                break;
            case WAVE_SQR:
            ...
            case WAVE_RANDOM:
                for(f_cnt_t f = 0; f < frames; ++f)
                {
                    if((tfp + f) % static_cast<int>(m_lfo_rate[i]) == 0)
                        m_lfo_last[i] = Oscillator::noiseSample(0.);
            ...
            case WAVE_RANDOM_SMOOTH:
                ...
                {
                    const f_cnt_t tm
                            = (tfp + f) % static_cast<int>(m_lfo_rate[i]);
                    if(tm == 0)
                    {
                        m_lfo_last[i] = m_lfo_next[i];
                        m_lfo_next[i] = Oscillator::noiseSample(0.);
                    }
                    lfo[i][f] = cosinusInterpolate(
                            m_lfo_last[i], m_lfo_next[i],
                            static_cast<real_t>(tm) / m_lfo_rate[i]);
                    m_lfo_phase[i] += m_lfo_inc[i];
                }
                break;
        }
        ...
        {
            if(tfp + f < m_lfoatt[i])
                lfo[i][f] *= (static_cast<sample_t>(tfp) / m_lfoatt[i]);
        }
        ...
        for(f_cnt_t f = 0; f < frames; ++f)
        {
            if(m_env_phase[i] < 4. && m_nph->isReleased()
               && f >= m_nph->framesBeforeRelease())
            {
                if(m_env_phase[i] < 1.)
                    m_env_phase[i] = 5.;
                else if(m_env_phase[i] < 2.)
                    m_env_phase[i] = 5. - fraction(m_env_phase[i]);
                else if(m_env_phase[i] < 3.)
                    m_env_phase[i] = 4.;
                else
                    m_env_phase[i] = 4. + fraction(m_env_phase[i]);
            }

            // process envelope
            if(m_env_phase[i] < 1.)  // pre-delay phase
            {
                env[i][f] = 0.;
                m_env_phase[i]
                        = qMin<real_t>(1., m_env_phase[i] + m_env_pre[i]);
            }
            else if(m_env_phase[i] < 2.)  // attack phase
            {
                env[i][f] = calcSlope(i, fraction(m_env_phase[i]));
                m_env_phase[i]
                        = qMin<real_t>(2., m_env_phase[i] + m_env_att[i]);
            }
            else if(m_env_phase[i] < 3.)  // hold phase
            {
                env[i][f] = 1.;
                m_env_phase[i]
                        = qMin<real_t>(3., m_env_phase[i] + m_env_hold[i]);
            }
            else if(m_env_phase[i] < 4.)  // decay phase
            {
                const real_t s
                        = calcSlope(i, 1. - fraction(m_env_phase[i]));
                if(s <= m_env_sus[i])
                {
                    env[i][f] = m_env_sus[i];
                }
                ...
                {
                    env[i][f]      = s;
                    m_env_phase[i] = qMin<real_t>(
                            4. - m_env_sus[i], m_env_phase[i] + m_env_dec[i]);
                    if(m_env_phase[i] == 4.)
                        m_env_phase[i]
                                = 5.;  // jump over release if sustain is
                                         // zero - fix for clicking
                }
            }
            else if(m_env_phase[i] < 5.)  // release phase
            {
                env[i][f] = calcSlope(i, 1. - fraction(m_env_phase[i]));
...
MonstroInstrument::MonstroInstrument(InstrumentTrack* _instrument_track) :
      ...
      m_osc1Spo(
              0.0, -180.0, 180.0, 0.1, this, tr("Osc 1 Stereo phase offset")),
      ...
      m_osc2Spo(
              0.0, -180.0, 180.0, 0.1, this, tr("Osc 2 Stereo phase offset")),
      ...
      m_osc3Spo(
              0.0, -180.0, 180.0, 0.1, this, tr("Osc 3 Stereo phase offset")),
      m_osc3Sub(
              0.0, -100.0, 100.0, 0.1, this, tr("Osc 3 Sub-oscillator mix")),
      ...
      m_lfo1Att(0., 0., 2000., 1., 2000., this, tr("LFO 1 Attack")),
      m_lfo1Rate(1., 0.1, 10000.0, 0.1, 10000., this, tr("LFO 1 Rate")),
      m_lfo1Phs(0.0, -180.0, 180.0, 0.1, this, tr("LFO 1 Phase")),

      m_lfo2Wave(this, tr("LFO 2 Waveform")),
      m_lfo2Att(0., 0., 2000., 1., 2000., this, tr("LFO 2 Attack")),
      m_lfo2Rate(1., 0.1, 10000.0, 0.1, 10000., this, tr("LFO 2 Rate")),
      ...
      m_env1Pre(0.,
                ...
                this,
                tr("Env 1 Pre-delay")),
      m_env1Att(0., 0., 2000., 1., 2000., this, tr("Env 1 Attack")),
      m_env1Hold(0., 0., 4000., 1., 4000., this, tr("Env 1 Hold")),
      m_env1Dec(0., 0., 4000., 1., 4000., this, tr("Env 1 Decay")),
      m_env1Sus(1., 0., 1., 0.001, this, tr("Env 1 Sustain")),
      m_env1Rel(
              0., 0., 4000., 1., 4000., this, tr("Env 1 Release")),
      m_env1Slope(0., -1., 1., 0.001, this, tr("Env 1 Slope")),
      ...
      m_env2Pre(0.,
                ...
                this,
                tr("Env 2 Pre-delay")),
      m_env2Att(0., 0., 2000., 1., 2000., this, tr("Env 2 Attack")),
      m_env2Hold(0., 0., 4000., 1., 4000., this, tr("Env 2 Hold")),
      m_env2Dec(0., 0., 4000., 1., 4000., this, tr("Env 2 Decay")),
      m_env2Sus(1., 0., 1., 0.001, this, tr("Env 2 Sustain")),
      m_env2Rel(
              0., 0., 4000., 1., 4000., this, tr("Env 2 Release")),
      m_env2Slope(0., -1., 1., 0.001, this, tr("Env 2 Slope")),

      m_o23Mod(0, 0, NUM_MODS - 1, this, tr("Osc2-3 modulation")),

      m_selectedView(0, 0, 1, this, tr("Selected view")),

      m_vol1env1(0., -1., 1., 0.001, this, tr("Vol1-Env1")),
      m_vol1env2(0., -1., 1., 0.001, this, tr("Vol1-Env2")),
      m_vol1lfo1(0., -1., 1., 0.001, this, tr("Vol1-LFO1")),
      m_vol1lfo2(0., -1., 1., 0.001, this, tr("Vol1-LFO2")),

      m_vol2env1(0., -1., 1., 0.001, this, tr("Vol2-Env1")),
      m_vol2env2(0., -1., 1., 0.001, this, tr("Vol2-Env2")),
      m_vol2lfo1(0., -1., 1., 0.001, this, tr("Vol2-LFO1")),
      m_vol2lfo2(0., -1., 1., 0.001, this, tr("Vol2-LFO2")),

      m_vol3env1(0., -1., 1., 0.001, this, tr("Vol3-Env1")),
      m_vol3env2(0., -1., 1., 0.001, this, tr("Vol3-Env2")),
      m_vol3lfo1(0., -1., 1., 0.001, this, tr("Vol3-LFO1")),
      m_vol3lfo2(0., -1., 1., 0.001, this, tr("Vol3-LFO2")),

      m_phs1env1(0., -1., 1., 0.001, this, tr("Phs1-Env1")),
      m_phs1env2(0., -1., 1., 0.001, this, tr("Phs1-Env2")),
      m_phs1lfo1(0., -1., 1., 0.001, this, tr("Phs1-LFO1")),
      m_phs1lfo2(0., -1., 1., 0.001, this, tr("Phs1-LFO2")),

      m_phs2env1(0., -1., 1., 0.001, this, tr("Phs2-Env1")),
      m_phs2env2(0., -1., 1., 0.001, this, tr("Phs2-Env2")),
      m_phs2lfo1(0., -1., 1., 0.001, this, tr("Phs2-LFO1")),
      m_phs2lfo2(0., -1., 1., 0.001, this, tr("Phs2-LFO2")),

      m_phs3env1(0., -1., 1., 0.001, this, tr("Phs3-Env1")),
      m_phs3env2(0., -1., 1., 0.001, this, tr("Phs3-Env2")),
      m_phs3lfo1(0., -1., 1., 0.001, this, tr("Phs3-LFO1")),
      m_phs3lfo2(0., -1., 1., 0.001, this, tr("Phs3-LFO2")),

      m_pit1env1(0., -1., 1., 0.001, this, tr("Pit1-Env1")),
      m_pit1env2(0., -1., 1., 0.001, this, tr("Pit1-Env2")),
      m_pit1lfo1(0., -1., 1., 0.001, this, tr("Pit1-LFO1")),
      m_pit1lfo2(0., -1., 1., 0.001, this, tr("Pit1-LFO2")),

      m_pit2env1(0., -1., 1., 0.001, this, tr("Pit2-Env1")),
      m_pit2env2(0., -1., 1., 0.001, this, tr("Pit2-Env2")),
      m_pit2lfo1(0., -1., 1., 0.001, this, tr("Pit2-LFO1")),
      m_pit2lfo2(0., -1., 1., 0.001, this, tr("Pit2-LFO2")),

      m_pit3env1(0., -1., 1., 0.001, this, tr("Pit3-Env1")),
      m_pit3env2(0., -1., 1., 0.001, this, tr("Pit3-Env2")),
      m_pit3lfo1(0., -1., 1., 0.001, this, tr("Pit3-LFO1")),
      m_pit3lfo2(0., -1., 1., 0.001, this, tr("Pit3-LFO2")),

      m_pw1env1(0., -1., 1., 0.001, this, tr("PW1-Env1")),
      m_pw1env2(0., -1., 1., 0.001, this, tr("PW1-Env2")),
      m_pw1lfo1(0., -1., 1., 0.001, this, tr("PW1-LFO1")),
      m_pw1lfo2(0., -1., 1., 0.001, this, tr("PW1-LFO2")),

      m_sub3env1(0., -1., 1., 0.001, this, tr("Sub3-Env1")),
      m_sub3env2(0., -1., 1., 0.001, this, tr("Sub3-Env2")),
      m_sub3lfo1(0., -1., 1., 0.001, this, tr("Sub3-LFO1")),
      m_sub3lfo2(0., -1., 1., 0.001, this, tr("Sub3-LFO2"))

{

    // setup waveboxes
    setwavemodel(m_osc2Wave) setwavemodel(m_osc3Wave1)
            setwavemodel(m_osc3Wave2) setlfowavemodel(m_lfo1Wave)
            ...
            connect(&m_osc1Vol, SIGNAL(dataChanged()), this,
                    SLOT(updateVolume1()));
    ...
    connect(&m_env1Hold, SIGNAL(dataChanged()), this,
            SLOT(updateEnvelope1()));
    ...
    connect(&m_env2Hold, SIGNAL(dataChanged()), this,
            SLOT(updateEnvelope2()));
...
void MonstroInstrument::updateEnvelope1()
{
    if(m_env1Pre.value() == 0.)
        ...
        m_env1_pre = 1. / (m_env1Pre.value() / 1000.) / m_samplerate;
    if(m_env1Att.value() == 0.)
        m_env1_att = 1.;
    else
        m_env1_att = 1. / (m_env1Att.value() / 1000.) / m_samplerate;
    if(m_env1Hold.value() == 0.)
        m_env1_hold = 1.;
    else
        m_env1_hold = 1. / (m_env1Hold.value() / 1000.) / m_samplerate;
    if(m_env1Dec.value() == 0.)
        m_env1_dec = 1.;
    else
        m_env1_dec = 1. / (m_env1Dec.value() / 1000.) / m_samplerate;
    if(m_env1Rel.value() == 0.)
        m_env1_rel = 1.;
    else
        m_env1_rel = 1. / (m_env1Rel.value() / 1000.) / m_samplerate;

    m_env1_len = (m_env1Pre.value() + m_env1Att.value() + m_env1Hold.value()
                  + m_env1Dec.value())
                 * m_samplerate / 1000.;
...
void MonstroInstrument::updateEnvelope2()
{
    if(m_env2Pre.value() == 0.)
        ...
        m_env2_pre = 1. / (m_env2Pre.value() / 1000.) / m_samplerate;
    if(m_env2Att.value() == 0.)
        m_env2_att = 1.;
    else
        m_env2_att = 1. / (m_env2Att.value() / 1000.) / m_samplerate;
    if(m_env2Hold.value() == 0.)
        m_env2_hold = 1.;
    else
        m_env2_hold = 1. / (m_env2Hold.value() / 1000.) / m_samplerate;
    if(m_env2Dec.value() == 0.)
        m_env2_dec = 1.;
    else
        m_env2_dec = 1. / (m_env2Dec.value() / 1000.) / m_samplerate;
    if(m_env2Rel.value() == 0.)
        m_env2_rel = 1.;
    else
        m_env2_rel = 1. / (m_env2Rel.value() / 1000.) / m_samplerate;

    m_env2_len = (m_env2Pre.value() + m_env2Att.value() + m_env2Hold.value()
                  + m_env2Dec.value())
                 * m_samplerate / 1000.;
...
void MonstroInstrument::updateSamplerate()
{
    ...
    m_integrator   = 0.5 - (0.5 - INTEGRATOR) * 44100. / m_samplerate;
    m_fmCorrection = 44100. / m_samplerate * FM_AMOUNT;
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    m_opViewButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("opview_inactive"));
    ToolTip::add(m_opViewButton, tr("Operators view"));
    m_opViewButton->setWhatsThis(tr(
            "The Operators view contains all the operators. These include "
            "both audible "
            "operators (oscillators) and inaudible operators, or modulators: "
            "Low-frequency oscillators and Envelopes. \n\n"
            "Knobs and other widgets in the Operators view have their own "
            "what's this -texts, "
    ...
    m_matViewButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("matview_inactive"));
    ToolTip::add(m_matViewButton, tr("Matrix view"));
    m_matViewButton->setWhatsThis(
            tr("The Matrix view contains the modulation matrix. Here you can "
               "define "
               "the modulation relationships between the various operators: "
               "Each "
               "audible operator (oscillators 1-3) has 3-4 properties that "
               "can be "
               "modulated by any of the modulators. Using more modulations "
               "consumes "
               "more CPU power. \n\n"
               "The view is divided to modulation targets, grouped by the "
               "target oscillator. "
               "Available targets are volume, pitch, phase, pulse width and "
               "sub-osc ratio. "
               "Note: some targets are specific to one oscillator only. \n\n"
               "Each modulation target has 4 knobs, one for each modulator. "
               "By default "
               "the knobs are at 0, which means no modulation. Turning a "
               "knob to 1 causes "
               "that modulator to affect the modulation target as much as "
               "possible. Turning "
...
QWidget* MonstroView::setupOperatorsView(QWidget* _parent)
{
    ...
    makeknob(m_osc1VolKnob, KNOBCOL1, O1ROW, tr("Volume"), "%", "osc1Knob")
            makeknob(m_osc1PanKnob, KNOBCOL2, O1ROW, tr("Panning"), "",
                     "osc1Knob") makeknob(m_osc1CrsKnob, KNOBCOL3, O1ROW,
                                          tr("Coarse detune"),
                                          tr(" semitones"), "osc1Knob")
                    makeknob(m_osc1FtlKnob, KNOBCOL4, O1ROW,
                             tr("Finetune left"), tr(" cents"),
                             "osc1Knob") makeknob(m_osc1FtrKnob, KNOBCOL5,
                                                  O1ROW, tr("Finetune right"),
                                                  tr(" cents"), "osc1Knob")
                            makeknob(m_osc1SpoKnob, KNOBCOL6, O1ROW,
                                     tr("Stereo phase offset"), tr(" deg"),
                                     "osc1Knob") makeknob(m_osc1PwKnob,
                                                          KNOBCOL7, O1ROW,
                                                          tr("Pulse width"),
                                                          "%", "osc1Knob")
    ...
    maketinyled(m_osc1SSRButton, 230, 34, tr("Send sync on pulse rise"))
            maketinyled(m_osc1SSFButton, 230, 44,
                        tr("Send sync on pulse fall"))

                    makeknob(m_osc2VolKnob, KNOBCOL1, O2ROW, tr("Volume"),
                             "%", "osc2Knob") makeknob(m_osc2PanKnob,
                                                       KNOBCOL2, O2ROW,
                                                       tr("Panning"), "",
                                                       "osc2Knob")
                            makeknob(m_osc2CrsKnob, KNOBCOL3, O2ROW,
                                     tr("Coarse detune"), tr(" semitones"),
                                     "osc2Knob") makeknob(m_osc2FtlKnob,
                                                          KNOBCOL4, O2ROW,
                                                          tr("Finetune left"),
                                                          tr(" cents"),
                                                          "osc2Knob")
                                    makeknob(m_osc2FtrKnob, KNOBCOL5, O2ROW,
                                             tr("Finetune right"),
                                             tr(" cents"), "osc2Knob")
                                            makeknob(
                                                    m_osc2SpoKnob, KNOBCOL6,
                                                    O2ROW,
                                                    tr("Stereo phase offset"),
                                                    tr(" deg"), "osc2Knob")
    ...
    maketinyled(m_osc2SyncHButton, 212, O2ROW - 3,
                tr("Hard sync oscillator 2"))
            maketinyled(m_osc2SyncRButton, 191, O2ROW - 3,
                        tr("Reverse sync oscillator 2"))

                    makeknob(m_osc3VolKnob, KNOBCOL1, O3ROW, tr("Volume"),
                             "%", "osc3Knob") makeknob(m_osc3PanKnob,
                                                       KNOBCOL2, O3ROW,
                                                       tr("Panning"), "",
                                                       "osc3Knob")
                            makeknob(m_osc3CrsKnob, KNOBCOL3, O3ROW,
                                     tr("Coarse detune"), tr(" semitones"),
                                     "osc3Knob")
                                    makeknob(m_osc3SpoKnob, KNOBCOL4, O3ROW,
                                             tr("Stereo phase offset"),
                                             tr(" deg"), "osc3Knob")
                                            makeknob(m_osc3SubKnob, KNOBCOL5,
                                                     O3ROW, tr("Sub-osc mix"),
                                                     "", "osc3Knob")
    ...
    maketinyled(m_osc3SyncHButton, 212, O3ROW - 3,
                tr("Hard sync oscillator 3"))
            maketinyled(m_osc3SyncRButton, 191, O3ROW - 3,
                        tr("Reverse sync oscillator 3"))
    ...
    maketsknob(m_lfo1AttKnob, LFOCOL1, LFOROW, tr("Attack"), " ms", "lfoKnob")
            maketsknob(m_lfo1RateKnob, LFOCOL2, LFOROW, tr("Rate"), " ms",
                       "lfoKnob") makeknob(m_lfo1PhsKnob, LFOCOL3, LFOROW,
                                           tr("Phase"), tr(" deg"), "lfoKnob")
    ...
    maketsknob(
            m_lfo2AttKnob, LFOCOL4, LFOROW, tr("Attack"), " ms",
            "lfoKnob") maketsknob(m_lfo2RateKnob, LFOCOL5, LFOROW, tr("Rate"),
                                  " ms",
                                  "lfoKnob") makeknob(m_lfo2PhsKnob, LFOCOL6,
                                                      LFOROW, tr("Phase"),
                                                      tr(" deg"), "lfoKnob")

            maketsknob(
                    m_env1PreKnob, KNOBCOL1, E1ROW, tr("Pre-delay"), " ms",
                    "envKnob") maketsknob(m_env1AttKnob, KNOBCOL2, E1ROW,
                                          tr("Attack"), " ms",
                                          "envKnob") maketsknob(m_env1HoldKnob,
                                                                KNOBCOL3,
                                                                E1ROW,
                                                                tr("Hold"),
                                                                " ms",
                                                                "envKnob")
                    maketsknob(m_env1DecKnob, KNOBCOL4, E1ROW, tr("Decay"),
                               " ms", "envKnob") makeknob(m_env1SusKnob,
                                                          KNOBCOL5, E1ROW,
                                                          tr("Sustain"), "",
                                                          "envKnob")
                            maketsknob(m_env1RelKnob, KNOBCOL6, E1ROW,
                                       tr("Release"), " ms",
                                       "envKnob") makeknob(m_env1SlopeKnob,
                                                           KNOBCOL7, E1ROW,
                                                           tr("Slope"), "",
                                                           "envKnob")

                                    maketsknob(
                                            m_env2PreKnob, KNOBCOL1, E2ROW,
                                            tr("Pre-delay"), " ms",
                                            "envKnob") maketsknob(m_env2AttKnob,
                                                                  KNOBCOL2,
                                                                  E2ROW,
                                                                  tr("Attac"
                                                                     "k"),
                                                                  ...
                                                                  "envKnob")
                                            maketsknob(
                                                    m_env2HoldKnob, KNOBCOL3,
                                                    E2ROW, tr("Hold"), " ms",
                                                    "envKnob") maketsknob(m_env2DecKnob,
                                                                          KNOBCOL4,
                                                                          E2ROW,
                                                                          tr("Decay"),
                                                                          " m"
    ...
    m_osc1CrsKnob->setWhatsThis(
            tr("The CRS knob changes the tuning of oscillator 1 in semitone "
               "steps. "));
    m_osc2CrsKnob->setWhatsThis(
            tr("The CRS knob changes the tuning of oscillator 2 in semitone "
               "steps. "));
    m_osc3CrsKnob->setWhatsThis(
            tr("The CRS knob changes the tuning of oscillator 3 in semitone "
               "steps. "));
    m_osc1FtlKnob->setWhatsThis(
            tr("FTL and FTR change the finetuning of the oscillator for left "
               "and right "
               "channels respectively. These can add stereo-detuning to the "
               "oscillator "
               "which widens the stereo image and causes an illusion of "
               "space. "));
    m_osc1FtrKnob->setWhatsThis(
            tr("FTL and FTR change the finetuning of the oscillator for left "
               "and right "
               "channels respectively. These can add stereo-detuning to the "
               "oscillator "
    ...
    m_osc2FtrKnob->setWhatsThis(
            tr("FTL and FTR change the finetuning of the oscillator for left "
               ...
               "which widens the stereo image and causes an illusion of "
               "space. "));
    m_osc1SpoKnob->setWhatsThis(
            tr("The SPO knob modifies the difference in phase between left "
               "and right "
               "channels. Higher difference creates a wider stereo image. "));
    m_osc2SpoKnob->setWhatsThis(
            tr("The SPO knob modifies the difference in phase between left "
               "and right "
               "channels. Higher difference creates a wider stereo image. "));
    m_osc3SpoKnob->setWhatsThis(
            tr("The SPO knob modifies the difference in phase between left "
               "and right "
               "channels. Higher difference creates a wider stereo image. "));
    m_osc1PwKnob->setWhatsThis(tr(
            "The PW knob controls the pulse width, also known as duty cycle, "
            "of oscillator 1. Oscillator 1 is a digital pulse wave "
            "oscillator, "
            "it doesn't produce bandlimited output, which means that you can "
            "use it as an audible oscillator but it will cause aliasing. You "
            "can "
            "also use it as an inaudible source of a sync signal, which can "
            "be "
            "used to synchronize oscillators 2 and 3. "));
    m_osc1SSRButton->setWhatsThis(
            tr("Send Sync on Rise: When enabled, the Sync signal is sent "
               "every time "
               "the state of oscillator 1 changes from low to high, ie. when "
               "the amplitude "
               "changes from -1 to 1. "
               "Oscillator 1's pitch, phase and pulse width may affect the "
               "timing of syncs, "
               "but its volume has no effect on them. Sync signals are sent "
               "independently "
               "for both left and right channels. "));
    m_osc1SSFButton->setWhatsThis(
            tr("Send Sync on Fall: When enabled, the Sync signal is sent "
               "every time "
               "the state of oscillator 1 changes from high to low, ie. when "
               "the amplitude "
               "changes from 1 to -1. "
               "Oscillator 1's pitch, phase and pulse width may affect the "
               "timing of syncs, "
               ...
               "independently "
               "for both left and right channels. "));
    m_osc2SyncHButton->setWhatsThis(
            tr("Hard sync: Every time the oscillator receives a sync signal "
               "from oscillator 1, "
               "its phase is reset to 0 + whatever its phase offset is. "));
    m_osc3SyncHButton->setWhatsThis(
            tr("Hard sync: Every time the oscillator receives a sync signal "
               "from oscillator 1, "
               "its phase is reset to 0 + whatever its phase offset is. "));
    m_osc2SyncRButton->setWhatsThis(
            tr("Reverse sync: Every time the oscillator receives a sync "
               "signal from oscillator 1, "
               "the amplitude of the oscillator gets inverted. "));
    m_osc3SyncRButton->setWhatsThis(
            tr("Reverse sync: Every time the oscillator receives a sync "
               "signal from oscillator 1, "
               "the amplitude of the oscillator gets inverted. "));
    m_osc2WaveBox->setWhatsThis(tr("Choose waveform for oscillator 2. "));
    m_osc3Wave1Box->setWhatsThis(
            tr("Choose waveform for oscillator 3's first sub-osc. "
               "Oscillator 3 can smoothly interpolate between two different "
               "waveforms. "));
    m_osc3Wave2Box->setWhatsThis(
            tr("Choose waveform for oscillator 3's second sub-osc. "
               "Oscillator 3 can smoothly interpolate between two different "
               "waveforms. "));
    m_osc3SubKnob->setWhatsThis(
            tr("The SUB knob changes the mixing ratio of the two sub-oscs of "
               "oscillator 3. "
               "Each sub-osc can be set to produce a different waveform, and "
               "oscillator 3 "
               "can smoothly interpolate between them. All incoming "
               "modulations to oscillator 3 are applied "
               "to both sub-oscs/waveforms in the exact same way. "));
    m_mixButton->setWhatsThis(
            tr("In addition to dedicated modulators, Monstro allows "
               "oscillator 3 to be modulated by "
               "the output of oscillator 2. \n\n"
               "Mix mode means no modulation: the outputs of the oscillators "
               "are simply mixed together. "));
    m_amButton->setWhatsThis(
            tr("In addition to dedicated modulators, Monstro allows "
               "oscillator 3 to be modulated by "
               "the output of oscillator 2. \n\n"
               "AM means amplitude modulation: Oscillator 3's amplitude "
               "(volume) is modulated by oscillator 2. "));
    m_fmButton->setWhatsThis(
            tr("In addition to dedicated modulators, Monstro allows "
               "oscillator 3 to be modulated by "
               "the output of oscillator 2. \n\n"
               "FM means frequency modulation: Oscillator 3's frequency "
               "(pitch) is modulated by oscillator 2. "
               "The frequency modulation is implemented as phase modulation, "
               "which gives a more stable overall pitch "
               "than \"pure\" frequency modulation. "));
    m_pmButton->setWhatsThis(
            tr("In addition to dedicated modulators, Monstro allows "
               "oscillator 3 to be modulated by "
               "the output of oscillator 2. \n\n"
               "PM means phase modulation: Oscillator 3's phase is modulated "
               "by oscillator 2. "
               "It differs from frequency modulation in that the phase "
               "changes are not cumulative. "));
    m_lfo1WaveBox->setWhatsThis(
            tr("Select the waveform for LFO 1. \n"
               "\"Random\" and \"Random smooth\" are special waveforms: "
               "they produce random output, where the rate of the LFO "
               "controls how often "
               "the state of the LFO changes. The smooth version "
               "interpolates between these "
               "states with cosine interpolation. These random modes can be "
               "used to give "
               "\"life\" to your presets - add some of that analog "
               "unpredictability... "));
    m_lfo2WaveBox->setWhatsThis(
            tr("Select the waveform for LFO 2. \n"
               "\"Random\" and \"Random smooth\" are special waveforms: "
               "they produce random output, where the rate of the LFO "
               "controls how often "
               "the state of the LFO changes. The smooth version "
               ...
               "\"life\" to your presets - add some of that analog "
               "unpredictability... "));
    m_lfo1AttKnob->setWhatsThis(
            tr("Attack causes the LFO to come on gradually from the start of "
               "the note. "));
    m_lfo2AttKnob->setWhatsThis(
            tr("Attack causes the LFO to come on gradually from the start of "
               "the note. "));
    m_lfo1RateKnob->setWhatsThis(
            tr("Rate sets the speed of the LFO, measured in milliseconds per "
               "cycle. Can be synced to tempo. "));
    m_lfo2RateKnob->setWhatsThis(
            tr("Rate sets the speed of the LFO, measured in milliseconds per "
               "cycle. Can be synced to tempo. "));
    m_lfo1PhsKnob->setWhatsThis(
            tr("PHS controls the phase offset of the LFO. "));
    m_lfo2PhsKnob->setWhatsThis(
            tr("PHS controls the phase offset of the LFO. "));

    m_env1PreKnob->setWhatsThis(
            tr("PRE, or pre-delay, delays the start of the envelope from the "
               "start of the note. 0 means no delay. "));
    m_env2PreKnob->setWhatsThis(
            tr("PRE, or pre-delay, delays the start of the envelope from the "
               "start of the note. 0 means no delay. "));
    m_env1AttKnob->setWhatsThis(
            tr("ATT, or attack, controls how fast the envelope ramps up at "
               "start, measured in milliseconds. "
               "A value of 0 means instant. "));
    m_env2AttKnob->setWhatsThis(
            tr("ATT, or attack, controls how fast the envelope ramps up at "
               "start, measured in milliseconds. "
               "A value of 0 means instant. "));
    m_env1HoldKnob->setWhatsThis(
            tr("HOLD controls how long the envelope stays at peak after the "
               "attack phase. "));
    m_env2HoldKnob->setWhatsThis(
            tr("HOLD controls how long the envelope stays at peak after the "
               "attack phase. "));
    m_env1DecKnob->setWhatsThis(
            tr("DEC, or decay, controls how fast the envelope falls off from "
               "its peak, measured in milliseconds "
               "it would take to go from peak to zero. The actual decay may "
               "be shorter if sustain is used. "));
    m_env2DecKnob->setWhatsThis(
            tr("DEC, or decay, controls how fast the envelope falls off from "
               "its peak, measured in milliseconds "
               "it would take to go from peak to zero. The actual decay may "
               "be shorter if sustain is used. "));
    m_env1SusKnob->setWhatsThis(
            tr("SUS, or sustain, controls the sustain level of the envelope. "
               "The decay phase will not go below this level "
               "as long as the note is held. "));
    m_env2SusKnob->setWhatsThis(
            tr("SUS, or sustain, controls the sustain level of the envelope. "
               "The decay phase will not go below this level "
               "as long as the note is held. "));
    m_env1RelKnob->setWhatsThis(
            tr("REL, or release, controls how long the release is for the "
               "note, measured in how long it would take to "
               "fall from peak to zero. Actual release may be shorter, "
               "depending on at what phase the note is released. "));
    m_env2RelKnob->setWhatsThis(
            tr("REL, or release, controls how long the release is for the "
               "note, measured in how long it would take to "
               "fall from peak to zero. Actual release may be shorter, "
               "depending on at what phase the note is released. "));
    m_env1SlopeKnob->setWhatsThis(
            tr("The slope knob controls the curve or shape of the envelope. "
               "A value of 0 creates straight rises and falls. "
               "Negative values create curves that start slowly, peak "
               "quickly and fall of slowly again. "
               "Positive values create curves that start and end quickly, "
               "and stay longer near the peaks. "));
    m_env2SlopeKnob->setWhatsThis(
            tr("The slope knob controls the curve or shape of the envelope. "
               "A value of 0 creates straight rises and falls. "
               "Negative values create curves that start slowly, peak "
               "quickly and fall of slowly again. "
...
QWidget* MonstroView::setupMatrixView(QWidget* _parent)
{
    ...
    makeknob(
            ...
            makeknob(
                    ...
                    makeknob(m_vol3env1Knob, MATCOL1, MATROW5,
                             tr("Modulation amount"), "",
                             "matrixKnob") makeknob(m_vol3env2Knob, MATCOL2,
                                                    MATROW5,
                                                    tr("Modulation amount"),
                                                    "", "matrixKnob")
                            makeknob(m_vol3lfo1Knob, MATCOL3, MATROW5,
                                     tr("Modulation amount"), "",
                                     "matrixKnob") makeknob(m_vol3lfo2Knob,
                                                            MATCOL4, MATROW5,
                                                            tr("Modulation "
                                                               "amount"),
                                                            "", "matrixKnob")
                                    ...
                                    makeknob(
                                            ...
                                            "matrixKnob") makeknob(m_phs1env2Knob,
                                                                   ...
                                                                   "b")
                                            makeknob(
                                                    m_phs1lfo1Knob, MATCOL3,
                                                    MATROW2,
                                                    tr("Modulation amount"),
                                                    "",
                                                    "matrixKnob") makeknob(m_phs1lfo2Knob,
                                                                           MATCOL4,
                                                                           MATROW2,
                                                                           tr("Modulation amount"),
                                                                           "",
                                                                                    ...
                                                                                    makeknob(
                                                                                            ...
                                                                                            makeknob(
                                                                                                    ...
                                                                                                    makeknob(m_phs3lfo2Knob,
                                                                                                            ...
                                                                                                            makeknob(
                                                                                                                    m_pit1env1Knob, MATCOL5,
                                                                                                                    MATROW1,
                                                                                                                    tr("Modulation amount"),
                                                                                                                    "",
                                                                                                                    "matrixKnob") makeknob(m_pit1env2Knob,
                                                                                                                                           MATCOL6, MATROW1, tr("Modulation amount"),
                                                                                                                                           "",
                                                                                                                                           "matrixKnob") makeknob(m_pit1lfo1Knob,
                                                                                                                                                                  MATCOL7,
                                                                                                                                                                  MATROW1,
                                                                                                                                                                  tr("Modulation amount"),
                                                                                                                                                                  "",
                                                                                                                                                                  "matrixKnob")
                                                                                                                    makeknob(m_pit1lfo2Knob,
                                                                                                                             MATCOL8, MATROW1, tr("Modulation amount"),
                                                                                                                             "",
                                                                                                                             "matrixKnob")

                                                                                                                            makeknob(
                                                                                                                                    m_pit2env1Knob, MATCOL5, MATROW3, tr("Modulation amount"),
                                                                                                                                    "",
                                                                                                                                    "matrixKnob")
                                                                                                                                    makeknob(
                                                                                                                                            m_pit2env2Knob,
                                                                                                                                            ...
                                                                                                                                            makeknob(
                                                                                                                                                    ...
                                                                                                                                                    tr("Modulation amount"),
                                                                                                                                                    "",
                                                                                                                                                    "matrixKnob") makeknob(m_pit2lfo2Knob,
                                                                                                                                                                           MATCOL8,
                                                                                                                                                                           MATROW3, tr("Modulation amount"),
                                                                                                                                                                           "",
                                                                                                                                                                           "matrixKnob")
                                                                                                                                                    ...
                                                                                                                                                    makeknob(
                                                                                                                                                            ...
                                                                                                                                                            makeknob(
                                                                                                                                                                    ...
                                                                                                                                                                    "matrixKnob")
                                                                                                                                                                    makeknob(
                                                                                                                                                                            m_pit3lfo1Knob, MATCOL7, MATROW5,
                                                                                                                                                                            tr("Modulation amount"),
                                                                                                                                                                            "",
                                                                                                                                                                            "matrixKnob") makeknob(m_pit3lfo2Knob,
                                                                                                                                                                                                   MATCOL8,
                                                                                                                                                                                                   MATROW5,
                                                                                                                                                                                                   tr("Modulation amount"),
                                                                                                                                                                                                   "",
                                                                                                                                                                            ...
                                                                                                                                                                            makeknob(
                                                                                                                                                                                    m_pw1env1Knob, MATCOL5, MATROW2,
                                                                                                                                                                                    tr("Modulation amount"),
                                                                                                                                                                                    "",
                                                                                                                                                                                    "matrixKnob") makeknob(m_pw1env2Knob, MATCOL6,
                                                                                                                                                                                                           MATROW2,
                                                                                                                                                                                                           tr("Modulation amount"),
                                                                                                                                                                                                           "",
                                                                                                                                                                                                           "matrixKnob") makeknob(m_pw1lfo1Knob,
                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                  tr("Modulation amount"),
                                                                                                                                                                                                                                  "",
                                                                                                                                                                                                                                  "matrixKnob") makeknob(m_pw1lfo2Knob,
                                                                                                                                                                                                                                                         MATCOL8,
                                                                                                                                                                                                                                                         MATROW2,
                                                                                                                                                                                                                                                         tr("Modulation amount"),
                                                                                                                                                                                                                                                         "",
                                                                                                                                                                                    ...
                                                                                                                                                                                    makeknob(
                                                                                                                                                                                            ...
                                                                                                                                                                                            "matrixKnob") makeknob(m_sub3env2Knob,
                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                   tr(
                                                                                                                                                                                                                           "Modulation amount"),
                                                                                                                                                                                                                   "",
                                                                                                                                                                                                                   "matrixKnob") makeknob(m_sub3lfo1Knob,
                                                                                                                                                                                                                                          MATCOL7,
                                                                                                                                                                                                                                          MATROW6,
                                                                                                                                                                                                                                          tr("Modulation amount"),
                                                                                                                                                                                                                                          "",
                                                                                                                                                                                                                                          "matrixKnob") makeknob(m_sub3lfo2Knob,
                                                                                                                                                                                                                                                                 MATCOL8,
                                                                                                                                                                                                                                                                 MATROW6,
                                                                                                                                                                                                                                                                 tr("Modulation amount"),
                                                                                                                                                                                                                                                                 "",
