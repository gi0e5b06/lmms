...
const sample_t MODCLIP = 2.;

const frequency_t MIN_FREQ = 18.;
const frequency_t MAX_FREQ = 48000.;

const real_t INTEGRATOR = 3. / 7.;

const real_t FM_AMOUNT = 0.25;
...
class MonstroSynth
{
  ...
  private:
    ...
    /*	inline sample_t interpolate( sample_t s1, sample_t s2, real_t x )
            {
                    return s1 + ( s2 - s1 ) * x;
    ...
    // checks for lower bound for phase, upper bound is already checked by
    // oscillator-functions in both oscillator.h and bandlimitedwave.h so we
    // save some cpu by only checking lower bound
    inline real_t lowBoundCheck(real_t ph)
    {
        return ph < 0. ? ph - (static_cast<int>(ph) - 1.) : ph;
    }

    inline sample_t oscillate(int _wave, const real_t _ph, real_t _wavelen)
    {
        switch(_wave)
        {
            case WAVE_SINE:
            ...
            case WAVE_SAW:
                ...
                return BandLimitedWave::oscillate(_ph, _wavelen,
    ...
    real_t m_osc1l_phase;
    real_t m_osc1r_phase;
    real_t m_osc2l_phase;
    real_t m_osc2r_phase;
    real_t m_osc3l_phase;
    real_t m_osc3r_phase;

    sample_t m_env_phase[2];
    real_t   m_lfo_phase[2];
...
#define setwavemodel(name)                                                   \
    name.addItem(tr("Sine wave"),                                            \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("sin"))); \
    name.addItem(tr("Bandlimited Triangle wave"),                            \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("tri"))); \
    name.addItem(tr("Bandlimited Saw wave"),                                 \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("saw"))); \
    name.addItem(                                                            \
            tr("Bandlimited Ramp wave"),                                     \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("ramp")));     \
    name.addItem(tr("Bandlimited Square wave"),                              \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("sqr"))); \
    name.addItem(                                                            \
            tr("Bandlimited Moog saw wave"),                                 \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("moog")));     \
    name.addItem(                                                            \
            tr("Soft square wave"),                                          \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("sqrsoft")));  \
    name.addItem(                                                            \
            tr("Absolute sine wave"),                                        \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("sinabs")));   \
    name.addItem(tr("Exponential wave"),                                     \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("exp"))); \
    name.addItem(                                                            \
            tr("White noise"),                                               \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("noise")));    \
    name.addItem(tr("Digital Triangle wave"),                                \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("tri"))); \
    name.addItem(tr("Digital Saw wave"),                                     \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("saw"))); \
    name.addItem(                                                            \
            tr("Digital Ramp wave"),                                         \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("ramp")));     \
    name.addItem(tr("Digital Square wave"),                                  \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("sqr"))); \
    name.addItem(                                                            \
            tr("Digital Moog saw wave"),                                     \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("moog")));

#define setlfowavemodel(name)                                                \
    name.addItem(tr("Sine wave"),                                            \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("sin"))); \
    name.addItem(tr("Triangle wave"),                                        \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("tri"))); \
    name.addItem(tr("Saw wave"),                                             \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("saw"))); \
    name.addItem(tr("Ramp wave"), static_cast<PixmapLoader*>(                \
                                          new PluginPixmapLoader("ramp")));  \
    name.addItem(tr("Square wave"),                                          \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("sqr"))); \
    name.addItem(                                                            \
            tr("Moog saw wave"),                                             \
            static_cast<PixmapLoader*>(new PluginPixmapLoader("moog")));     \
    ...
    name.addItem(tr("Exponential wave"),                                     \
                 static_cast<PixmapLoader*>(new PluginPixmapLoader("exp"))); \
    name.addItem(tr("Random"), static_cast<PixmapLoader*>(                   \
                                       new PluginPixmapLoader("rand")));     \
    name.addItem(                                                            \
            tr("Random smooth"),                                             \
  ...
  protected:
    ...
    real_t m_integrator;
    real_t m_fmCorrection;
    int    m_counterMax;

  private:
    inline real_t leftCh(real_t _vol, real_t _pan)
    {
        return (_pan <= 0 ? 1.0 : 1.0 - (_pan / 100.0)) * _vol / 100.0;
    }

    inline real_t rightCh(real_t _vol, real_t _pan)
    {
        return (_pan >= 0 ? 1.0 : 1.0 + (_pan / 100.0)) * _vol / 100.0;
    }
