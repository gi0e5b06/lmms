	...
	m_uri( LV2EffectGDXSubPluginFeatures::subPluginKeyToLV2Key( _key ) )
{
	...
	{
		Engine::getSong()->collectError(tr( "Unknown LV2 effect %1 requested." )
                                                .arg( m_uri ) );
		setOkay( false );
		return;
	}
...
bool LV2EffectGDX::processAudioBuffer( sampleFrame * _buf, 
				       const fpp_t _frames )
{
	m_pluginMutex.lock();
	...
	for( ch_cnt_t proc = 0; proc < processorCount(); ++proc )
	{
		//(m_descriptor->run)( m_handles[proc], frames );
                //tmp
                manager->run(m_uri, m_handles[proc], frames);
	}
...
uint32_t LV2EffectGDX::inputChannels()
{
	...
	uint32_t r=0;
        for(int i=0;i<m_portCount;i++)
                if(manager->isPortInput(m_uri,i)&&
                   manager->isPortAudio(m_uri,i))
                        r++;
        return r;
}
...
uint32_t LV2EffectGDX::outputChannels()
{
	...
	uint32_t r=0;
        for(int i=0;i<m_portCount;i++)
                if(manager->isPortOutput(m_uri,i)&&
                   manager->isPortAudio(m_uri,i))
                        r++;
        return r;
}
...
void LV2EffectGDX::pluginInstantiation()
{
	...
	setProcessorCount( lmms_chnls / effect_channels );
        ...
        qInfo("LV2EffectGDX::pluginInstantiation effect_channels=%d processors=%d",
	...
	for( ch_cnt_t proc = 0; proc < processorCount(); proc++ )
	{
		...
		for( int port = 0; port < m_portCount; port++ )
		{
			...
			if( manager->isPortAudio( m_uri, port ) )
			{
				...
				{
					p->rate = CHANNEL_IN;
					p->buffer = MM_ALLOC( LV2_Data, Engine::mixer()->framesPerPeriod() );
			...
			if( manager->isPortControl( m_uri, port ) )
			{
	...
	if( m_descriptor == NULL )
	{
		QMessageBox::warning( 0, "Effect", 
                                      "Can't get LV2 descriptor function: "+m_uri,//.second
	...
	{
		QMessageBox::warning( 0, "Effect",
                                      "Plugin has no processor: " + m_uri,//.second,
			QMessageBox::Ok, QMessageBox::NoButton );
		setDontRun( true );
	}
        */
	...
	for( ch_cnt_t proc = 0; proc < processorCount(); proc++ )
	{
		LV2_Instance effect=manager->instantiate
                        (m_uri,
                         Engine::mixer()->processingSampleRate());//m_maxSampleRate);
		if( effect == NULL )
		{
			QMessageBox::warning
                                (0, "LV2 Effect: "+name, "Can't create: "+m_uri,
                                 QMessageBox::Ok, QMessageBox::NoButton );
			setOkay( false );
			return;
		}
