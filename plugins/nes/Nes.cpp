...
extern "C"
{
    ...
    Plugin::Descriptor PLUGIN_EXPORT nes_plugin_descriptor
            = {STRINGIFY(PLUGIN_NAME),
               "Nescaline",
               QT_TRANSLATE_NOOP("pluginBrowser", "A NES-like synthesizer"),
...
NesObject::NesObject(NesInstrument*      nes,
                     ...
                     NotePlayHandle*     nph) :
      m_parent(nes),
      m_samplerate(samplerate), m_nph(nph)
{
    ...
    m_lastNoteFreq  = 0;
    m_lastNoiseFreq = -1.0f;  // value that is always different than noisefreq
                              // so it gets updated at start
...
void NesObject::renderOutput(sampleFrame* buf, fpp_t frames)
{
    ...
    int   ch1EnvLen
            = wavelength(floorf(240.0 / (m_parent->m_ch1EnvLen.value() + 1)));
    bool ch1EnvLoop = m_parent->m_ch1EnvLooped.value();

    float ch2DutyCycle = DUTY_CYCLE[m_parent->m_ch2DutyCycle.value()];
    int   ch2EnvLen
            = wavelength(floorf(240.0 / (m_parent->m_ch2EnvLen.value() + 1)));
    bool ch2EnvLoop = m_parent->m_ch2EnvLooped.value();

    int ch4EnvLen
            = wavelength(floorf(240.0 / (m_parent->m_ch4EnvLen.value() + 1)));
    ...
    int ch1SweepRate = wavelength(
            floorf(120.0 / (m_parent->m_ch1SweepRate.value() + 1)));
    int ch2SweepRate = wavelength(
            floorf(120.0 / (m_parent->m_ch2SweepRate.value() + 1)));
    int ch4SweepRate = wavelength(
            floorf(60.0f / (8.0f - qAbs(m_parent->m_ch4Sweep.value()))));
    ...
    for(f_cnt_t f = 0; f < frames; f++)
    {
        ...
        {
            m_ch1SweepCounter = 0;
            if(m_parent->m_ch1SweepEnabled.value() && m_wlen1 <= m_maxWlen
               && m_wlen1 >= MIN_WLEN)
            {
        ...
        {
            m_ch2SweepCounter = 0;
            if(m_parent->m_ch2SweepEnabled.value() && m_wlen2 <= m_maxWlen
               && m_wlen2 >= MIN_WLEN)
            {
                // check if the sweep goes up or down
                if(ch2Sweep > 0)
        ...
        if(m_wlen3 <= m_maxWlen && ch3Enabled)
        {
            ...
            ch3 = m_wlen3 ? TRIANGLE_WAVETABLE[(m_ch3Counter * 32) / m_wlen3]
                          : 0;
            ch3 = (ch3 * ch3Level) / 15;
        }
        else
        ...
        if(m_ch4SweepCounter >= ch4SweepRate)
        {
            ...
            if(ch4Sweep != 0)
            {
                int freqN = nearestNoiseFreq(static_cast<float>(m_samplerate)
                                             / m_wlen4);
                freqN     = qBound(0, freqN + ch4Sweep, 15);
                m_wlen4   = wavelength(NOISE_FREQS[freqN]);

                if(m_wlen4 == 0
                   && ch4Sweep == 1)  // a workaround for sweep getting stuck
                                      // on 0 wavelength
                {
                    while(m_wlen4 == 0)
                    {
        ...
        // simple first order iir filter, to simulate the frequency response
        // falloff in nes analog audio output
        ...
        const float mixdown = (pin1 + pin2) * NES_MIXING_ALL
                              * m_parent->m_masterVol.value();
...
void NesObject::updateVibrato(float* freq)
{
    ...
    float vibratoRatio
            = 1.0f
              + (static_cast<float>(TRIANGLE_WAVETABLE[m_vibratoPhase])
                 * 0.02f * vibratoAmt);
    *freq *= vibratoRatio;
}
...
void NesObject::updatePitch()
{
    ...
    {
        float f = freq * 2.0f;
        if(m_parent->m_ch4NoiseQuantize
                   .value())  // note freq can be quantized to the preset
                              // freqs
        {
...
NesInstrument::NesInstrument(InstrumentTrack* instrumentTrack) :
      ...
      m_ch1EnvEnabled(false, this), m_ch1EnvLooped(false, this),
      ...
      m_ch2EnvEnabled(false, this), m_ch2EnvLooped(false, this),
      ...
      m_ch4EnvEnabled(false, this), m_ch4EnvLooped(false, this),
      m_ch4EnvLen(0.f, 0.f, 15.f, 1.f, this, tr("Channel 4 Envelope length")),

      m_ch4NoiseMode(false, this), m_ch4NoiseFreqMode(false, this),
      m_ch4NoiseFreq(
              0.f, 0.f, 15.f, 1.f, this, tr("Channel 4 Noise frequency")),
      ...
      m_ch4Sweep(0.f,
                 ...
                 1.f,
                 this,
                 tr("Channel 4 Noise frequency sweep")),
