...
class lb302Filter
{
  ...
  protected:
    ...
    // Filter Decay
    real_t vcf_c0;  // c0=e1 on retrigger; c0*=ed every sample; cutoff=e0+c0
    real_t vcf_e0,  // e0 and e1 for interpolation
            vcf_e1;
    real_t vcf_rescoeff;  // Resonance coefficient [0.30,9.54]
};
...
class lb302FilterIIR2 : public lb302Filter
{
  public:
    ...
    virtual void     recalc();
    virtual void     envRecalc();
    virtual sample_t process(const sample_t& samp);

  protected:
    real_t vcf_d1,   //   d1 and d2 are added back into the sample with
            vcf_d2;  //   vcf_a and b as coefficients. IIR2 resonance
                     //   loop.

    // IIR2 Coefficients for mixing dry and delay.
    real_t vcf_a,   //   Mixing coefficients for the final sound.
            vcf_b,  //
            vcf_c;
...
class lb302Filter3Pole : public lb302Filter
{
  ...
  protected:
    real_t kfcn, kp, kp1, kp1h, kres;
    real_t ay1, ay2, aout, lastin, value;
};
...
class lb302Synth : public Instrument
{
  ...
  private:
    // Oscillator
    real_t vco_inc,  // Sample increment for the frequency. Creates Sawtooth.
            vco_k,   // Raw oscillator sample [-0.5,0.5]
            vco_c;   // Raw oscillator sample [-0.5,0.5]

    real_t vco_slide,       //* Current value of slide exponential curve.
                            //Nonzero=sliding
            vco_slideinc,   //* Slide base to use in next node. Nonzero=slide
                            //next note
