...
#define LB_DIST_RATIO 4.
#define LB_24_VOL_ADJUST 3.
...
//#define engine::mixer()->processingSampleRate() 44100.
...
lb302Filter::lb302Filter(lb302FilterKnobState* p_fs) :
      fs(p_fs), vcf_c0(0), vcf_e0(0), vcf_e1(0){};

void lb302Filter::recalc()
{
    vcf_e1 = exp(6.109 + 1.5876 * (fs->envmod) + 2.1553 * (fs->cutoff)
                 - 1.2 * (1. - (fs->reso)));
    vcf_e0 = exp(5.613 - 0.8 * (fs->envmod) + 2.1553 * (fs->cutoff)
                 - 0.7696 * (1. - (fs->reso)));
...
{
    vcf_c0 *= fs->envdecay;  // Filter Decay. vcf_decay is adjusted for Hz and
                             // ENVINC
...
lb302FilterIIR2::lb302FilterIIR2(lb302FilterKnobState* p_fs) :
      lb302Filter(p_fs), vcf_d1(0), vcf_d2(0), vcf_a(0), vcf_b(0), vcf_c(1)
{

    m_dist = new DspEffectLibrary::Distortion(1., 1.);
};
...
{
    real_t ret = vcf_a * vcf_d1 + vcf_b * vcf_d2 + vcf_c * samp;
...
lb302Filter3Pole::lb302Filter3Pole(lb302FilterKnobState* p_fs) :
      lb302Filter(p_fs), ay1(0), ay2(0), aout(0), lastin(0){};
...
void lb302Filter3Pole::envRecalc()
{
    ...
    // kfcn = fs->cutoff;
    kfcn = 2. * kfco / Engine::mixer()->processingSampleRate();
#else
    kfcn = w;
#endif
    ...
    kres = (((k)))
           * (((-2.7079 * kp1 + 10.963) * kp1 - 14.934) * kp1 + 8.4974);
#else
    kres = (((fs->reso)))
           * (((-2.7079 * kp1 + 10.963) * kp1 - 14.934) * kp1 + 8.4974);
#endif
    value = 1.0
            + ((fs->dist)
               * (1.5 + 2. * kres * (1. - kfcn)));  // ENVMOD was DIST
}
...
sample_t lb302Filter3Pole::process(const sample_t& samp)
{
    ...
    return tanh(aout * value) * LB_24_VOL_ADJUST / (1. + fs->dist);
}
...
lb302Synth::lb302Synth(InstrumentTrack* _instrumentTrack) :
      ...
      vcf_cut_knob(0.75, 0., 1.5, 0.005, this, tr("VCF Cutoff Frequency")),
      vcf_res_knob(0.75, 0., 1.25, 0.005, this, tr("VCF Resonance")),
      vcf_mod_knob(0.1, 0., 1., 0.005, this, tr("VCF Envelope Mod")),
      vcf_dec_knob(0.1, 0., 1., 0.005, this, tr("VCF Envelope Decay")),
      dist_knob(0., 0., 1., 0.01, this, tr("Distortion")),
      wave_shape(8., 0., 11., this, tr("Waveform")),
      slide_dec_knob(0.6, 0., 1., 0.005, this, tr("Slide Decay")),
      ...
      vca_attack(1. - 0.96406088), vca_decay(0.99897516), vca_a0(0.5),
      vca_a(0.), vca_mode(never_played)
{

    connect(Engine::mixer(), SIGNAL(sampleRateChanged()), this,
            SLOT(filterChanged()));

    connect(&vcf_cut_knob, SIGNAL(dataChanged()), this,
            SLOT(filterChanged()));

    connect(&vcf_res_knob, SIGNAL(dataChanged()), this,
            SLOT(filterChanged()));

    connect(&vcf_mod_knob, SIGNAL(dataChanged()), this,
            SLOT(filterChanged()));

    connect(&vcf_dec_knob, SIGNAL(dataChanged()), this,
            SLOT(filterChanged()));
...
void lb302Synth::filterChanged()
{
    ...
    real_t d = 0.2 + (2.3 * vcf_dec_knob.value());
...
void lb302Synth::recalcFilter()
{
    ...
    /*kfcn = 2. * (((vcf_cutoff*3000))) /
    engine::mixer()->processingSampleRate(); kp   = ((-2.7528*kfcn
    + 3.0429)*kfcn + 1.718)*kfcn - 0.9984; kp1  = kp+1.; kp1h = 0.5*kp1; kres
    = (((vcf_reso))) * (((-2.7079*kp1 + 10.963)*kp1 - 14.934)*kp1 + 8.4974);
    value = 1.0+( (((0))) *(1.5 + 2.0*kres*(1.0-kfcn))); // ENVMOD was DIST*/

    vcf_envpos = ENVINC;  // Trigger filter update in process()
}

inline double GET_INC(frequency_t freq)
{
    return freq
           / Engine::mixer()->processingSampleRate();  // TODO: Use actual
                                                       // sampling rate.
}
...
int lb302Synth::process(sampleFrame* outbuf, const int size)
{
    const double sampleRatio
            = 44100. / Engine::mixer()->processingSampleRate();
    real_t   w;
    sample_t samp;
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        if(vcf_envpos >= ENVINC)
        {
            ...
            if(vco_slide)
            {
                ...
                vco_slide -= vco_slide
                             * (0.1 - slide_dec_knob.value() * 0.0999)
                             * sampleRatio;  // TODO: Adjust for ENVINC
            }
        }
        ...
        switch(vco_shape)
        {
            ...
            case ROUND_SQUARE:  // p0: width of round
                vco_k = (vco_c < 0) ? (sqrt(1 - (vco_c * vco_c * 4)) - 0.5)
                                    : -0.5;
                break;

            case MOOG:  // Maybe the fall should be exponential/sinsoidal
                        // instead of quadric.
                ...
                {
                    w     = 2. * (vco_k - 0.5) - 1.;
                    vco_k = 0.5 - sqrt(1. - (w * w));
                }
                vco_k *= 2.;  // MOOG wave gets filtered away
                break;

            case SINE:
                // [-0.5, 0.5]  : [-pi, pi]
                vco_k = 0.5 * Oscillator::sinSample(vco_c + 0.5);
                break;

            case EXPONENTIAL:
                vco_k = 0.5 * Oscillator::expSample(vco_c + 0.5);
                break;

            case WHITE_NOISE:
                vco_k = 0.5 * Oscillator::noiseSample(vco_c + 0.5);
                break;

            case BL_SAWTOOTH:
                vco_k = BandLimitedWave::oscillate(
                                vco_c + 0.5,
                                BandLimitedWave::pdToLen(vco_inc),
                                BandLimitedWave::BLSaw)
                        * 0.5;
                break;
            ...
            case BL_MOOG:
                vco_k = BandLimitedWave::oscillate(
                        vco_c + 0.5, BandLimitedWave::pdToLen(vco_inc),
                        BandLimitedWave::BLMoog);
                break;
        }
...
#endif
        /*
        real_t releaseFrames = desiredReleaseFrames();
        samp *= (releaseFrames - catch_decay)/releaseFrames;
        */
        // LB302 samp *= (real_t)(decay_frames -
        // catch_decay)/(real_t)decay_frames;
...
/*  Prepares the active LB302 note.  I separated this into a function because
 * it needs to be called onplayNote() when a new note is started.  It also
 * needs to be called from process() when a prior edge-to-edge note is done
 * releasing.
 */
...
void lb302Synth::initNote(lb302Note* n)
{
    ...
    {
        vco_slideinc = vco_inc;  // May need to equal vco_slidebase+vco_slide
                                 // if last note slid
    }
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    m_db24Toggle->setWhatsThis(tr("303-es-que, 24dB/octave, 3 pole filter"));
    ...
    roundSqrWaveBtn->setActiveGraphic(
            embed::getIconPixmap("round_square_wave_active"));
    roundSqrWaveBtn->setInactiveGraphic(
            embed::getIconPixmap("round_square_wave_inactive"));
...
extern "C"
{
    ...
    Plugin* PLUGIN_EXPORT lmms_plugin_main(Model*, void* _data)
    {
        ...
        return (new lb302Synth(static_cast<InstrumentTrack*>(_data)));
    }
}
