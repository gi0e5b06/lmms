 ...
 *
 * Copyright (c) 2007-2008 Javier Serrano Polo
 * <jasp00/at/users.sourceforge.net> Copyright (c) 2009-2014 Tobias Doerffel
...
patmanInstrument::patmanInstrument(InstrumentTrack* _instrument_track) :
      ...
      m_patchFile(QString::null), m_loopedModel(true, this),
                                ...
                                sampleFrame*    _working_buffer)
{
    ...
    float play_freq
            = hdata->tuned ? _n->frequency() : hdata->sample->frequency();

    if(hdata->sample->play(_working_buffer + offset, hdata->state, frames,
                           play_freq,
                           m_loopedModel.value() ? SampleBuffer::LoopOn
                                                 : SampleBuffer::LoopOff))
    {
...
void patmanInstrument::setFile(const QString& _patch_file, bool _rename)
{
           ...
           || m_patchFile == ""))
    {
        ...
        instrumentTrack()->setName(QFileInfo(_patch_file).fileName());
    }
    ...
    LoadErrors error
            = loadPatch(SampleBuffer::tryToMakeAbsolute(_patch_file));
    if(error)
    {
        printf("Load error\n");
    }
...
patmanInstrument::LoadErrors
        patmanInstrument::loadPatch(const QString& _filename)
{
    unloadCurrentPatch();
...
void patmanInstrument::selectSample(NotePlayHandle* _n)
{
        ...
        it != m_patchSamples.end(); ++it)
    {
        ...
        float dist
                = freq >= patch_freq ? freq / patch_freq : patch_freq / freq;
      ...
      InstrumentView(_instrument, _parent), m_pi(NULL)
{
    ...
    m_openFileButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("select_file_on"));
    m_openFileButton->setInactiveGraphic(
...
void PatmanView::openFile(void)
{
    ...
    {
        QString f
                = ConfigManager::inst()->userSamplesDir() + m_pi->m_patchFile;
...
void PatmanView::updateFilename(void)
{
    ...
    while(idx > 0
          && fm.size(Qt::TextSingleLine, m_displayFilename + "...").width()
                     < 225)
    {
