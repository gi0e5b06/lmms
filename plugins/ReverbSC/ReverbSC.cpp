      ...
      m_reverbSCControls(this)
{
    ...
    sp_dcblock_init(
            sp, dcblk[0],
            Engine::mixer()->currentQualitySettings().sampleRateMultiplier());
    sp_dcblock_init(
            sp, dcblk[1],
            Engine::mixer()->currentQualitySettings().sampleRateMultiplier());
}
...
bool ReverbSCEffect::processAudioBuffer(sampleFrame* _buf,
                                        const fpp_t  _frames)
{
    ...
    ValueBuffer* inGainBuf
            = m_reverbSCControls.m_inputGainModel.valueBuffer();
    ...
    ValueBuffer* outGainBuf
            = m_reverbSCControls.m_outputGainModel.valueBuffer();
    ...
    for(fpp_t f = 0; f < _frames; ++f)
    {
        ...
        s1 *= inGain;
        revsc->feedback
                = (SPFLOAT)(sizeBuf ? sizeBuf->value(f)
                                    : m_reverbSCControls.m_sizeModel.value());

        revsc->lpfreq = (SPFLOAT)(
                colorBuf ? colorBuf->value(f)
