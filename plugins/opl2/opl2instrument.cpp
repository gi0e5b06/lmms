...
// - Extras:
//   - double release: first release is in effect until noteoff (heard if
//   percussive sound),
...
const unsigned int adlib_opadd[OPL2_VOICES]
...
opl2instrument::opl2instrument(InstrumentTrack* _instrument_track) :
      ...
      op1_a_mdl(14., 0., 15., 1., this, tr("Op 1 Attack")),
      op1_d_mdl(14., 0., 15., 1., this, tr("Op 1 Decay")),
      op1_s_mdl(3., 0., 15., 1., this, tr("Op 1 Sustain")),
      op1_r_mdl(10., 0., 15., 1., this, tr("Op 1 Release")),
      op1_lvl_mdl(62., 0., 63., 1., this, tr("Op 1 Level")),
      op1_scale_mdl(0., 0., 3., 1., this, tr("Op 1 Level Scaling")),
      op1_mul_mdl(0., 0., 15., 1., this, tr("Op 1 Frequency Multiple")),
      feedback_mdl(0., 0., 7., 1., this, tr("Op 1 Feedback")),
      ...
      op1_vib_mdl(false, this, tr("Op 1 Vibrato")), op1_w0_mdl(),
      op1_w1_mdl(), op1_w2_mdl(), op1_w3_mdl(),
      op1_waveform_mdl(0, 0, 3, this, tr("Op 1 Waveform")),

      op2_a_mdl(1., 0., 15., 1., this, tr("Op 2 Attack")),
      op2_d_mdl(3., 0., 15., 1., this, tr("Op 2 Decay")),
      op2_s_mdl(14., 0., 15., 1., this, tr("Op 2 Sustain")),
      op2_r_mdl(12., 0., 15., 1., this, tr("Op 2 Release")),
      op2_lvl_mdl(63., 0., 63., 1., this, tr("Op 2 Level")),
      op2_scale_mdl(0., 0., 3., 1., this, tr("Op 2 Level Scaling")),
      op2_mul_mdl(1., 0., 15., 1., this, tr("Op 2 Frequency Multiple")),
      ...
      op2_vib_mdl(true, this, tr("Op 2 Vibrato")), op2_w0_mdl(), op2_w1_mdl(),
      op2_w2_mdl(), op2_w3_mdl(),
      ...
      trem_depth_mdl(false, this, tr("Tremolo Depth"))
{
    ...
    emulatorMutex.lock();
    theEmulator = new CTemuopl(Engine::mixer()->processingSampleRate(), true,
                               false);
    theEmulator->init();
    ...
    // Can the buffer size change suddenly? I bet that would break lots of
    // stuff
...
// Samplerate changes when choosing oversampling, so this is more or less
// mandatory
...
void opl2instrument::setVoiceVelocity(int voice, int vel)
{
    ...
    // Only calculate for operator 1 if in adding mode, don't want to change
    // timbre
    if(fm_mdl.value() == false)
    {
        vel_adjusted = 63 - (op1_lvl_mdl.value() * vel / 127.);
    }
    else
    {
    ...
    vel_adjusted = 63 - (op2_lvl_mdl.value() * vel / 127.);
...
#ifdef false
    printf("<-- %d %d %d %d %d %d %d %d %d \n", voiceLRU[0], voiceLRU[1],
           voiceLRU[2], voiceLRU[3], voiceLRU[4], voiceLRU[5], voiceLRU[6],
           voiceLRU[7], voiceLRU[8]);
#endif
    return tmp;
}
...
bool opl2instrument::handleMidiEvent(const MidiEvent& event,
                                     const MidiTime&  time,
                                     f_cnt_t          offset)
{
    emulatorMutex.lock();
    ...
    switch(event.type())
    {
        case MidiNoteOn:
            ...
            {
                // Turn voice on, NB! the frequencies are straight by voice
                // number, not by the adlib_opadd table!
                theEmulator->write(0xA0 + voice, fnums[key] & 0xff);
                theEmulator->write(0xB0 + voice,
                                   32 + ((fnums[key] & 0x1f00) >> 8));
        ...
        case MidiPitchBend:
            ...
            {
                int vn      = (voiceNote[v]
                          & ~OPL2_VOICE_FREE);  // remove the flag bit
                int playing = (voiceNote[v] & OPL2_VOICE_FREE)
                              == 0;  // just the flag bit
                theEmulator->write(0xA0 + v, fnums[vn] & 0xff);
                theEmulator->write(0xB0 + v,
                                   (playing ? 32 : 0)
                                           + ((fnums[vn] & 0x1f00) >> 8));
            }
            break;
        case MidiControlChange:
            switch(event.controllerNumber())
            {
                ...
                default:
#ifdef LMMS_DEBUG
                    // qInfo("Midi CC %02x %02x", event.controllerNumber(),
...
void opl2instrument::loadPatch(const unsigned char inst[14])
{
    ...
    for(int v = 0; v < OPL2_VOICES; ++v)
    {
        theEmulator->write(0x20 + adlib_opadd[v],
                           inst[0]);  // op1 AM/VIB/EG/KSR/Multiplier
        theEmulator->write(0x23 + adlib_opadd[v], inst[1]);  // op2
        // theEmulator->write(0x40+adlib_opadd[v],inst[2]); // op1 KSL/Output
        // Level - these are handled by noteon/aftertouch code
...
int opl2instrument::Hz2fnum(frequency_t Hz)
{
    ...
    {
        unsigned int fnum = Hz * exp2(20. - double(block)) * (1. / 49716.);
        if(fnum < 1023)
        {
...
void opl2instrument::loadFile(const QString& file)
{
    ...
    if(!file.isEmpty() && QFileInfo(file).exists())
    {
        ...
        if(sbiname.size() > 0
           && instrumentTrack()->displayName() == storedname)
        {
...
#ifdef false
        printf("SBI: %02x %02x %02x %02x %02x -- %02x %02x %02x %02x %02x "
               "%02x\n",
               (unsigned char)sbidata[36], (unsigned char)sbidata[37],
               (unsigned char)sbidata[38], (unsigned char)sbidata[39],
               (unsigned char)sbidata[40], (unsigned char)sbidata[41],
               (unsigned char)sbidata[42], (unsigned char)sbidata[43],
               (unsigned char)sbidata[44], (unsigned char)sbidata[45],
...
#define KNOB_GEN(knobname, hinttext, hintunit, xpos, ypos) \
    ...
    knobname->setCenterPointX(11.);                        \
    knobname->setCenterPointY(11.);                        \
    knobname->setTotalAngle(270.);                         \
...
#define BUTTON_GEN(buttname, tooltip, xpos, ypos)                          \
    ...
    buttname->setInactiveGraphic(                                          \
            PLUGIN_NAME::getIconPixmap("opl2_led_off"));                   \
    buttname->setCheckable(true);                                          \
    ToolTip::add(buttname, tr(tooltip));                                   \
    buttname->move(xpos, ypos);

#define WAVEBUTTON_GEN(buttname, tooltip, xpos, ypos, icon_on, icon_off, \
                       buttgroup)                                        \
    buttname = new PixmapButton(this, NULL);                             \
    ...
    WAVEBUTTON_GEN(op1_w0_btn, "Sine", 154, 86, "wave1_on", "wave1_off",
                   op1_waveform);
    WAVEBUTTON_GEN(op1_w1_btn, "Half sine", 178, 86, "wave2_on", "wave2_off",
                   op1_waveform);
    WAVEBUTTON_GEN(op1_w2_btn, "Absolute sine", 199, 86, "wave3_on",
                   "wave3_off", op1_waveform);
    WAVEBUTTON_GEN(op1_w3_btn, "Quarter sine", 220, 86, "wave4_on",
                   "wave4_off", op1_waveform);
    ...
    WAVEBUTTON_GEN(op2_w0_btn, "Sine", 154, 176, "wave1_on", "wave1_off",
                   op2_waveform);
    WAVEBUTTON_GEN(op2_w1_btn, "Half sine", 178, 176, "wave2_on", "wave2_off",
                   op2_waveform);
    WAVEBUTTON_GEN(op2_w2_btn, "Absolute sine", 199, 176, "wave3_on",
                   "wave3_off", op2_waveform);
    WAVEBUTTON_GEN(op2_w3_btn, "Quarter Sine", 220, 176, "wave4_on",
                   "wave4_off", op2_waveform);
...
{
    // Envelope times in ms: t[0] = 0, t[n] = ( 1<<n ) * X, X = 0.11597 for A,
...
void opl2instrumentView::modelChanged()
{
    ...
    connect(&m->op1_a_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));
    connect(&m->op2_a_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));

    connect(&m->op1_d_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));
    connect(&m->op2_d_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));

    connect(&m->op1_r_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));
    connect(&m->op2_r_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));

    connect(&m->op1_mul_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));
    connect(&m->op2_mul_mdl, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHints()));
