...
PadsGDXWaveView::PadsGDXWaveView(QWidget* _parent, int _w, int _h) :
      QWidget(_parent), m_sampleBuffer(NULL),
      m_graph(QPixmap(_w - 2 * s_padding, _h - 2 * s_padding)), m_from(0),
      m_to(0), m_last_from(0), m_last_to(0),
      // m_last_amp( 0 ),
      m_startKnob(NULL), m_endKnob(NULL), m_loopStartKnob(NULL),
      m_loopEndKnob(NULL),
      // m_isDragging( false ),
      m_reversed(false), m_framesPlayed(0),
    ...
    if(m_sampleBuffer)
    {
        ...
        m_endFrameX = graph_rect.x()
                      + (m_sampleBuffer->endFrame() - m_from)
                                * double(graph_rect.width()) / frames;
        m_loopStartFrameX = graph_rect.x()
                            + (m_sampleBuffer->loopStartFrame() - m_from)
                                      * double(graph_rect.width()) / frames;
        m_loopEndFrameX = graph_rect.x()
                          + (m_sampleBuffer->loopEndFrame() - m_from)
                                    * double(graph_rect.width()) / frames;

        const int xw = (m_framesPlayed - m_from) * double(graph_rect.width())
                       / frames;
        ...
        p.fillRect(m_loopStartFrameX + 1, yw,
                   ...
                   QColor(0, 255, 0, 128));
        // z 3
        p.fillRect(m_loopEndFrameX + 1, yw, m_endFrameX - m_loopEndFrameX - 1,
                   hw, QColor(255, 0, 0, 128));
...
void PadsGDXWaveView::updateGraph(bool _full)
{
    ...
    */
    ...
    {
        m_to = qMin(m_sampleBuffer->frames(),
                    m_sampleBuffer->endFrame() * 11 / 10);
    }
    ...
    if(!_full && m_last_from == m_from && m_last_to == m_to)
...
void PadsGDXWaveView::setKnobs(knob* _start,
                               knob* _end,
                               knob* _loopStart,
                               knob* _loopEnd)
{
...
void PadsGDXWaveView::mouseDoubleClickEvent ( QMouseEvent * _me )
{
        ...
        case sample_loopStart:
                ...
                step1=m_sampleBuffer->findClosestZero(step0);
                qInfo("mouseDoubleClickEvent: loopStart=%d,
step=%d",step0,step1); slideSamplePointByFrames(loopStart,step1-step0); break;
        case sample_loopEnd:
                step0=m_sampleBuffer->loopEndFrame();
                step1=m_sampleBuffer->findClosestZero(step0);
                qInfo("mouseDoubleClickEvent: loopEnd=%d,
step=%d",step0,step1); slideSamplePointByFrames(loopEnd,step1-step0); break;
        default:
                break;
        }
...
void PadsGDXWaveView::zoom( const bool _out )
{
        ...
        if( ( _out && d_from < d_to ) || ( ! _out && d_to < d_from ) )
        {
                ...
                new_to = qBound(
                        ...
                        m_to + f_cnt_t( step_to * ( new_from == m_from ? 1 :
comp_ratio ) ), frames
                );
        }
        else
        {
                ...
                new_from = qBound(
                        0,
                        m_from + f_cnt_t( step_from * ( new_to == m_to ? 1 :
comp_ratio ) ), start
                );
        }
...
void PadsGDXWaveView::slide( int _px )
{
        ...
        f_cnt_t step_from = qBound( 0, m_from + step, m_sampleBuffer->frames()
) - m_from; f_cnt_t step_to = qBound( m_from + 1, m_to + step,
m_sampleBuffer->frames() ) - m_to;
...
void PadsGDXWaveView::slideSamplePointByFrames(knobType _point, f_cnt_t
_frames, bool _slideTo)
{
        if(!m_sampleBuffer) return;
        ...
        case loopEnd:
                ...
                break;
        default:
                qFatal("PadsGDXWaveView::slideSamplePointByFrames invalid
KnobType: %d",_point);
        }
...
void PadsGDXWaveView::knob::convert(const QPoint& _p, FLOAT& value_, FLOAT&
dist_)
{
...
bool PadsGDXWaveView::knob::checkBound(double _v) const
{
        ...
        const double d1 = qAbs( m_relatedKnob->model()->value() -
model()->value() )
                * ( m_waveView->m_sampleBuffer->frames() )
...
{
    if(model() && model()->isDefaultConstructed())
        return;
    Knob::paintEvent(_pe);
}
