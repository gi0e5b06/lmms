      ...
      InstrumentView(_instrument, _parent)
{
    ...
    */
    ...
    m_loadSFZFileButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("load_sf"));
    m_loadSFZFileButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("load_sf"));
    connect(m_loadSFZFileButton, SIGNAL(clicked()), this,
            SLOT(loadSFZFile()));
    ...
    */
    ...
    m_saveSFZFileButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("save_sf"));
    m_saveSFZFileButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("save_sf"));
    connect(m_saveSFZFileButton, SIGNAL(clicked()), this,
            SLOT(saveSFZFile()));
    ...
    */
    ...
    m_waveView->setKnobs(
            dynamic_cast<PadsGDXWaveView::knob*>(m_startKnob),
...
void PadsGDXView::doConnections()
{
    ...
    connect(a, SIGNAL(isPlaying(f_cnt_t)), m_waveView,
            SLOT(onPlaying(f_cnt_t)));

    connect(a, SIGNAL(keyUpdated(int)), this, SLOT(onKeyUpdated(int)));

    disconnect(a, SIGNAL(dataChanged()), this, SLOT(update()));

    connect(a, SIGNAL(dataChanged()), this, SLOT(onModelChanged()));

    connect(a, SIGNAL(sampleUpdated()), this, SLOT(onSampleUpdated()));
}
...
void PadsGDXView::onModelChanged()
{
    ...
    /*
      TODO
      FloatModel* startPointModel = nullptr;
      if(key >= 0 && key <= 127 && a->currentSample() != nullptr)
      startPointModel = a->m_startPointModel[key];
      if(startPointModel == nullptr)
      startPointModel = new FloatModel(0., 0., 0., 1., nullptr, "", true);
      m_startKnob->setModel(startPointModel);
    */
...
void PadsGDXView::dropEvent(QDropEvent* _de)
{
    ...
    // qInfo("PadsGDXView::dropEvent %s
    // %s",qPrintable(type),qPrintable(value));
    ...
    else if(type == QString("tco_%1").arg(Track::SampleTrack))
    {
        ...
        QString  fileName
                = dataFile.content().firstChild().toElement().attribute(
                        "src");
...
void PadsGDXView::paintEvent(QPaintEvent*)
{
    ...
    if(sample)
    {
        ...
        fileName         = fileName.replace(
                QRegExp(QString("^.*%1").arg(QDir::separator())), "");
...
QString PadsGDXView::selectSFZFile(const QString& _file)
{
    ...
    // set filters
    QStringList types;
    types << tr("All Soundfonts (*.sfz)") << tr("SFZ-Files (*.sfz)");
