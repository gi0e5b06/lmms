...
PadsGDX::PadsGDX(InstrumentTrack* _instrument_track) :
      ...
      m_currentKey(-1), m_checking(false), m_loading(false), m_SFZFile(""),
      m_nextPlayStartPoint(0), m_nextPlayBackwards(false)
{
    for(int i = 0; i < 128; i++)
    {
        m_sampleBuffer[i]        = nullptr;
        m_startPointModel[i]     = nullptr;
        m_endPointModel[i]       = nullptr;
        m_loopStartPointModel[i] = nullptr;
        m_loopEndPointModel[i]   = nullptr;
        m_reverseModel[i]        = nullptr;
        m_loopModel[i]           = nullptr;
        m_stutterModel[i]        = nullptr;
    }
...
void PadsGDX::playNote(NotePlayHandle* _n, sampleFrame* _buffer)
{
    ...
    // played.
    if(m_stutterModel[key] && m_stutterModel[key]->value() == true
       && _n->frequency() < 20.0)
    {
    ...
    if(!_n->m_pluginData)
    {
        ...
        */
        _n->m_pluginData = new SampleBuffer::HandleState(
                m_nextPlayStartPoint, _n->hasDetuningInfo(), 10,
                m_nextPlayBackwards);
    ...
    if(!_n->isFinished())
    {
        if(sample->play(_buffer + offset,
                        ...
                        440,  // force _n->frequency(),
                        static_cast<SampleBuffer::LoopMode>(
                                m_loopModel[key]->value()),
                        _n->isReleased()))
        {
            applyRelease(_buffer, _n);
            instrumentTrack()->processAudioBuffer(_buffer, frames + offset,
                                                  _n);
...
int PadsGDX::getBeatLen(NotePlayHandle* _n) const
{
    ...
    const real_t freq_factor = BaseFreq / _n->frequency()
                               * Engine::mixer()->processingSampleRate()
                               / Engine::mixer()->baseSampleRate();

    return static_cast<int>(
            floor((sample->endFrame() - sample->startFrame()) * freq_factor));
}
...
void PadsGDX::createKey(int _key, SampleBuffer* _sample, double _semitones)
{
    ...
    m_ampModel[_key]
            = new FloatModel(100., 0., 500., 0.1, this, tr("Amplify"));
    m_tuneModel[_key]
            = new FloatModel(_semitones, -144., 144., 0.01, this, tr("Tune"));
    m_startPointModel[_key]     = new FloatModel(0., 0., 1., 0.0000001, this,
                                             tr("Start of sample"));
    m_endPointModel[_key]       = new FloatModel(1., 0., 1., 0.0000001, this,
                                           tr("End of sample"));
    m_loopStartPointModel[_key] = new FloatModel(0., 0., 1., 0.0000001, this,
                                                 tr("Start of loop"));
    m_loopEndPointModel[_key]
            = new FloatModel(1., 0., 1., 0.0000001, this, tr("End of loop"));
    ...
    _sample->retune(m_tuneModel[_key]->rawValue());

    connect(_sample, SIGNAL(sampleUpdated()), this, SLOT(onSampleUpdated()));

    connect(m_ampModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onAmpModelChanged()));
    connect(m_tuneModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onTuneModelChanged()));
    connect(m_reverseModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onReverseModelChanged()));
    connect(m_startPointModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onStartPointChanged()));
    connect(m_endPointModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onEndPointChanged()));
    connect(m_loopStartPointModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onLoopStartPointChanged()));
    connect(m_loopEndPointModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onLoopEndPointChanged()));
    connect(m_stutterModel[_key], SIGNAL(dataChanged()), this,
            SLOT(onStutterModelChanged()));
...
void PadsGDX::destroyKey(int _key)
{
    ...
    m_sampleBuffer[_key] = nullptr;
    if(old)
    {
        disconnect(old, SIGNAL(sampleUpdated()), this,
                   SLOT(onSampleUpdated()));
        delete old;
    }
    if(m_startPointModel[_key])
    {
        delete m_startPointModel[_key];
        m_startPointModel[_key] = nullptr;
    }
    if(m_endPointModel[_key])
    {
        delete m_endPointModel[_key];
        m_endPointModel[_key] = nullptr;
    }
    if(m_loopStartPointModel[_key])
    {
        delete m_loopStartPointModel[_key];
        m_loopStartPointModel[_key] = nullptr;
    }
    if(m_loopEndPointModel[_key])
    {
        delete m_loopEndPointModel[_key];
        m_loopEndPointModel[_key] = nullptr;
    }
    if(m_reverseModel[_key])
    {
        delete m_reverseModel[_key];
        m_reverseModel[_key] = nullptr;
    }
    if(m_loopModel[_key])
    {
        delete m_loopModel[_key];
        m_loopModel[_key] = nullptr;
    }
    if(m_stutterModel[_key])
    {
        delete m_stutterModel[_key];
        m_stutterModel[_key] = nullptr;
    }
}
...
SampleBuffer* PadsGDX::currentSample()
{
    ...
    // if(key<0 || key>127) return nullptr;
...
void PadsGDX::setAudioFile(const QString& _audioFile, bool _rename)
{
    ...
    if(!sample)
    {
        ...
        */
        ...
        sample->setAudioFile(_audioFile);
...
void PadsGDX::onAmpModelChanged()
{
    ...
    if(sample)
    {
        sample->setAmplification(m_ampModel[key]->value() / 100.);
        emit dataChanged();
    }
}
...
void PadsGDX::onTuneModelChanged()
{
    ...
    if(sample)
    {
        qInfo("PadsGDX::onTuneModelChanged t=%f", m_tuneModel[key]->value());
...
bool PadsGDX::checkPointBounds(int _key)
{
    ...
    real_t old_start     = m_startPointModel[_key]->value();
    real_t old_end       = m_endPointModel[_key]->value();
    real_t old_loopStart = m_loopStartPointModel[_key]->value();
    real_t old_loopEnd   = m_loopEndPointModel[_key]->value();

    const real_t MINSZ = 0.0001;

    // check if start is over end and swap values if so
    if(m_startPointModel[_key]->value() > m_endPointModel[_key]->value())
    {
        real_t tmp = m_endPointModel[_key]->value();
        m_endPointModel[_key]->setValue(m_startPointModel[_key]->value());
        m_startPointModel[_key]->setValue(tmp);
    }

    if(abs(m_startPointModel[_key]->value() - m_endPointModel[_key]->value())
       < MINSZ)
    {
        m_endPointModel[_key]->setValue(m_startPointModel[_key]->value()
                                        + MINSZ);
        m_startPointModel[_key]->setValue(m_endPointModel[_key]->value()
                                          - MINSZ);
    }

    // check if start is over end and swap values if so
    if(m_loopStartPointModel[_key]->value()
       > m_loopEndPointModel[_key]->value())
    {
        real_t tmp = m_loopEndPointModel[_key]->value();
        m_loopEndPointModel[_key]->setValue(
                m_loopStartPointModel[_key]->value());
        m_loopStartPointModel[_key]->setValue(tmp);
    }

    m_loopStartPointModel[_key]->setValue(
            qBound(m_startPointModel[_key]->value(),
                   m_loopStartPointModel[_key]->value(),
    ...
    if(abs(m_loopStartPointModel[_key]->value()
           - m_loopEndPointModel[_key]->value())
       < MINSZ)
    {
        m_loopEndPointModel[_key]->setValue(
                m_loopStartPointModel[_key]->value() + MINSZ);
        m_loopStartPointModel[_key]->setValue(
                m_loopEndPointModel[_key]->value() - MINSZ);
...
void PadsGDX::onPointChanged()
{
    ...
    f_cnt_t   f_start   = static_cast<f_cnt_t>(m_startPointModel[key]->value()
                                           * lastFrame);
    f_cnt_t   f_end
            = static_cast<f_cnt_t>(m_endPointModel[key]->value() * lastFrame);
    f_cnt_t f_loopStart = static_cast<f_cnt_t>(
            m_loopStartPointModel[key]->value() * lastFrame);
    f_cnt_t f_loopEnd = static_cast<f_cnt_t>(m_loopEndPointModel[key]->value()
                                             * lastFrame);
    ...
    // qInfo("POINTS s=%d e=%d ls=%d
    // le=%d",f_start,f_end,f_loopStart,f_loopEnd);
...
void PadsGDX::loadSFZ(const QString& _file)
{
    ...
    else
    {
        ...
        while(!s.isEmpty())
        {
            ...
            {
                QString sc = s.section(QRegExp("<[a-z]+>"), 0, 1,
                                       QString::SectionIncludeLeadingSep);
                qInfo("[SC]%s\n---\n", sc.toUtf8().constData());
                s = s.mid(sc.length()).trimmed();
                qInfo("[S]%s\n---\n", s.toUtf8().constData());

                QStringList opcodes = sc.split(QRegExp("\\b(?=[_a-z]+=)"));
            ...
            {
                QString sg = s.section(QRegExp("<[a-z]+>"), 0, 1,
                                       QString::SectionIncludeLeadingSep);
                qInfo("[SG]%s\n---\n", sg.toUtf8().constData());
                s = s.mid(sg.length()).trimmed();
                qInfo("[S]%s\n---\n", s.toUtf8().constData());
                continue;
            }
            if(!s.startsWith("<group>"))
            {
                qWarning("SFZ: error: %s", s.toUtf8().constData());
                break;
            }
            ...
            foreach(QString g, groups)
            {
                ...
                foreach(QString r, regions)
                {
                    ...
                    foreach(QString k, tr.keys())
                    {
                        ...
                        if(k == "lokey")
                        {
                            f_lokey = Note::findKeyNum(v);
                            if(f_lokey == -1)
                                f_lokey = v.toInt();
                        }
                        if(k == "hikey")
                        {
                            f_hikey = Note::findKeyNum(v);
                            if(f_hikey == -1)
                                f_hikey = v.toInt();
                        }
                        if(k == "pitch_keycenter")
                        {
                            f_pitch_keycenter = Note::findKeyNum(v);
                            if(f_pitch_keycenter == -1)
                                f_pitch_keycenter = v.toInt();
                        }
                        if(k == "transpose")
                            f_transpose = v.toInt();
                        if(k == "tune")
                    ...
                    if(f_sample == "")
                        continue;
                    f_sample.replace('\\', '/');
                    QDir cd = QFileInfo(in).dir();
                    QDir dp = QDir(defaultPath);
                    if(dp.isRelative())
                        dp = QDir(cd.path() + QDir::separator()
                                  + defaultPath);

                    QFileInfo fi(QFileInfo(dp, f_sample));
                    if(!fi.exists())
                    {
                        qWarning("PadsGDX: sample file not found: %s",
                                 fi.absoluteFilePath().toUtf8().constData());
                        if(!defaultPath.isEmpty())
                            fi = QFileInfo(cd, f_sample);
                    }
                    if(!fi.exists())
                    {
                        qWarning("PadsGDX: sample file not found: %s",
                                 fi.absoluteFilePath().toUtf8().constData());
                        fi = QFileInfo(dp, f_sample);
                    }
                    if(!fi.exists())
                    {
                        qWarning("PadsGDX: sample file not found: %s",
                    ...
                    if((f_lokey == -1) || (f_hikey == -1)
                       || (f_pitch_keycenter == -1) || (f_lokey > f_hikey)
                       || (f_lokey < 0) || (f_hikey > 127))
                    {
