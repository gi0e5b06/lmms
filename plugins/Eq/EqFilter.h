...
/// A wrapper for the StereoBiQuad class, giving it freq, res, and gain
/// controls. It is designed to process periods in one pass, with
/// recalculation of coefficents upon parameter changes. The intention is to
/// use this as a bass class, children override the calcCoefficents()
/// function, providing the coefficents a1, a2, b0, b1, b2.
///
class EqFilter : public StereoBiQuad
{
  public:
    ...
    virtual /*inline*/ void setFrequency(frequency_t freq)
    {
        if(freq != m_freq)
        {
            m_freq = freq;
    ...
    virtual /*inline*/ void setQ(real_t res)
    {
        if(res != m_res)
        {
            m_res = res;
    ...
    virtual /*inline*/ void setGain(real_t gain)
    {
        if(gain != m_gain)
        {
            m_gain = gain;
    ...
    virtual /*inline*/ void setParameters(sample_rate_t sampleRate,
                                          frequency_t   freq,
                                          real_t        res,
                                          real_t        gain)
    {
        bool hasChanged = (freq != m_freq || res != m_res || gain != m_gain
...
class EqHp12Filter : public EqFilter
{
  public:
    ...
    {
        // calc intermediate
        real_t w0    = R_2PI * m_freq / m_sampleRate;
        real_t c     = cos(w0);
        real_t s     = sin(w0);
        real_t alpha = s / (2 * m_res);
...
class EqPeakFilter : public EqFilter
{
  public:
    virtual void calcCoefficents()
    {
        ...
        real_t c     = cos(w0);
        real_t s     = sin(w0);
        real_t A     = pow(10, m_gain * 0.025);
        real_t alpha = s * sinh(R_2_LOG / 2. * m_bw * w0 / sin(w0));
        real_t a0, a1, a2, b0, b1, b2;  // coeffs to calculate
    ...
    virtual /*inline*/ void setParameters(real_t      sampleRate,
                                          frequency_t freq,
                                          real_t      bw,
                                          real_t      gain)
    {
