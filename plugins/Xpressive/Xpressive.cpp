 ...
 * nice test:
O1 ->
trianglew(2t*f)*(0.5+0.5sinew(12*A1*t+0.5))+sinew(t*f)*(0.5+0.5sinew(12*A1*t))
O2 ->
trianglew(2t*f)*(0.5+0.5sinew(12*A1*t))+sinew(t*f)*(0.5+0.5sinew(12*A1*t+0.5))
*/
...
Xpressive::Xpressive(InstrumentTrack* instrument_track) :
      ...
      m_graphO1(-1.0f, 1.0f, 360, this), m_graphO2(-1.0f, 1.0f, 360, this),
      ...
      m_interpolateW1(false, this), m_interpolateW2(false, this),
      ...
      m_exprValid(false, this)
{
    m_outputExpression[0]
            = "sinew(integrate(f*(1+0.05sinew(12t))))*(2^(-(1.1+A2)*t)*(0.4+"
              "0.1(1+A3)+0.4sinew((2.5+2A1)t))^2)";
...
void Xpressive::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
    ...
    _this.setAttribute("W1sample",
                       base64::encodeFloats(m_rawgraphW1.samples(),
                                            m_rawgraphW1.length()));
    _this.setAttribute("W2sample",
                       base64::encodeFloats(m_rawgraphW2.samples(),
                                            m_rawgraphW2.length()));
    _this.setAttribute("W3sample",
                       base64::encodeFloats(m_rawgraphW3.samples(),
                                            m_rawgraphW3.length()));
...
void Xpressive::loadSettings(const QDomElement& _this)
{
    ...
    base64::decodeFloats(_this.attribute("W2sample"), &dst);
    m_rawgraphW2.setSamples(dst);
    delete[] dst;

    base64::decodeFloats(_this.attribute("W3sample"), &dst);
    m_rawgraphW3.setSamples(dst);
    delete[] dst;
...
void Xpressive::playNote(NotePlayHandle* nph, sampleFrame* working_buffer)
{
    ...
    if(nph->totalFramesPlayed() == 0 || nph->m_pluginData == nullptr)
    {
        ...
        ExprFront* exprO1
                = new ExprFront(m_outputExpression[0].constData(),
                                Engine::mixer()->processingSampleRate());
        ExprFront* exprO2
                = new ExprFront(m_outputExpression[1].constData(),
                                Engine::mixer()->processingSampleRate());
        ...
        nph->m_pluginData = new ExprSynth(
                &m_W1, &m_W2, &m_W3, exprO1, exprO2, nph,
                Engine::mixer()->processingSampleRate(), &m_panning1,
                &m_panning2, m_relTransition.value());
    }
    ...
    instrumentTrack()->processAudioBuffer(working_buffer, frames + offset,
                                          nph);
}
...
class XpressiveKnob : public Knob
{
  public:
    ...
    XpressiveKnob(QWidget* _parent) : Knob(knobStyled, _parent)
    {
        setStyle();
    }
};
...

{
    ...
    m_helpBtn->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("help_inactive"));
    ...
    m_sinWaveBtn->setInactiveGraphic(
            embed::getIconPixmap("sin_wave_inactive"));
    ...
    m_moogWaveBtn->setInactiveGraphic(
    ...
    m_expWaveBtn->setInactiveGraphic(
    ...
    m_sawWaveBtn->setInactiveGraphic(
    ...
    m_usrWaveBtn->setInactiveGraphic(
            embed::getIconPixmap("usr_wave_inactive"));
    ...
    m_waveInterpolate = new LedCheckBox(
            "Interpolate", this, tr("WaveInterpolate"), LedCheckBox::Green);
...
void XpressiveView::expressionChanged()
{
    ...
    if(text.size() > 0)
    {
        ...
        if(m_output_expr)
        {
            ...
            expr.add_cyclic_vector("W1", e->graphW1().samples(),
                                   e->graphW1().length());
            expr.add_cyclic_vector("W2", e->graphW2().samples(),
                                   e->graphW2().length());
            expr.add_cyclic_vector("W3", e->graphW3().samples(),
                                   e->graphW3().length());
        }
...
void Xpressive::smooth(float             smoothness,
                       const GraphModel* in,
                       GraphModel*       out)
{
    ...
    if(smoothness > 0)
    {
        ...
        const float  a        = 1.0f / (sqrtf(F_2PI) * delta);  // 2.0f * F_PI
        ...
        for(i = 0; i < guass_size; i++)
        {
            ...
            sum += guassian[i] = a * expf(-0.5f * temp * temp);  // powf(F_E,
        }
        for(i = 0; i < guass_size; i++)
        {
...
void XpressiveView::smoothChanged()
{
    ...
    Engine::getSong()->setModified();
    m_graph->setEnabled(m_smoothKnob->model()->value() == 0
                        && m_expressionEditor->toPlainText().size() == 0);
}
...
void XpressiveView::updateLayout()
{
    ...
    switch(m_selectedGraphGroup->model()->value())
    {
        case W1_EXPR:
            ...
            m_graph->setEnabled((e->smoothW1().value() == 0
                                 && e->wavesExpression(0).size() == 0));
        ...
        case W2_EXPR:
            ...
            m_graph->setEnabled((e->smoothW2().value() == 0
                                 && e->wavesExpression(1).size() == 0));
        ...
        case W3_EXPR:
            ...
            m_graph->setEnabled((e->smoothW3().value() == 0
                                 && e->wavesExpression(2).size() == 0));
...
QString XpressiveHelpView::s_helpText
        = "<b>O1, O2</b> - Two output waves. Panning is controled by PN1 and "
          "PN2.<br>"
          "<b>W1, W2, W3</b> - Wave samples evaluated by expression. In "
          "these samples, t variable ranges [0,1).<br>"
          "These waves can be used as functions inside the output waves (O1, "
          "O2). The wave period is 1.<br>"
          "<h4>Available variables:</h4><br>"
          "<b>t</b> - Time in seconds.<br>"
          "<b>f</b> - Note's pitched frequency. Available only in the output "
          "expressions.<br>"
          "<b>key</b> - Note's keyboard key. 0 denotes C0, 48 denotes C4, 96 "
          "denotes C8. Available only in the output expressions.<br>"
          "<b>bnote</b> - Base note. By default it is 57 which means A5, "
          "unless you change it.<br>"
          "<b>srate</b> - Sample rate. In wave expression it returns the "
          "wave's number of samples.<br>"
          "<b>tempo</b> - Song's Tempo. Available only in the output "
          "expressions.<br>"
          "<b>v</b> - Note's volume. Note that the output is already "
          "multiplied by the volume. Available only in the output "
          "expressions.<br>"
          "<b>rel</b> - Gives 0.0 while the key is holded, and 1.0 after the "
          "key release. Available only in the output expressions.<br>"
          "<b>trel</b> - Time after release. While the note is holded, it "
          "gives 0.0. Afterwards, it start counting seconds.<br>"
          "The time it takes to shift from 0.0 to 1.0 after key release is "
          "determined by the REL knob<br>"
          "<b>seed</b> - A random value that remains consistent in the "
          "lifetime of a single wave. meant to be used with <b>randsv</b><br>"
          "<b>A1, A2, A3</b> - General purpose knobs. You can reference them "
          "only in O1 and O2. In range [-1,1].<br>"
          "<h4>Available functions:</h4><br>"
          "<b>W1, W2, W3</b> - As mentioned before. You can reference them "
          "only in O1 and O2.<br>"
          "<b>cent(x)</b> - Gives pow(2,x/1200), so you can multiply it with "
          "the f variable to pitch the frequency.<br>"
          "100 cents equals one semitone<br>"
          "<b>semitone(x)</b> - Gives pow(2,x/12), so you can multiply it "
          "with the f variable to pitch the frequency.<br>"
          "<b>last(n)</b> - Gives you the last n'th evaluated sample. In O1 "
          "and O2 it keeps a whole second. Thus the argument n must be in "
          "the range [1,srate], or else, it will return 0.<br>"
          "<b>integrate(x)</b> - Integrates x by delta t (It sums values and "
          "divides them by sample rate).<br>"
          "If you use notes with automated frequency, you should use:<br>"
          "sinew(integrate(f)) instead of sinew(t*f)<br>"
          "<b>randv(x)</b> - A random vector. Each cell is reference by an "
          "integer index in the range [0,2^31]<br>"
          "Each evaluation of an expression results in different random "
          "vector.<br>"
          "Although, it remains consistent in the lifetime of a single "
          "wave.<br>"
          "If you want a single random values you can use "
          "randv(0),randv(1)... <br>"
          ...
          "if you want to try different random values and make it consistent "
          "in each evaluation.<br>"
          "<b>sinew(x)</b> - A sine wave with period of 1 (In contrast to "
          "real sine wave which have a period of 2*pi).<br>"
          ...
          "<b>clamp(min_val,x,max_val)</b> - If x is in range of "
          "(min_val,max_val) it returns x. Otherwise if it's greater than "
          "max_val it returns max_val, else returns min_val.<br>"
          "<b>abs, sin, cos, tan, cot, asin, acos, atan, atan2, sinh, cosh, "
          "tanh, asinh, acosh, atanh, sinc, "
          "hypot, exp, log, log2, log10, logn, pow, sqrt, min, max, floor, "
          ...
          "<b>Frequency Modulation</b> - [vol1]*W1( integrate( f + "
          "srate*[vol2]*W2( integrate(f) ) ) )<br>"
          "<b>Phase Modulation</b> - [vol1]*W1( integrate(f) + [vol2]*W2( "
          "integrate(f) ) )<br>";
