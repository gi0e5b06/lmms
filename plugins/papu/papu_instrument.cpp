...
papuInstrument::papuInstrument(InstrumentTrack* _instrumentTrack) :
      ...
      m_ch1SweepRtShiftModel(
              4., 0., 7., 1., this, tr("Sweep RtShift amount")),
      m_ch1WavePatternDutyModel(
              2., 0., 3., 1., this, tr("Wave Pattern Duty")),
      m_ch1VolumeModel(15., 0., 15., 1., this, tr("Channel 1 volume")),
      m_ch1VolSweepDirModel(false, this, tr("Volume sweep direction")),
      m_ch1SweepStepLengthModel(
              0., 0., 7., 1., this, tr("Length of each step in sweep")),

      m_ch2WavePatternDutyModel(
              2., 0., 3., 1., this, tr("Wave Pattern Duty")),
      m_ch2VolumeModel(15., 0., 15., 1., this, tr("Channel 2 volume")),
      ...
      m_ch3VolumeModel(3., 0., 3., 1., this, tr("Channel 3 volume")),

      m_ch4VolumeModel(15., 0., 15., 1., this, tr("Channel 4 volume")),
      ...
      m_so1VolumeModel(7., 0., 7., 1., this, tr("Right Output level")),
      m_so2VolumeModel(7., 0., 7., 1., this, tr("Left Output level")),
      ...
      m_trebleModel(-20., -100., 200., 1., this, tr("Treble")),
      m_bassModel(461., -1., 600., 1., this, tr("Bass")),
...
void papuInstrument::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
    ...
    _this.setAttribute("sampleShape",
                       base64::encodeFloats(m_GraphModel.samples(),
                                            m_GraphModel.length()));
}
...
void papuInstrument::loadSettings(const QDomElement& _this)
{
    ...
    FLOAT* dst = 0;
    base64::decodeFloats(_this.attribute("sampleShape"), &dst);
    m_GraphModel.setSamples(dst);
    delete[] dst;
}
...
Engine::mixer()->processingSampleRate(); int maxrel = 0; for( int i = 0 ; i <
3 ; ++i )
        {
                              ...
                              sampleFrame*    _working_buffer)
{
    ...
    if(tfp == 0)
    {
        // PRNG Frequency = (1048576 Hz / (ratio + 1)) / 2 ^ (shiftclockfreq +
        // 1)
        char   sopt = 0;
        char   ropt = 1;
        real_t fopt = 524288. / (ropt * exp2(sopt + 1.0));
        real_t f;
        for(char s = 0; s < 16; s++)
            for(char r = 0; r < 8; r++)
            {
    ...
    while(framesleft > 0)
    {
        ...
        for(fpp_t frame = 0; frame < count; ++frame)
        {
            ...
            {
                sample_t s = sample_t(buf[frame * 2 + ch]) / 32768.;
...
{
  public:
    papuKnob(QWidget* _parent) : Knob(knobStyled, _parent)
    {
        setFixedSize(30, 30);
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    m_ch1SweepTimeKnob->setWhatsThis(
            tr("The amount of increase or"
    ...
    m_ch1SweepRtShiftKnob->setWhatsThis(
            tr("The rate at which increase or"
    ...
    m_ch1WavePatternDutyKnob->setWhatsThis(
            tr("The duty cycle is the ratio of"
    ...
    ToolTip::add(m_ch1SweepStepLengthKnob,
                 tr("Length of each step in sweep"));
    m_ch1SweepStepLengthKnob->setWhatsThis(
            tr("The delay between step change"));
    ...
    m_ch2WavePatternDutyKnob->setWhatsThis(
            tr("The duty cycle is the ratio of"
               " the duration (time) that a signal is ON"
               " versus the total period of the signal."));
    ...
    ToolTip::add(m_ch2SweepStepLengthKnob,
                 tr("Length of each step in sweep"));
    m_ch2SweepStepLengthKnob->setWhatsThis(
            tr("The delay between step change"));
    ...
    ToolTip::add(m_ch4SweepStepLengthKnob,
                 tr("Length of each step in sweep"));
    m_ch4SweepStepLengthKnob->setWhatsThis(
            tr("The delay between step change"));
...
extern "C"
{
    ...
    {
        return (new papuInstrument(static_cast<InstrumentTrack*>(_data)));
    }
}
