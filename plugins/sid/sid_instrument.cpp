...
unsigned char sidorder[]
        = {0x15, 0x16, 0x18, 0x17, 0x05, 0x06, 0x02, 0x03, 0x00,
           0x01, 0x04, 0x0c, 0x0d, 0x09, 0x0a, 0x07, 0x08, 0x0b,
           0x13, 0x14, 0x10, 0x11, 0x0e, 0x0f, 0x12};

static const char* attackTime[16]
        = {"2 mS",  "8 mS",  "16 mS",  "24 mS",  "38 mS",  "56 mS",
           "68 mS", "80 mS", "100 mS", "250 mS", "500 mS", "800 mS",
           "1 S",   "3 S",   "5 S",    "8 S"};
static const char* decRelTime[16]
        = {"6 mS",   "24 mS",  "48 mS",  "72 mS",  "114 mS", "168 mS",
           "204 mS", "240 mS", "300 mS", "750 mS", "1.5 S",  "2.4 S",
           "3 S",    "9 S",    "15 S",   "24 S"};
// release time time in ms
static const int relTime[16]
...
voiceObject::voiceObject(Model* _parent, int _idx) :
      ...
      m_attackModel(
              8., 0., 15., 1., this, tr("Voice %1 attack").arg(_idx + 1)),
      m_decayModel(8., 0., 15., 1., this, tr("Voice %1 decay").arg(_idx + 1)),
      m_sustainModel(
              15., 0., 15., 1., this, tr("Voice %1 sustain").arg(_idx + 1)),
      m_releaseModel(
              8., 0., 15., 1., this, tr("Voice %1 release").arg(_idx + 1)),
      m_coarseModel(0.,
                    -24.,
                    24.,
                    1.,
...
sidInstrument::sidInstrument(InstrumentTrack* _instrument_track) :
      ...
      // filter
      m_filterFCModel(1024., 0., 2047., 1., this, tr("Cutoff")),
      m_filterResonanceModel(8., 0., 15., 1., this, tr("Resonance")),
      m_filterModeModel(
              LowPass, 0, NumFilterTypes - 1, this, tr("Filter type")),

      // misc
      m_voice3OffModel(false, this, tr("Voice 3 off")),
      m_volumeModel(15., 0., 15., 1., this, tr("Volume")),
...
void sidInstrument::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
    ...
    for(int i = 0; i < 3; ++i)
    {
        ...
        m_voice[i]->m_pulseWidthModel.saveSettings(_doc, _this,
                                                   "pulsewidth" + is);
...
f_cnt_t sidInstrument::desiredReleaseFrames() const
{
    ...
    return f_cnt_t(real_t(relTime[maxrel]) * samplerate / 1000.);
}

static int sid_fillbuffer(
        unsigned char* sidreg, cSID* sid, int tdelta, short* ptr, int samples)
{
    int tdelta2;
    int result;
    int total = 0;
    ...
    if(samples > 0)
    {
        ...
        /*
        qWarning("sid_instrument: %d samples missing
        (tdelta=%d)",samples,tdelta); result = sid->clock(tdelta, ptr,
        samples); total += result; ptr += result; samples -= result; tdelta =
        0; qWarning("              : %d samples missing
        (tdelta=%d)",samples,tdelta);
        */
    }
...
void sidInstrument::playNote(NotePlayHandle* _n, sampleFrame* _working_buffer)
{
    ...
    for(reg8 i = 0; i < 3; ++i)
    {
        ...
        freq   = 440. * exp2((note - 69.) / 12.);
        data16 = int(freq / real_t(clockrate) * 16777216.);
...
{
  public:
    sidKnob(QWidget* _parent) : Knob(knobStyled, _parent)
    {
        setFixedSize(16, 16);
        setCenterPointX(7.5);
        setCenterPointY(7.5);
      ...
      InstrumentView(_instrument, _parent)
{
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        ak->setWhatsThis(tr("Attack rate determines how rapidly the output "
                            "of Voice %1 rises from zero to peak amplitude.")
                                 .arg(i + 1));
        ...
        dk->setWhatsThis(
                tr("Decay rate determines how rapidly the output "
                   "falls from the peak amplitude to the selected Sustain "
                   "level."));
        ...
        sk->setWhatsThis(tr("Output of Voice %1 will remain at the selected "
                            "Sustain amplitude as long as the note is held.")
                                 .arg(i + 1));
        ...
        rk->setWhatsThis(tr("The output of of Voice %1 will fall from "
                            "Sustain amplitude to zero amplitude at the "
                            "selected Release "
                            "rate.")
                                 .arg(i + 1));
        ...
        pwk->setWhatsThis(tr("The Pulse Width resolution allows the width "
                             "to be smoothly swept with no discernable "
                             "stepping. The Pulse "
                             "waveform on Oscillator %1 must be selected to "
                             "have any audible"
                             " effect.")
                                  .arg(i + 1));
        ...
        sync_btn->setWhatsThis(
                tr("Sync synchronizes the fundamental "
                   "frequency of Oscillator %1 with the fundamental "
                   "frequency of "
                   "Oscillator %2 producing \"Hard Sync\" effects.")
                        .arg(i + 1)
                        .arg(syncRingWidth[i]));
        ...
        ringMod_btn->setWhatsThis(tr("Ring-mod replaces the Triangle "
                                     "Waveform output of Oscillator %1 with "
                                     "a \"Ring Modulated\" "
                                     "combination of Oscillators %1 and %2.")
                                          .arg(i + 1)
                                          .arg(syncRingWidth[i]));
        ...
        filter_btn->setWhatsThis(tr("When Filtered is on, Voice %1 will be "
                                    "processed through the Filter. When "
                                    "Filtered is off, Voice %1 "
                                    "appears directly at the output, and the "
                                    "Filter has no effect on "
                                    "it.")
                                         .arg(i + 1));
        ...
        test_btn->setWhatsThis(
                tr("Test, when set, resets and locks "
                   "Oscillator %1 at zero until Test is turned off.")
                        .arg(i + 1));

        m_voiceKnobs[i]
                = voiceKnobs(ak, dk, sk, rk, pwk, crsk, wfbg, sync_btn,
                             ringMod_btn, filter_btn, test_btn);
    }
...
void sidInstrumentView::updateKnobHint()
{
    ...
    for(int i = 0; i < 3; ++i)
    {
        m_voiceKnobs[i].m_attKnob->setHintText(
                tr("Attack:") + " ",
                " ("
                        + QString::fromLatin1(
                                attackTime[(int)k->m_voice[i]
                                                   ->m_attackModel.value()])
                        + ")");
        ...
        m_voiceKnobs[i].m_pwKnob->setHintText(
                tr("Pulse Width:") + " ",
                " ("
                        + QString::number((double)k->m_voice[i]
                                                  ->m_pulseWidthModel.value()
                                          / 40.95)
                        + "%)");
        ToolTip::add(m_voiceKnobs[i].m_pwKnob,
                     QString::number(
                             (double)k->m_voice[i]->m_pulseWidthModel.value()
                             / 40.95)
                             + "%");
    }
    m_cutKnob->setHintText(
            tr("Cutoff frequency:") + " ",
            " ("
                    + QString::number(
                            (int)(9970. / 2047.
                                          * (double)k->m_filterFCModel.value()
                                  + 30.))
                    + "Hz)");
    ToolTip::add(
            m_cutKnob,
            QString::number(
                    (int)(9970. / 2047. * (double)k->m_filterFCModel.value()
                          + 30.))
                    + "Hz");
}
...
void sidInstrumentView::modelChanged()
{
    ...
    for(int i = 0; i < 3; ++i)
    {
        connect(&k->m_voice[i]->m_attackModel, SIGNAL(dataChanged()), this,
                SLOT(updateKnobHint()));
        connect(&k->m_voice[i]->m_decayModel, SIGNAL(dataChanged()), this,
                SLOT(updateKnobHint()));
        connect(&k->m_voice[i]->m_releaseModel, SIGNAL(dataChanged()), this,
                SLOT(updateKnobHint()));
        connect(&k->m_voice[i]->m_pulseWidthModel, SIGNAL(dataChanged()),
                this, SLOT(updateKnobHint()));
        connect(&k->m_voice[i]->m_sustainModel, SIGNAL(dataChanged()), this,
                SLOT(updateKnobToolTip()));
        connect(&k->m_voice[i]->m_coarseModel, SIGNAL(dataChanged()), this,
                SLOT(updateKnobToolTip()));
    }

    connect(&k->m_volumeModel, SIGNAL(dataChanged()), this,
            SLOT(updateKnobToolTip()));
    connect(&k->m_filterResonanceModel, SIGNAL(dataChanged()), this,
            SLOT(updateKnobToolTip()));
    connect(&k->m_filterFCModel, SIGNAL(dataChanged()), this,
            SLOT(updateKnobHint()));
...
extern "C"
{
    ...
    {
        return (new sidInstrument(static_cast<InstrumentTrack*>(_data)));
    }
}
