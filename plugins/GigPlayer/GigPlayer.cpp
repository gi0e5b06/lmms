...
GigInstrument::GigInstrument(InstrumentTrack* _instrument_track) :
      ...
      m_instance(NULL), m_instrument(NULL), m_filename(""),
      m_bankNum(0, 0, 999, this, tr("Bank")),
      m_patchNum(0, 0, 127, this, tr("Patch")),
      m_gain(1.0f, 0.0f, 5.0f, 0.01f, this, tr("Gain")),
      m_interpolation(SRC_LINEAR), m_RandomSeed(0), m_currentKeyDimension(0)
{
...
void GigInstrument::openFile(const QString& _gigFile, bool updateTrackName)
{
...

    {
        ...
        try
        {
            m_instance = new GigInstance(
                    SampleBuffer::tryToMakeAbsolute(_gigFile));
...
void GigInstrument::playNote(NotePlayHandle* _n, sampleFrame*)
{
    ...
    int midiNote = (int)floor(12.0 * (log2(_n->unpitchedFrequency()) - LOG440)
                              - 4.0);
    ...
    if(tfp == 0)
    {
        ...
        m_notes.push_back(GigNote(midiNote, velocity,
                                  _n->unpitchedFrequency(), pluginData));
    }
}
...
void GigInstrument::play(sampleFrame* _working_buffer)
{
    ...
    std::memset(&_working_buffer[0][0], 0,
                DEFAULT_CHANNELS * frames * sizeof(sample_t));
    ...
    for(QList<GigNote>::iterator it = m_notes.begin(); it != m_notes.end();
        ++it)
    {
        ...
        // Delete ended notes (either in the completed state or all the
        // samples ended)
        ...
        ++it)
    {
            ...
            sample != it->samples.end(); ++sample)
        {
            ...
            // solely for this one note (e.g. one or two samples per octave)
            // or we are processing at a different sample rate
            if(sample->region->PitchTrack == true
               || rate != sample->sample->SamplesPerSecond)
            {
                resample = true;
            ...
            if(resample == true)
            {
                ...
                // Only output if resampling is successful (note that "used"
                // is output)
                if(sample->convertSampleRate(*sampleData, *convertBuf,
...
void GigInstrument::loadSample(GigSample&   sample,
                               ...
                               f_cnt_t      samples)
{
    ...
    if(sample.region->pSampleLoops != NULL)
    {
        ...
        {
            loop     = true;
            loopType = static_cast<gig::loop_type_t>(
                    sample.region->pSampleLoops[i].LoopType);
    ...
    if(loop == true
       && (sample.pos >= loopStart || sample.pos + samples > loopStart))
    {
        // Calculate the new position based on the type of loop
        if(loopType == gig::loop_type_bidirectional)
        {
            sample.pos = getPingPongIndex(sample.pos, loopStart,
                                          loopStart + loopLength);
        }
        else
        {
            sample.pos = getLoopedIndex(sample.pos, loopStart,
                                        loopStart + loopLength);
        ...
        // Load the samples (based on gig::Sample::ReadAndLoop) even around
        // the end of a loop boundary wrapping to the beginning of the loop
        // region
    ...
    else
    {
        ...
        unsigned long size = sample.sample->Read(&buffer[0], samples)
    ...
    if(sample.sample->BitDepth == 24)  // 24 bit
    {
        ...
        for(f_cnt_t i = 0; i < samples; ++i)
        {
            ...
            sampleData[i][0]
                    = 1. / 0x100000000 * sample.attenuation * valueLeft;
            ...
            else
            {
                ...
                sampleData[i][1]
                        = 1. / 0x100000000 * sample.attenuation * valueRight;
            }
        }
    }
    else  // 16 bit
        ...
        for(f_cnt_t i = 0; i < samples; ++i)
        {
            ...
            else
            {
                sampleData[i][1] = 1. / 0x10000
                                   * pInt[sample.sample->Channels * i + 1]
                                   * sample.attenuation;
            }
        }
    }
...
f_cnt_t GigInstrument::getLoopedIndex(f_cnt_t index,
                                      f_cnt_t startf,
                                      f_cnt_t endf) const
{
    if(index < endf)
...
f_cnt_t GigInstrument::getPingPongIndex(f_cnt_t index,
                                        f_cnt_t startf,
                                        f_cnt_t endf) const
{
    if(index < endf)
    ...
    return (looppos < looplen) ? endf - looppos
...
void GigInstrument::deleteNotePluginData(NotePlayHandle* _n)
{
    ...
    // TODO: not sample exact? What about in the middle of us writing out the
    // sample?
...
void GigInstrument::addSamples(GigNote& gignote, bool wantReleaseSample)
{
    ...
    {
        Dimension dim
                = getDimensions(pRegion, gignote.velocity, wantReleaseSample);
        gig::DimensionRegion* pDimRegion
                = pRegion->GetDimensionRegionByValue(dim.DimValues);
        ...
        if(pSample != NULL && pSample->SamplesTotal != 0)
        {
            ...
            {
                real_t attenuation = pDimRegion->GetVelocityAttenuation(
                        gignote.velocity);
                ...
                gignote.samples.push_back(
                        GigSample(pSample, pDimRegion, attenuation,
                                  m_interpolation, gignote.frequency));
            }
        }
...
Dimension GigInstrument::getDimensions(gig::Region* pRegion,
                                       int          velocity,
                                       bool         release)
{
    Dimension dim;
    ...
    for(int i = pRegion->Dimensions - 1; i >= 0; --i)
    {
        switch(pRegion->pDimensionDefinitions[i].dimension)
        {
            ...
            case gig::dimension_keyboard:
                dim.DimValues[i]
                        = (uint)(m_currentKeyDimension
                                 * pRegion->pDimensionDefinitions[i].zones);
                break;
            ...
            case gig::dimension_random:
                ...
                dim.DimValues[i]
                        = uint(m_RandomSeed / 4294967296.0f
                               * pRegion->pDimensionDefinitions[i].bits);
                break;
...
// Get the selected instrument from the GIG file we opened if we haven't
// gotten it already. This is based on the bank and patch numbers.
...
{
  public:
    gigKnob(QWidget* _parent) : Knob(knobBright_26, _parent)
    {
        setFixedSize(31, 38);
    }
};

GigInstrumentView::GigInstrumentView(Instrument* _instrument,
                                     QWidget*    _parent) :
      InstrumentView(_instrument, _parent)
{
    GigInstrument* k = castModel<GigInstrument>();

    connect(&k->m_bankNum, SIGNAL(dataChanged()), this,
            SLOT(updatePatchName()));
    connect(&k->m_patchNum, SIGNAL(dataChanged()), this,
            SLOT(updatePatchName()));

    // File Button
    m_fileDialogButton = new PixmapButton(this);
    m_fileDialogButton->setCursor(QCursor(Qt::PointingHandCursor));
    m_fileDialogButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("fileselect_on"));
    m_fileDialogButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("fileselect_off"));
    m_fileDialogButton->move(223, 68);

    connect(m_fileDialogButton, SIGNAL(clicked()), this,
            SLOT(showFileDialog()));

    ToolTip::add(m_fileDialogButton, tr("Open other GIG file"));

    m_fileDialogButton->setWhatsThis(
            tr("Click here to open another GIG file"));

    // Patch Button
    m_patchDialogButton = new PixmapButton(this);
    m_patchDialogButton->setCursor(QCursor(Qt::PointingHandCursor));
    m_patchDialogButton->setActiveGraphic(
            PLUGIN_NAME::getIconPixmap("patches_on"));
    m_patchDialogButton->setInactiveGraphic(
            PLUGIN_NAME::getIconPixmap("patches_off"));
    m_patchDialogButton->setEnabled(false);
    m_patchDialogButton->move(223, 94);

    connect(m_patchDialogButton, SIGNAL(clicked()), this,
            SLOT(showPatchDialog()));

    ToolTip::add(m_patchDialogButton, tr("Choose the patch"));

    m_patchDialogButton->setWhatsThis(
            tr("Click here to change which patch of the GIG file to use"));
    ...
    m_bankNumLcd->setWhatsThis(
            tr("Change which instrument of the GIG file is being played"));
    m_patchNumLcd->setWhatsThis(
            tr("Change which instrument of the GIG file is being played"));
    ...
    m_filenameLabel->setWhatsThis(
            tr("Which GIG file is currently being used"));
    m_patchLabel->setWhatsThis(
            tr("Which patch of the GIG file is currently being used"));
...
void GigInstrumentView::updateFilename()
{
    ...
    m_filenameLabel->setText(
            fm.elidedText(file, Qt::ElideLeft, m_filenameLabel->width()));
...
void GigInstrumentView::updatePatchName()
{
    ...
    m_patchLabel->setText(
            fm.elidedText(patch, Qt::ElideLeft, m_patchLabel->width()));
...
void GigInstrumentView::showPatchDialog()
{
    ...
    pd.setup(k->m_instance, 1, k->instrumentTrack()->name(), &k->m_bankNum,
             &k->m_patchNum, m_patchLabel);
    pd.exec();
}

// Store information related to playing a sample from the GIG file
GigSample::GigSample(gig::Sample*          pSample,
                     gig::DimensionRegion* pDimRegion,
                     real_t                attenuation,
                     int                   interpolation,
                     frequency_t           desiredFreq) :
      sample(pSample),
      region(pDimRegion), attenuation(attenuation), pos(0),
      interpolation(interpolation), srcState(NULL), sampleFreq(0),
      freqFactor(1)
{
...
bool GigSample::convertSampleRate(sampleFrame& oldBuf,
                                  sampleFrame& newBuf,
                                  f_cnt_t      oldSize,
                                  f_cnt_t      newSize,
                                  double       freq_factor,
...
#ifdef REAL_IS_FLOAT
{
    ...
    if( error != 0 )
    {
            qCritical( "GigInstrument: error while resampling: %s",
src_strerror( error ) ); return false;
    }

    if( oldSize != 0 && src_data.output_frames_gen == 0 )
    {
            qCritical( "GigInstrument: could not resample, no frames
generated" ); return false;
    ...
    */
    {
        f_cnt_t generated = 0;
        return SampleRate::resample(&oldBuf, &newBuf, oldSize, newSize,
                                    freq_factor, 10, used, generated,
                                    srcState);
    }
    //#endif
...
ADSR::ADSR() :
      ...
      sustain(0), release(0), amplitude(0), isAttack(true), isRelease(false),
      isDone(false), attackPosition(0), attackLength(0), decayLength(0),
      ...
      releasePosition(0), releaseLength(0)
{
    if(region != NULL)
    {
        ...
        preattack = 1.0 * region->EG1PreAttack
                    / 1000;  // EG1PreAttack is 0-1000 permille
        attack          = region->EG1Attack;
        decay1          = region->EG1Decay1;
        decay2          = region->EG1Decay2;
        infiniteSustain = region->EG1InfiniteSustain;
        sustain         = 1.0 * region->EG1Sustain
                  / 1000;  // EG1Sustain is 0-1000 permille
...
real_t ADSR::value()
{
    ...
    if(isAttack == true)
    {
        ...
        {
            amplitude = preattack
                        + (1.0 - preattack) / attackLength * attackPosition;
        }
        else if(attackPosition < attackLength + decayLength)
        {
            amplitude = 1.0
                        - (1.0 - sustain) / decayLength
                                  * (attackPosition - attackLength);
        }
        else
        {
            isAttack = false;
    ...
    {
        // Maybe not the best way of doing this, but it appears to be about
        // right Satisfies f(0) = sustain and f(releaseLength) = very small
        amplitude = (sustain + 1e-3)
                            * expf(-5.0 / releaseLength * releasePosition)
                    - 1e-3;
