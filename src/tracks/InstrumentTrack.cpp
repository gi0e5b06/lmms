...
InstrumentTrack::InstrumentTrack(TrackContainer* tc) :
      ...
      m_audioPort(tr("Instrument track"),
                  true,
                  &m_volumeEnabledModel,
                  &m_volumeModel,
                  &m_panningEnabledModel,
                  &m_panningModel,
                  &m_bendingEnabledModel,
                  &m_bendingModel,
                  &m_mutedModel,
                  &m_frozenModel,
                  &m_clippingModel),
...
{
    int mp = m_useMasterPitchModel.value() ? Engine::getSong()->masterPitch()
                                           : 0;
...
void InstrumentTrack::processOutEvent(const MidiEvent& event,
                                      ...
                                      f_cnt_t          offset)
{
    // do nothing if we do not have an instrument instance (e.g. when loading
    // settings)
        ...
        for(tcoVector::Iterator it = tcos.begin(); it != tcos.end(); ++it)
        {
            ...
            while(nit != notes.end() && (cur_note = *nit)->pos() == cur_start)
            {
                ...
                if(_tco_num < 0)
                {
                    ...
                    notePlayHandle->setSongGlobalParentOffset(
                            p->startPosition());
                }
...
Instrument* InstrumentTrack::loadInstrument(const QString& _plugin_name)
{
    ...
    const bool nmb = !m_instrument->isMidiBased();
      ...
      m_window(nullptr), m_lastPos(-1, -1)
{
    ...
    if(!Engine::mixer()->midiClient()->isRaw())
    {
        ...
        _it->m_midiPort.m_writablePortsMenu
                = new MidiPortMenu(MidiPort::Output);
        _it->m_midiPort.m_readablePortsMenu->setModel(&_it->m_midiPort);
        _it->m_midiPort.m_writablePortsMenu->setModel(&_it->m_midiPort);
        m_midiInputAction
                = m_midiMenu->addMenu(_it->m_midiPort.m_readablePortsMenu);
        m_midiOutputAction
                = m_midiMenu->addMenu(_it->m_midiPort.m_writablePortsMenu);
    }
    else
    {
...
QMenu* InstrumentTrackView::createMidiInputMenu()
{
    ...
    if(!Engine::mixer()->midiClient()->isRaw())
    {
        it->m_midiPort.m_readablePortsMenu
                = new MidiPortMenu(MidiPort::Input);
...
QMenu* InstrumentTrackView::createMidiOutputMenu()
{
    ...
    if(!Engine::mixer()->midiClient()->isRaw())
    {
        it->m_midiPort.m_writablePortsMenu
                = new MidiPortMenu(MidiPort::Output);
...
class fxLineLcdSpinBox : public LcdSpinBox
{
  ...
  protected:
    ...
    virtual void contextMenuEvent(QContextMenuEvent* event)
    {
        ...
        // This condition is here just as a safety check, fxLineLcdSpinBox is
        // aways created inside a TabWidget inside an InstrumentTrackWindow
      ...
      m_itv(_itv), m_instrumentView(nullptr)
{
    ...
    basicControlsLayout->addWidget(m_bendingKnob, 0, 3);
    basicControlsLayout->setAlignment(m_bendingKnob, widgetAlignment);
    ...
    basicControlsLayout->setAlignment(m_bendingRangeSpinBox, widgetAlignment);
    ...
    basicControlsLayout->setAlignment(saveSettingsBtn, widgetAlignment);
    ...
    &m_track->m_noteFiltering ); m_noteHumanizingView = new
    InstrumentFunctionNoteHumanizingView( &m_track->m_noteHumanizing );
    m_noteStackingView = new InstrumentFunctionNoteStackingView(
    &m_track->m_noteStacking ); m_arpeggioView = new
    InstrumentFunctionArpeggioView( &m_track->m_arpeggio );
    m_noteDuplicatesRemovingView = new
    InstrumentFunctionNoteDuplicatesRemovingView(
    &m_track->m_noteDuplicatesRemoving );
    ...
    */
    ...
    // FX tab
    m_effectView
            = new EffectRackView(m_track->m_audioPort.effects(), m_tabWidget);
...
void InstrumentTrackWindow::dragEnterEventGeneric(QDragEnterEvent* event)
{
    StringPairDrag::processDragEnterEvent(
            event, "instrument,presetfile,pluginpresetfile");
}
...
void InstrumentTrackWindow::viewInstrumentInDirection(int d)
{
    ...
    // etc) sometimes, the next InstrumentTrackView may already be open, in
    // which case
    //   replace our window contents with the *next* closed Instrument Track
    ...
    {
        // save current window pos and then hide the window by unchecking its
        // button in the track list
        QPoint curPos = parentWidget()->pos();
        m_itv->m_tlb->setChecked(false);

        // enable the new window by checking its track list button & moving it
