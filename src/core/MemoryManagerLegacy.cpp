...
void * MemoryManagerLegacy::alloc( size_t size , const char* file , long line )
{
	...
	qFatal( "MemoryManagerLegacy.cpp: Couldn't allocate memory: %d chunks asked", requiredChunks );
	return NULL;
}
...
void MemoryManagerLegacy::free( void * ptr , const char* file , long line )
{
	...
	{
		qFatal( "MemoryManagerLegacy: Couldn't find pointer info for pointer: %p", ptr );
	}
...
void MemoryPool::releaseChunks( void * ptr, int chunks )
{
	...
	if( start < 0 )
	{
		qFatal( "MemoryManagerLegacy: error at releaseChunks() - corrupt pointer info?" );
	}
