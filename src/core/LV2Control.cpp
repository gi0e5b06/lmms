    ...
    switch(m_port->data_type)
    {
        ...
        case INTEGER:
            m_knobModel.setRange(
                    static_cast<int>(m_port->max.f),
                    static_cast<int>(m_port->min.f),
                    1
                            + static_cast<int>(m_port->max.f - m_port->min.f)
                                      / 400);
            m_knobModel.setInitValue(static_cast<int>(m_port->def.f));
...
void LV2Control::setValue(LV2_Data _value)
{
    switch(m_port->data_type)
    {
        ...
        case FLOATING:
            ...
            break;
        case TIME:
            m_tempoSyncKnobModel.setValue(static_cast<float>(_value.f));
            break;
        default:
