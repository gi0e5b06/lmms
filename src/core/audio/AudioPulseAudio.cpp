...
AudioPulseAudio::AudioPulseAudio(bool& _success_ful, Mixer* _mixer) :
      AudioDevice(tLimit<ch_cnt_t>(ConfigManager::inst()
                                           ->value("audiopa", "channels")
                                           .toInt(),
                                   DEFAULT_CHANNELS,
                                   SURROUND_CHANNELS),
                  _mixer),
      m_s(nullptr), m_quit(false), m_convertEndian(false)
{
    setObjectName("pulseaudioAudio");
...
static void context_state_callback(pa_context* c, void* userdata)
{
    ...
    switch(pa_context_get_state(c))
    {
        ...
        case PA_CONTEXT_READY:
        {
            ...
            pa_stream_set_state_callback(_this->m_s, stream_state_callback,
                                         _this);
            pa_stream_set_write_callback(_this->m_s, stream_write_callback,
                                         _this);
...
void AudioPulseAudio::streamWriteCallback(pa_stream* s, size_t length)
{
    const fpp_t          fpp    = mixer()->framesPerPeriod();
    surroundSampleFrame* temp   = new surroundSampleFrame[fpp];
    sampleS16_t*         pcmbuf = (sampleS16_t*)pa_xmalloc(fpp * channels()
                                                   * sizeof(sampleS16_t));
    ...
    while(fd < length / 4 && m_quit == false)
    {
        ...
        if(bytes > 0)
        {
            pa_stream_write(m_s, pcmbuf, bytes, nullptr, 0, PA_SEEK_RELATIVE);
        }
        fd += frames;
    }
...
void AudioPulseAudio::setupWidget::saveSettings()
{
    ...
    ConfigManager::inst()->setValue(
            "audiopa", "channels", QString::number(m_channels->value<int>()));
}
