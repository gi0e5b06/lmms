...
AudioAlsaGdx::AudioAlsaGdx( bool & _success_ful, Mixer*  _mixer ) :
	AudioDevice( tLimit<ch_cnt_t>
                     (ConfigManager::inst()->value("audioalsa","channels").
                      toInt(),
	...
	m_convertEndian( false )
{
	...
	{
		qCritical("AudioAlsaGdx: could not avoid possible "
                          "interception by PulseAudio");
	}

	int err;

	if( ( err = snd_pcm_open( &m_outHandle,
                                  probeDevice().toLatin1().constData(),
                                  SND_PCM_STREAM_PLAYBACK,
                                  0 ) ) < 0 )
        {
		qCritical("AudioAlsaGdx: playback open error: %s",
                          snd_strerror( err ) );
		return;
	}
...
int AudioAlsaGdx::handleError( int _err )
{
	if( _err == -EPIPE )
	{
		...
		if( _err < 0 )
			qCritical("AudioAlsaGdx: can not recover from underrun,"
                                  " prepare failed: %s",
                                  snd_strerror( _err ) );
		return 0;
	}
	...
	else if( _err == -ESTRPIPE )
	{
		...
		if( _err < 0 )
		{
			_err = snd_pcm_prepare( m_outHandle );
			if( _err < 0 )
				qCritical("AudioAlsaGdx: can not recover from"
		...
		if( ( err = snd_pcm_open( &m_outHandle,
					probeDevice().toLatin1().constData(),
						SND_PCM_STREAM_PLAYBACK,
								0 ) ) < 0 )
		{
			qCritical("AudioAlsaGdx: open error: %s",
                                  snd_strerror( err ) );
			return;
		}
...
int AudioAlsaGdx::setHWParams( const ch_cnt_t _channels, snd_pcm_access_t _access )
{
	int err, dir;
	...
	if( ( err = snd_pcm_hw_params_set_access( m_outHandle, m_hwParams,
							_access ) ) < 0 )
	{
		qCritical("AudioAlsaGdx: "
	...
	if( ( snd_pcm_hw_params_set_format( m_outHandle, m_hwParams,
						SND_PCM_FORMAT_S16_LE ) ) < 0 )
	{
		...
		{
			qCritical("AudioAlsaGdx: "
                                  "neither little- nor big-endian available "
                                  "for playback: %s",
                                  snd_strerror( err ) );
			return err;
		}
	...
	if( ( err = snd_pcm_hw_params_set_channels( m_outHandle, m_hwParams,
							_channels ) ) < 0 )
	{
		qCritical("AudioAlsaGdx: "
...
int AudioAlsaGdx::setSWParams()
{
	...
	if( ( err = snd_pcm_sw_params_set_start_threshold( m_outHandle,
	...
	if( ( err = snd_pcm_sw_params_set_xfer_align( m_outHandle,
							m_swParams, 1 ) ) < 0 )
	{
		qCritical("AudioAlsaGdx: "
