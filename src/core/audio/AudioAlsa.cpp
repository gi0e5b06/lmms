...
AudioAlsa::AudioAlsa(bool& _success_ful, Mixer* _mixer) :
      AudioDevice(tLimit<ch_cnt_t>(ConfigManager::inst()
                                           ->value("audioalsa", "channels")
                                           .toInt(),
                                   DEFAULT_CHANNELS,
                                   SURROUND_CHANNELS),
                  _mixer),
      m_handle(NULL), m_hwParams(NULL), m_swParams(NULL),
      m_convertEndian(false)
{
    qWarning("AudioAlsa::AudioAlsa");

    setObjectName("audio_alsa");
    ...
    {
        qWarning(
                "AudioAlsa: Could not avoid possible interception by "
                "PulseAudio");
    }

    int err;

    if((err = snd_pcm_open(&m_handle, probeDevice().toLatin1().constData(),
    ...
    {
        qCritical("AudioAlsa: Setting of hwparams failed: %s",
                  snd_strerror(err));
        return;
    }
    if((err = setSWParams()) < 0)
    {
        qCritical("AudioAlsa: Setting of swparams failed: %s",
                  snd_strerror(err));
        return;
    }
...
void AudioAlsa::applyQualitySettings()
{
    if(hqAudio())
    {
        ...
        int err;
        if((err
            = snd_pcm_open(&m_handle, probeDevice().toLatin1().constData(),
                           SND_PCM_STREAM_PLAYBACK, 0))
           < 0)
        {
...
int AudioAlsa::setHWParams(const ch_cnt_t _channels, snd_pcm_access_t _access)
{
    ...
    if((err = snd_pcm_hw_params_set_access(m_handle, m_hwParams, _access))
       < 0)
    {
    ...
    if((err = snd_pcm_hw_params_set_channels(m_handle, m_hwParams, _channels))
       < 0)
    {
    ...
    // set the sample rate
    if((err
        = snd_pcm_hw_params_set_rate(m_handle, m_hwParams, sampleRate(), 0))
       < 0)
    {
...
int AudioAlsa::setSWParams()
{
    ...
    if((err = snd_pcm_sw_params_set_start_threshold(m_handle, m_swParams,
                                                    m_periodSize))
       < 0)
    {
    ...
    // processed
    if((err
        = snd_pcm_sw_params_set_avail_min(m_handle, m_swParams, m_periodSize))
       < 0)
    {
