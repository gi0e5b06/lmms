                  ...
                  _mixer)
{
    ...
    const QString& configBackend
            = ConfigManager::inst()->value("audiosoundio", "backend");
    const QString& configDeviceId
            = ConfigManager::inst()->value("audiosoundio", "out_device_id");
    const QString& configDeviceRaw
            = ConfigManager::inst()->value("audiosoundio", "out_device_raw");
    ...
    int selected_device_index
            = soundio_default_output_device_index(m_soundio);
    ...
    for(int i = 0; i < outDeviceCount; i += 1)
    {
        ...
        bool           isThisOne
                = (configDeviceId == device->id && wantRaw == device->is_raw);
    ...
    SoundIoDevice* device
            = soundio_get_output_device(m_soundio, selected_device_index);
    ...
    for(int i = 0; i < device->sample_rate_count; i += 1)
    {
        ...
        if(closestSupportedSampleRate == -1
           || abs(range->max - currentSampleRate)
                      < abs(closestSupportedSampleRate - currentSampleRate))
        {
    ...
    m_outstream->software_latency
            = (double)mixer()->framesPerPeriod() / (double)currentSampleRate;
...
void AudioSoundIo::writeCallback(int frameCountMin, int frameCountMax)
{
    ...
    while(framesLeft > 0)
    {
        ...
        if((err = soundio_outstream_begin_write(m_outstream, &areas,
                                                &frameCount)))
        {
            errorCallback(err);
            return;
...
static void setupWidgetOnBackendDisconnect(SoundIo* soundio, int err)
{
    AudioSoundIo::setupWidget* setupWidget
            = (AudioSoundIo::setupWidget*)soundio->userdata;
...
AudioSoundIo::setupWidget::setupWidget(QWidget* _parent) :
      ...
      m_backendModel(nullptr, "SoundIo backed"),
      m_deviceModel(nullptr, "SoundIo device")

{
    ...
    bool ok = connect(&m_backendModel, SIGNAL(dataChanged()), &m_setupUtil,
                      SLOT(reconnectSoundIo()));
    assert(ok);
...
AudioSoundIo::setupWidget::~setupWidget()
{
    bool ok = disconnect(&m_backendModel, SIGNAL(dataChanged()), &m_setupUtil,
                         SLOT(reconnectSoundIo()));
    assert(ok);
    if(m_soundio)
    {
...
void AudioSoundIo::setupWidget::saveSettings()
{
    ...
    ConfigManager::inst()->setValue("audiosoundio", "backend",
                                    m_backendModel.currentText());
    ConfigManager::inst()->setValue("audiosoundio", "out_device_id",
                                    deviceId->id);
    ConfigManager::inst()->setValue("audiosoundio", "out_device_raw",
                                    configDeviceRaw);
}
