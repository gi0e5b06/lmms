...
AudioSdl::AudioSdl(bool& _success_ful, Mixer* _mixer) :
      ...
      m_convertedBufPos(0), m_convertEndian(false)
{
    setObjectName("sdlAudio");

    _success_ful = false;

    m_convertedBufSize
            = mixer()->framesPerPeriod() * channels() * sizeof(sampleS16_t);
...
void AudioSdl::sdlAudioCallback(Uint8* _buf, int _len)
{
    ...
    while(_len)
    {
        if(m_convertedBufPos == 0)
        {
            ...
            m_convertedBufSize = frames * channels() * sizeof(sampleS16_t);

            convertToS16(m_outBuf, frames, (sampleS16_t*)m_convertedBuf,
                         m_convertEndian);
        }
        const int min_len
                = qMin(_len, m_convertedBufSize - m_convertedBufPos);
...
{
    ConfigManager::inst()->setValue("audiosdl", "device", m_device->text());
}
