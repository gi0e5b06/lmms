...
Oscillator::Oscillator(const IntModel*    _wave_shape_model,
      ...
      m_modulationAlgoModel(_mod_algo_model), m_freq(_freq),
      m_detuning(_detuning), m_volume(_volume),
      m_ext_phaseOffset(_phase_offset), m_subOsc(_sub_osc),
      m_phaseOffset(_phase_offset), m_phase(_phase_offset),
      m_userWave(nullptr)
{
}

void Oscillator::update(sampleFrame*   _ab,
                        const fpp_t    _frames,
                        const ch_cnt_t _chnl)
{
    if(m_freq >= Engine::mixer()->processingSampleRate() / 2)
    {
        memset(_ab, 0,
               sizeof(sampleFrame)
                       * _frames);  // BufferManager::clear( _ab, _frames );
        return;
    }
...
void Oscillator::updateNoSub(sampleFrame*   _ab,
                             const fpp_t    _frames,
                             const ch_cnt_t _chnl)
{
...
void Oscillator::updatePM(sampleFrame*   _ab,
                          const fpp_t    _frames,
                          const ch_cnt_t _chnl)
{
    switch(m_waveShapeModel->value())
...
void Oscillator::updateAM(sampleFrame*   _ab,
                          const fpp_t    _frames,
                          const ch_cnt_t _chnl)
{
    switch(m_waveShapeModel->value())
...
void Oscillator::updateMix(sampleFrame*   _ab,
                           const fpp_t    _frames,
                           const ch_cnt_t _chnl)
{
    switch(m_waveShapeModel->value())
...
void Oscillator::updateSync(sampleFrame*   _ab,
                            const fpp_t    _frames,
                            const ch_cnt_t _chnl)
{
    switch(m_waveShapeModel->value())
...
void Oscillator::updateFM(sampleFrame*   _ab,
                          const fpp_t    _frames,
                          const ch_cnt_t _chnl)
{
    switch(m_waveShapeModel->value())
...
inline void Oscillator::recalcPhase()
{
    ...
    m_phase = positivefraction(m_phase) + 2.;
}
...
real_t Oscillator::syncInit(sampleFrame*   _ab,
                            const fpp_t    _frames,
                            const ch_cnt_t _chnl)
{
    if(m_subOsc != nullptr)
                          ...
                          const ch_cnt_t _chnl)
{
    ...
    recalcPhase();
    const real_t osc_coeff = m_freq * m_detuning;
    const real_t sampleRateCorrection
...
template <>
inline sample_t
        Oscillator::getSample<Oscillator::SineWave>(const real_t _sample)
{
...
template <>
inline sample_t
        Oscillator::getSample<Oscillator::TriangleWave>(const real_t _sample)
{
...
template <>
inline sample_t
        Oscillator::getSample<Oscillator::SawWave>(const real_t _sample)
{
...
template <>
inline sample_t
        Oscillator::getSample<Oscillator::SquareWave>(const real_t _sample)
{
...
template <>
inline sample_t
        Oscillator::getSample<Oscillator::MoogSawWave>(const real_t _sample)
{
...
template <>
inline sample_t
        Oscillator::getSample<Oscillator::WhiteNoise>(const real_t _sample)
{
