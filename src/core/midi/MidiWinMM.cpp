...
void MidiWinMM::processOutEvent(const MidiEvent& event,
                                const MidiTime&  time,
                                const MidiPort*  port)
{
...
QString MidiWinMM::sourcePortName(const MidiEvent& event) const
{
    if(event.sourcePort())
    {
        return m_inputDevices.value(
                *static_cast<const HMIDIIN*>(event.sourcePort()));
    }

    return MidiClient::sourcePortName(event);
}

void MidiWinMM::subscribeReadablePort(MidiPort*      port,
                                      const QString& dest,
                                      bool           subscribe)
{
...
void MidiWinMM::subscribeWritablePort(MidiPort*      port,
                                      const QString& dest,
                                      bool           subscribe)
{
...
void WINAPI CALLBACK MidiWinMM::inputCallback(HMIDIIN   hm,
                                              UINT      msg,
                                              DWORD_PTR inst,
                                              DWORD_PTR param1,
                                              DWORD_PTR param2)
    ...
    for(MidiPorts::ConstIterator it = l.begin(); it != l.end(); ++it)
    {
        switch(cmdtype)
        {
            ...
            case MidiKeyPressure:
                (*it)->processInEvent(MidiEvent(cmdtype, chan,
                                                par1,  // - KeysPerOctave,
                                                par2 & 0xff, &hm));
                break;
            ...
            case MidiChannelPressure:
                (*it)->processInEvent(
                        MidiEvent(cmdtype, chan, par1, par2 & 0xff, &hm));
                break;

            case MidiPitchBend:
                (*it)->processInEvent(
                        MidiEvent(cmdtype, chan, par1 + par2 * 128, 0, &hm));
                break;
