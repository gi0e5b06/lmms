...
MidiAlsaSeq::MidiAlsaSeq() :
      MidiClient(), m_seqMutex(), m_seqHandle(NULL), m_mtcPort(nullptr),
      m_queueID(-1), m_quit(false), m_portListUpdateTimer(this)
{
    ...
    snd_seq_queue_tempo_set_tempo(tempo,
                                  6000000 / Engine::getSong()->getTempo());
    ...
    if(CONFIG_GET_BOOL("midi.mtc_enabled"))
        m_mtcPort = new MidiPort("MTC", this, nullptr, nullptr,
                                 MidiPort::Output);

    // initial list-update
    updatePortList();

    connect(&m_portListUpdateTimer, SIGNAL(timeout()), this,
            SLOT(updatePortList()));
...
MidiAlsaSeq::~MidiAlsaSeq()
{
    ...
    if(m_mtcPort != nullptr)
        DELETE_HELPER(m_mtcPort)
}
...
void MidiAlsaSeq::processOutEvent(const MidiEvent& event,
                                  const MidiTime&  time,
                                  const MidiPort*  port)
{
    ...
    snd_seq_event_t ev;
    snd_seq_ev_clear(&ev);
    snd_seq_ev_set_source(&ev, (m_portIDs[p][1] != -1) ? m_portIDs[p][1]
                                                       : m_portIDs[p][0]);
...
void MidiAlsaSeq::sendBytes(const uint8_t*  _bytes,
                            const int       _size,
                            ...
                            const MidiPort* _port)
{
    ...
    if(m_portIDs[p][1] != -1)
        ...
        num = m_portIDs[p][0];
    if(num == -1)
        qWarning("MidiAlsaSeq: num == -1");

    snd_seq_event_t ev;
    snd_seq_ev_clear(&ev);
    snd_seq_ev_set_source(&ev, num);
    snd_seq_ev_set_subs(&ev);
    snd_seq_ev_schedule_tick(&ev, m_queueID, 1, static_cast<int>(_time));
    ev.queue = m_queueID;

    snd_midi_event_t* dev;
    if(snd_midi_event_new(_size, &dev) != 0)
        qInfo("MidiAlsaSeq: error snd_midi_event_new");
    snd_midi_event_init(dev);
    if(snd_midi_event_encode(dev, _bytes, _size, &ev) != _size)
        qInfo("MidiAlsaSeq: error snd_midi_event_encode");
    if(ev.type == SND_SEQ_EVENT_NONE)
        qWarning("MidiAlsaSeq: unrecognized byte seq");
    snd_midi_event_free(dev);

    m_seqMutex.lock();
    if(snd_seq_event_output(m_seqHandle, &ev) < 0)
        qWarning("MidiAlsaSeq: error sending %d bytes on %s", _size,
                 qPrintable(_port->displayName()));
    snd_seq_drain_output(m_seqHandle);
    m_seqMutex.unlock();
}
...
void MidiAlsaSeq::applyPortMode(MidiPort* _port)
{
    ...
    for(int i = 0; i < 2; ++i)
    {
        if(caps[i] != 0)
        {
            ...
            {
                // then create one;
                m_portIDs[_port][i] = snd_seq_create_simple_port(
                        m_seqHandle,
                        _port->displayName().toUtf8().constData(), caps[i],
...
void MidiAlsaSeq::applyPortName(MidiPort* _port)
{
    ...
    for(int i = 0; i < 2; ++i)
    {
        ...
        snd_seq_port_info_t* port_info;
        snd_seq_port_info_malloc(&port_info);
        snd_seq_get_port_info(m_seqHandle, m_portIDs[_port][i], port_info);
        snd_seq_port_info_set_name(port_info,
                                   _port->displayName().toUtf8().constData());
        snd_seq_set_port_info(m_seqHandle, m_portIDs[_port][i], port_info);
        snd_seq_port_info_free(port_info);
    }
