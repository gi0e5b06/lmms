				   ...
				   snd_seq_port_info_t * _pinfo ) const
{
	...
	if(cid==m_clientID) cid=128;
	if(cnm==m_clientName) cnm="LMMS";
...
QString MidiAlsaGdx::alsaPortName(const snd_seq_addr_t& _addr ) const // snd_seq_t * _seq, 
{
	...
	m_portListUpdateTimer( this )
{
	...
	snd_seq_queue_tempo_set_tempo( tempo, 60000000 / Engine::getSong()->getTempo() );
	...
	do
	{
		if(n==1) m_clientName="LMMS";
		else     m_clientName=QString("LMMS%1").arg(n);
		n++;
...
void MidiAlsaGdx::setFD(const MidiPort* _port,int _i,int _v)
{
	...
	{
	case 1:
		if(_v==-1) m_FD1.remove(_port);
		else m_FD1.insert(_port,_v);
		break;
	case 0:
		if(_v==-1) m_FD0.remove(_port);
		else m_FD0.insert(_port,_v);
		break;
	default:
...
void MidiAlsaGdx::createLmmsPorts()
{
	...
	{
		QString name=QString("LMMS %1").arg(TYPICAL_CLIENT_NAME[_num]);//.section(':',0,0));
		s_lmmsPorts[_num]=new MidiPort(name,this,this,NULL,MidiPort::Duplex);
	}
	...
	for(int _num=0;_num<16;_num++)
	{
		...
		QString name=QString("out to %1").arg(TYPICAL_CLIENT_NAME[_num]);//.section(':',0,0));
	...
	for(int _num=0;_num<16;_num++)
	{
		int caps=SND_SEQ_PORT_CAP_WRITE |
			SND_SEQ_PORT_CAP_SUBS_WRITE;
		QString name=QString("in from %1").arg(TYPICAL_CLIENT_NAME[_num]);//.section(':',0,0));
...
void MidiAlsaGdx::destroyLmmsPorts()
{
	for(int _num=0;_num<16;_num++)
	{
		...
		if((pout!=-1)&&(snd_seq_delete_simple_port( m_seqHandle, pout)<0))
...
void MidiAlsaGdx::alsaConnectOut(int _pout,QString _pextin)
{
	...
	if( snd_seq_parse_address(m_seqHandle,&dest,
				  s_alsaReadablePorts.value(_pextin).section(' ',0,0).toLatin1().constData()))
	{
		qWarning("MidiAlsaGdx: Error parsing dest-address %s",qPrintable(_pextin));
		return;
	}
...
void MidiAlsaGdx::alsaConnectIn(int _pin,QString _pextout)
{
	...
	if( snd_seq_parse_address(m_seqHandle,&sender,
				  s_alsaWritablePorts.value(_pextout).section(' ',0,0).toLatin1().constData()))
	{
		qWarning("MidiAlsaGdx: Error parsing sender-address %s",qPrintable(_pextout));
		return;
	}
...
void MidiAlsaGdx::alsaConnect(const snd_seq_addr_t& sender, const snd_seq_addr_t& dest)
{
	m_seqMutex.lock();
	...
	*/
	...
	for( int i = 0; i < 2; ++i )
	{
		...
		if( caps[i] != 0 )
		{
			...
			{
				//qInfo("create port '%s'",_port->displayName().toUtf8().constData());
				// then create one;
				//m_portIDs[_port][i] =
				int p=snd_seq_create_simple_port
					(m_seqHandle,
					 ...
					 SND_SEQ_PORT_TYPE_MIDI_GENERIC |
					 SND_SEQ_PORT_TYPE_APPLICATION );
				if(p<0) qFatal("Fail to create alsa port %d:%d",m_clientID,p);
				else qInfo("Alsa port created %d:%d",m_clientID,p);

				setFD(_port,i,p);
				qInfo("port created p=%d i=%d fd=%d",p,i,getFD(_port,i));
				continue;
			}
			qInfo("port exists already p=%d i=%d fd=%d",p,i,getFD(_port,i));
			   ...
			   (m_FD1.keys(p).size()==0))
				{
					if(snd_seq_delete_simple_port(m_seqHandle,p))
						qInfo("fail to delete alsa port %d",p);
					else
						qInfo("Alsa port %d:%d deleted",m_clientID,p);
				}
			SHOW_MIDI_MAP();
		}
	}
...
void MidiAlsaGdx::applyPortName( MidiPort * _port )
{
	...
	*/
	...
	for( int i = 0; i < 1; ++i ) //2
	{
		...
		snd_seq_port_info_malloc( &port_info );
		snd_seq_get_port_info( m_seqHandle, p, port_info );
		snd_seq_port_info_set_name( port_info, _port->displayName().toUtf8().constData() );
		snd_seq_set_port_info( m_seqHandle, p, port_info );
		snd_seq_port_info_free( port_info );
	}
	*/
					 ...
					 bool _subscribe )
{
	...
	{
		qWarning("MidiAlsaGdx::subscribeReadablePort port with invalid FD");
		return;
	}
	*/
	...
	*/
	...
	{
		qWarning("MidiAlsaGdx: Error parsing sender-address %s",qPrintable(_dest));
		m_seqMutex.unlock();
		return;
	}
	...
	if(_subscribe)
	{
		if(m_mapReadSubs.contains(sender))
		{
			...
			{
				qWarning("MidiAlsaGdx: sender already subscribed %u:%u",sender.client,sender.port);
			}
			else
			{
	...
	qInfo("MidiAlsaGdx::subscribeReadablePort dest %u:%u",dest->client,dest->port);
	snd_seq_port_subscribe_t * subs;
	snd_seq_port_subscribe_malloc( &subs );
	snd_seq_port_subscribe_set_sender( subs, &sender );
	snd_seq_port_subscribe_set_dest( subs, dest );
	if( _subscribe )
	{
		qInfo("MidiAlsaGdx::subscribeReadablePort subscribe physical %u:%u",sender.client,sender.port);
		if(snd_seq_subscribe_port( m_seqHandle, subs ))
	...
	{
		qInfo("MidiAlsaGdx::subscribeReadablePort unsubcribe physical %u:%u",sender.client,sender.port);
		if(snd_seq_unsubscribe_port( m_seqHandle, subs ))
			qWarning("failed to unsubscribe");
					 ...
					 bool _subscribe )
{
	...
	*/
	...
	if(getFD(_port,1)<0)
	{
		qWarning("MidiAlsaGdx::subscribeWritablePort port with invalid FD");
		return;
	}
	*/
	...
	if( snd_seq_parse_address( m_seqHandle, &dest,
			_dest.section( ' ', 0, 0 ).toLatin1().constData() ) )
	{
		qWarning("MidiAlsaGdx: Error parsing dest-address %s",qPrintable(_dest));
		m_seqMutex.unlock();
		return;
	}
	...
	*/
	...
	/*
	qInfo("MidiAlsaGdx::subscribeWritablePort connecting physical %u:%u",dest.client,dest.port);
	int p1=getFD(_port,1);
...
void MidiAlsaGdx::run()
{
	...
	static QQueue<snd_seq_event_t> s_evqueue;
	...
	while( m_quit == false )
	{
		...
		if(pollRet>0)
		{
			...
			while( ((nbe=snd_seq_event_input_pending( m_seqHandle, false ))>0)||
				...
				if( (res=snd_seq_event_input( m_seqHandle, &ev )) < 0 )
				{
					...
					if(res==-ENOSPC)
						qCritical("MidiAlsaGdx: The input FIFO overran, and some events are lost.");
					else
		...
		for(int j=0;j<ports.size();j++)
		{
			...
			//if(!dn.isNull()) qWarning("MidiAlsaGdx: dispatching event to %s",qPrintable(dn));
			...
			case SND_SEQ_EVENT_PORT_SUBSCRIBED:
				qWarning( "MidiAlsaSeq: seq event port subscribed %d", ev->type );
				break;

			case SND_SEQ_EVENT_PORT_UNSUBSCRIBED:
				qWarning( "MidiAlsaSeq: seq event port unsubscribed %d", ev->type );
				break;

			default:
				qWarning( "MidiAlsaSeq: unhandled input event %d", ev->type );
				break;
			} // end switch
		}
...
{
	m_seqMutex.lock();
	snd_seq_change_queue_tempo( m_seqHandle, m_queueID, 60000000 / (int) _bpm, NULL );
	snd_seq_drain_output( m_seqHandle );
	m_seqMutex.unlock();
}
