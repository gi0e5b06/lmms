...
MidiApple::MidiApple() :
      MidiClient(), m_inputDevices(), m_outputDevices(), m_inputSubs(),
      m_outputSubs()
{
    openDevices();
}
...
void MidiApple::processOutEvent(const MidiEvent& event,
                                const MidiTime&  time,
                                const MidiPort*  port)
{
    qDebug("MidiApple:processOutEvent displayName:'%s'",
    ...
    for(QMap<QString, MIDIEndpointRef>::Iterator it = m_outputDevices.begin();
...
void MidiApple::sendMidiOut(MIDIEndpointRef& endPointRef,
...
{
    qDebug("applyPortMode displayName:'%s'",
           port->displayName().toLatin1().constData());
...
{
    qDebug("removePort displayName:'%s'",
           port->displayName().toLatin1().constData());
    for(SubMap::Iterator it = m_inputSubs.begin(); it != m_inputSubs.end();
        ++it)
    {
        it.value().removeAll(port);
    }

    for(SubMap::Iterator it = m_outputSubs.begin(); it != m_outputSubs.end();
        ++it)
    {
        it.value().removeAll(port);
    }
...
void MidiApple::subscribeReadablePort(MidiPort*      port,
                                      const QString& dest,
                                      bool           subscribe)
{
    qDebug("subscribeReadablePort %s subscribe=%d",
           dest.toLatin1().constData(), subscribe);
...
void MidiApple::subscribeWritablePort(MidiPort*      port,
                                      const QString& dest,
                                      bool           subscribe)
{
    qDebug("subscribeWritablePort %s",
           port->displayName().toLatin1().constData());
...
void MidiApple::ReadCallback(const MIDIPacketList* pktlist,
                             void*                 readProcRefCon,
                             void*                 srcConnRefCon)
{
...
void MidiApple::HandleReadCallback(const MIDIPacketList* pktlist,
                                   void*                 srcConnRefCon)
{
    ...
    for(uint32_t i = 0; i < pktlist->numPackets; ++i)
    {
        ...
        if(continueSysEx)
        {
            unsigned int lengthToCopy
                    = qMin(nBytes, SYSEX_LENGTH - sysExLength);
            ...
            {
                // We would process the SysEx message here, as it is we're
                // just ignoring it
        ...
        else
        {
            ...
            while(iByte < nBytes)
            {
                ...
                {
                    // MIDI SysEx then we copy the rest of the message into
                    // the SysEx message buffer
                    unsigned int lengthLeftInMessage = nBytes - iByte;
                    unsigned int lengthToCopy
                            = qMin(lengthLeftInMessage, SYSEX_LENGTH);

                    memcpy(sysExMessage + sysExLength, packet->data,
                           lengthToCopy);
                    sysExLength += lengthToCopy;
                    ...
                    // Check whether the message at the end is the end of the
                    // SysEx
                    continueSysEx = (packet->data[nBytes - 1] != 0xF7);
                }
                else if(status < 0xF3)
                ...
                const MidiEventTypes cmdtype
                        = static_cast<MidiEventTypes>(status & 0xF0);
                ...
                switch(cmdtype)
                {
                    ...
                    case 0xF0:
                        break;
                    default:
                        qDebug("endPointRef name='%s':Some other message %d",
                               refName, cmdtype);
                        break;
                }
                iByte += size;
...
const void printQStringKeys(char const*                           mapName,
                            const QMap<QString, MIDIEndpointRef>& inputMap)
{
    qDebug("%s:", mapName);
...
void MidiApple::openDevices()
{
    ...
    for(ItemCount i = 0; i < deviceCount; ++i)
    {
        ...
        SInt32 isOffline = 0;
        MIDIObjectGetIntegerProperty(device, kMIDIPropertyOffline,
                                     &isOffline);
        ...
        for(ItemCount j = 0; j < entityCount; ++j)
        {
            ...
            for(ItemCount k = 0; k < sourceCount; ++k)
            {
                ...
                QString sourceName = qsDeviceName + ":"
                                     + QString::fromUtf8((char*)(name));
                qDebug("	Source name: '%s'",
                       sourceName.toLatin1().constData());
            ...
            for(ItemCount k = 0; k < destCount; ++k)
            {
                ...
                QString destinationName = qsDeviceName + ":"
                                          + QString::fromUtf8((char*)(name));
                qDebug("	Destination name: '%s'",
                       destinationName.toLatin1().constData());
...
void MidiApple::openMidiReference(MIDIEndpointRef reference,
                                  QString         refName,
                                  bool            isIn)
{
    ...
    CFStringRef inName = CFStringCreateWithCString(0, registeredName,
                                                   kCFStringEncodingASCII);
    if(isIn)
    {
        MIDIInputPortCreate(mClient, inName, &MidiApple::ReadCallback, this,
                            &mPort);
    }
    else
    {
...
char* MidiApple::getFullName(MIDIEndpointRef& endpoint_ref)
{
    ...
    char* fullName
            = (char*)malloc(strlen(deviceName) + strlen(endPointName) + 1);
