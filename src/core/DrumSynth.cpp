...
// Bah, I'll move these into the class once I sepearate DrumsynthFile from
// DrumSynth llama
...
float mem_t = 1.0f, mem_o = 1.0f, mem_n = 1.0f, mem_b = 1.0f, mem_tune = 1.0f,
      mem_time = 1.0f;
...
void DrumSynth::UpdateEnv(int e, long t)
{
    float endEnv, dT;
    // 0.2's added
    envData[e][NEXTT] = envpts[e][0][(long)(envData[e][PNT] + 1.f)]
                        * timestretch;  // get next point
    if(envData[e][NEXTT] < 0)
        envData[e][NEXTT] = 442000 * timestretch;  // if end point, hold
    envData[e][ENV] = envpts[e][1][(long)(envData[e][PNT] + 0.f)]
                      * 0.01f;  // this level
    endEnv = envpts[e][1][(long)(envData[e][PNT] + 1.f)]
             * 0.01f;  // next level
...
int DrumSynth::GetPrivateProfileString(const char* sec,
                                       const char* key,
                                       const char* def,
                                       char*       buffer,
                                       int         size,
...
int DrumSynth::GetPrivateProfileInt(const char* sec,
                                    const char* key,
                                    int         def,
                                    const char* file)
{
...
float DrumSynth::GetPrivateProfileFloat(const char* sec,
                                        const char* key,
                                        float       def,
                                        const char* file)
{
...
//  an associative array or something once we have a datastructure to load in
//  to. llama

int DrumSynth::GetDSFileSamples(const char*   dsfile,
                                int16_t*&     wave,
                                int           channels,
                                sample_rate_t Fs)
{
    ...
    float Ophi1, Ophi2, OF1, OF2, OL, Ot = 0 /*PG: init */, OBal1, OBal2,
                                      ODrive;
    ...
    if(ver[11] != '1' && ver[11] != '2')
        return 0;  // version fail

    // read master parameters
    GetPrivateProfileString(sec, "Comment", "", comment, sizeof(comment),
                            dsfile);
    ...
    timestretch = 0.01f * mem_time
                  * GetPrivateProfileFloat(sec, "Stretch", 100.0, dsfile);
    if(timestretch < 0.2f)
        timestretch = 0.2f;
    if(timestretch > 10.f)
        timestretch = 10.f;
    ...
    F1 = MasterTune * TwoPi * GetPrivateProfileFloat(sec, "F1", 200.0, dsfile)
         / Fs;
    if(fabs(F1) < 0.001f)
        F1 = 0.001f;  // to prevent overtone ratio div0
    F2 = MasterTune * TwoPi * GetPrivateProfileFloat(sec, "F2", 120.0, dsfile)
         / Fs;
    ...
    if(TDroopRate > 0.f)
    {
        ...
        TDroop     = 1;
        F2         = F1
             + ((F2 - F1) / (1.f - (float)exp(TDroopRate * envData[1][MAX])));
        ddF = F1 - F2;
    }
    else
        ddF = F2 - F1;

    Tphi = GetPrivateProfileFloat(sec, "Phase", 90.f, dsfile)
           / 57.29578f;  // degrees>radians
    ...
    OF1   = MasterTune * TwoPi
          * GetPrivateProfileFloat(sec, "F1", 200.0, dsfile) / Fs;
    OF2 = MasterTune * TwoPi
          * GetPrivateProfileFloat(sec, "F2", 120.0, dsfile) / Fs;
    ...
    BF = MasterTune * TwoPi * GetPrivateProfileFloat(sec, "F", 1000.0, dsfile)
         / Fs;
    BPhi = TwoPi / 8.f;
    ...
    BF2       = MasterTune * TwoPi
          * GetPrivateProfileFloat(sec, "F", 1000.0, dsfile) / Fs;
    BPhi2 = TwoPi / 8.f;
    ...
    if(DiON == 1)
    {
        ...
        DAtten = (float)powf(
                2.0, 2.0 * GetPrivateProfileInt(sec, "Bits", 0, dsfile));
        DGain = DAtten * DGain
                * (float)powf(10.0,
                              0.05
                                      * GetPrivateProfileInt(sec, "Clipping",
                                                             0, dsfile));
    }
    ...
    while(tpos < Length)
    {
        ...
        if(BON == 1)  // noise band 1
        {
            for(t = tpos; t <= tplus; t++)
            {
                ...
                botmp     = t - tpos;
                DF[botmp] = DF[botmp]
                            + (float)cos(fmod(BPhi, TwoPi)) * envData[5][ENV]
                                      * BL;
            }
            if(t >= envData[5][MAX])
                BON = 0;
        ...
        if(BON2 == 1)  // noise band 2
        {
            for(t = tpos; t <= tplus; t++)
            {
                ...
                botmp     = t - tpos;
                DF[botmp] = DF[botmp]
                            + (float)cos(fmod(BPhi2, TwoPi)) * envData[6][ENV]
                                      * BL2;
            }
            if(t >= envData[6][MAX])
                BON2 = 0;
