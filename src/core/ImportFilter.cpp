...
void ImportFilter::import(const QString& _file_to_import, TrackContainer* tc)
{
    ...
    for(const Plugin::Descriptor* desc:
        pluginFactory->descriptors(Plugin::ImportFilter))
    {
        qInfo("Notice: trying import filter: %s", qPrintable(desc->name()));
        Plugin* p = Plugin::instantiate(desc->name(), nullptr, s);
        if(dynamic_cast<ImportFilter*>(p) != nullptr
