...
DataFile::typeDescStruct DataFile::s_types[DataFile::TypeCount]
...
DataFile::LocaleHelper::LocaleHelper(Mode mode)
{
    ...
    {
        case ModeLoad:
            // set a locale for which QString::fromFloat() returns valid
            // values if floating point separator is a comma - otherwise we
            // would fail to load older projects made by people from various
            // countries due to their locale settings
            QLocale::setDefault(QLocale::German);
            break;

        case ModeSave:
            // set default locale to C so that floating point decimals are
            // rendered to strings with periods as decimal point instead of
            // commas in some countries
...
int DataFile::seed_count = 0;

DataFile::DataFile(Type type) :
      QDomDocument("lmms-project"), m_content(), m_head(), m_type(type)
{
    DataFile::seed_count++;
    if(DataFile::seed_count > 1)
...
bool DataFile::validate(QString extension)
{
    switch(m_type)
    {
        ...
        case Type::UnknownType:
            if(!(extension == "mmp" || extension == "mpt"
                 || extension == "mmpz" || extension == "xpf"
                 || extension == "xml"
                 || (extension == "xiz"
                     && !pluginFactory->pluginSupportingExtension(extension)
                                 .isNull())
                 || extension == "sf2" || extension == "pat"
                 || extension == "mid" || extension == "dll"))
            {
                return true;
            }
...
QString DataFile::nameWithExtension(const QString& _fn) const
{
    ...
    {
        case SongProject:
            if(_fn.section('.', -1) != "mmp" && _fn.section('.', -1) != "mpt"
               && _fn.section('.', -1) != "mmpz")
            {
        ...
        while(!node.isNull())
        {
            if(node.isElement())
            {
                ...
                if(name.endsWith("link"))
                {
                    el.setAttribute(
                            name,
                            node.namedItem("time").toElement().attribute(
                                    "value"));
...
{
    // Upgrade to version 0.2.1-20070508 from some version greater than or
    // equal to 0.2.1-20070501
    QDomNodeList list = elementsByTagName("arpandchords");
    for(int i = 0; !list.item(i).isNull(); ++i)
    {
        QDomElement el = list.item(i).toElement();
        if(el.hasAttribute("chorddisabled"))
        {
            el.setAttribute("chord-enabled",
                            !el.attribute("chorddisabled").toInt());
            el.setAttribute("arp-enabled",
                            !el.attribute("arpdisabled").toInt());
        }
    ...
    for(int i = 0; !list.item(i).isNull(); ++i)
    {
        ...
        {
            QDomNodeList vol_list = el.namedItem("automation-pattern")
                                            .namedItem("vol")
                                            .toElement()
...
{
    // Upgrade to version 0.3.0-rc2 from some version greater than or equal to
    // 0.2.1-20070508
    QDomNodeList list = elementsByTagName("arpandchords");
    for(int i = 0; !list.item(i).isNull(); ++i)
    {
...
{
    // Upgrade to version 0.3.0 (final) from some version greater than or
    // equal to 0.3.0-rc2
    QDomNodeList list;
...
{
    // Upgrade to version 0.4.0-20080104 from some version greater than or
    // equal to 0.3.0 (final)
...
{
    // Upgrade to version 0.4.0-20080118 from some version greater than or
    // equal to 0.4.0-20080104
    QDomNodeList list;
...
{
    // Upgrade to version 0.4.0-20080129 from some version greater than or
    // equal to 0.4.0-20080118
    QDomNodeList list;
...
{
    // Upgrade to version 0.4.0-20080409 from some version greater than or
    // equal to 0.4.0-20080129
    QStringList s;
    s << "note"
      << "pattern"
      << "bbtco"
...
{
    // Upgrade to version 0.4.0-20080607 from some version greater than or
    // equal to 0.3.0-20080409
    QDomNodeList list;
...
{
    // Upgrade to version 0.4.0-20080622 from some version greater than or
    // equal to 0.3.0-20080607
    QDomNodeList list;
...
{
    // Upgrade to version 0.4.0-beta1 from some version greater than or equal
    // to 0.4.0-20080622 convert binary effect-key-blobs to XML
    QDomNodeList list;
...
{
    // Upgrade to version 0.4.0-rc2 from some version greater than or equal to
    // 0.4.0-beta1
...
void DataFile::upgrade_1_1_91()
{
    ...
    {
        QDomElement el = list.item(i).toElement();
        if(el.attribute("name") == "plugin"
           && el.attribute("value") == "vocoder-lmms")
        {
...
void DataFile::upgrade_1_2_0_rc3()
{
    ...
    {
        QDomNodeList patterns
                = bbtracks.item(i).toElement().elementsByTagName("pattern");
...
{
    ProjectVersion version = documentElement()
                                     .attribute("creatorversion")
                                     .replace("svn", "");
...
void DataFile::loadData(const QByteArray& _data, const QString& _sourceFile)
{
    ...
    if(!setContent(_data, &errorMsg, &line, &col))
    {
        ...
        */
        ...
        if(line >= 0 && col >= 0)
        {
            qWarning("Warning: %s#%d:%d: %s", qPrintable(_sourceFile), line,
                     col, qPrintable(errorMsg));
            if(gui)
            {
                QMessageBox::critical(
    ...
    if(root.hasAttribute("creatorversion"))
    {
        ...
        {
            // only one compareType needs to be set, and we can compare on one
            // line because setCompareType returns ProjectVersion
               ...
               != openedWith)
            {
                if(gui != nullptr && root.attribute("type") == "song")
                {
                    TextFloat::displayMessage(
                            ...
                            SongEditor::tr("This %1 was created with "
                                           "LMMS %2.")
            ...
            // the upgrade needs to happen after the warning as it updates the
            // project version.
            if(createdWith.setCompareType(ProjectVersion::Build) < openedWith)
            {
                upgrade();
            }
        }
