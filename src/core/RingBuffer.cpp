/*
 * RingBuffer.cpp - an effective and flexible implementation of a ringbuffer
 * for LMMS
 *
...
RingBuffer::RingBuffer(real_t size) :
      m_fpp(Engine::mixer()->framesPerPeriod()),
      m_samplerate(Engine::mixer()->processingSampleRate())
{
...
void RingBuffer::changeSize(real_t size)
{
...
void RingBuffer::movePosition(real_t amount)
{
...
void RingBuffer::read(sampleFrame* dst, f_cnt_t offset)
{
    ...
    if(pos + m_fpp
       <= m_size)  // we won't go over the edge so we can just memcpy here
    {
...
void RingBuffer::read(sampleFrame* dst, real_t offset)
{
...
void RingBuffer::read(sampleFrame* dst, real_t offset, f_cnt_t length)
{
...
void RingBuffer::write(sampleFrame* src, real_t offset, f_cnt_t length)
{
...
void RingBuffer::writeAdding(sampleFrame* src, real_t offset, f_cnt_t length)
{
    writeAdding(src, msToFrames(offset), length);
}

void RingBuffer::writeAddingMultiplied(sampleFrame* src,
                                       f_cnt_t      offset,
                                       f_cnt_t      length,
                                       real_t       level)
{
    const f_cnt_t pos = (m_position + offset) % m_size;
    // qDebug( "pos %d m_pos %d ofs %d siz %d", pos, m_position, offset,
    // m_size );
    if(length == 0)
    {
        length = m_fpp;
...
void RingBuffer::writeSwappedAddingMultiplied(sampleFrame* src,
                                              f_cnt_t      offset,
                                              f_cnt_t      length,
                                              real_t       level)
{
    ...
    else
    {
        ...
        MixHelpers::addSwappedMultiplied(m_buffer, &src[first], level,
                                         second);
    }
}
...
{
    real_t newsize = (real_t((m_size - m_fpp)
                             * Engine::mixer()->processingSampleRate()))
                     / m_samplerate;
    m_size       = static_cast<f_cnt_t>(ceil(newsize)) + m_fpp;
    m_samplerate = Engine::mixer()->processingSampleRate();
    delete[] m_buffer;
    m_buffer = new sampleFrame[m_size];
    memset(m_buffer, 0, m_size * sizeof(sampleFrame));
