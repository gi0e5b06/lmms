/*
 * MidiEffectSelectDialog.cpp - dialog to choose effect plugin
 *
...
MidiEffectSelectDialog::MidiEffectSelectDialog( QWidget * _parent ) :
	QDialog( _parent ),
	ui( new Ui::MidiEffectSelectDialog ),
	m_sourceModel(),
	m_model(),
	m_descriptionWidget( NULL )
{
	...
	MidiEffectKeyList subPluginEffectKeys;

	for (const Plugin::Descriptor* desc: pluginFactory->descriptors(Plugin::MidiEffect))
	{
		...
		{
			m_effectKeys << MidiEffectKey( desc, desc->name );
	...
	int row = 0;
	for( MidiEffectKeyList::ConstIterator it = m_effectKeys.begin();
...
MidiEffectSelectDialog::~MidiEffectSelectDialog()
{
	delete ui;
}
...
MidiEffect * MidiEffectSelectDialog::instantiateSelectedPlugin( MidiEffectChain * _parent )
{
	if( !m_currentSelection.name.isEmpty() && m_currentSelection.desc )
	{
		return MidiEffect::instantiate( m_currentSelection.desc->name,
...
void MidiEffectSelectDialog::acceptSelection()
{
...
void MidiEffectSelectDialog::rowChanged( const QModelIndex & _idx,
										const QModelIndex & )
{
...
void MidiEffectSelectDialog::sortAgain()
{
...
void MidiEffectSelectDialog::updateSelection()
{
