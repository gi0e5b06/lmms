...
LV2ControlView::LV2ControlView( QWidget * _parent,
				      LV2Control * _ctl ) :
	QWidget( _parent ),
	ModelView( _ctl, this ),
	m_ctl( _ctl )
{
	...
	switch( m_ctl->port()->data_type )
	{
		case TOGGLED:
		{
			LedCheckBox * toggle = new LedCheckBox
				(m_ctl->port()->name, this,
				 ...
				 LedCheckBox::Green );
			qWarning("LV2ControlView: LedCheckBox %p name=%s",toggle,qPrintable(m_ctl->port()->name));
		...
		case FLOATING:
			...
			qWarning("LV2ControlView: Knob %p name=%s",knb,qPrintable(m_ctl->port()->name));
			break;

		case TIME:
			knb = new TempoSyncKnob( knobBright_26, this, m_ctl->port()->name );
			qWarning("LV2ControlView: TempoSyncKnob %p name=%s",knb,qPrintable(m_ctl->port()->name));
			break;
