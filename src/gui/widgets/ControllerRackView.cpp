...
void ControllerRackView::deleteController(Controller* c)
{
    if(c->connectionCount() > 0)
    {
        ...
        msgBox.setText(tr(
                "Confirm delete? There are existing connection(s) "
...
void ControllerRackView::onControllerAdded(Controller* controller)
{
    ...
    if(controllerView->model() == nullptr)
    {
        qWarning("+++ model is null");
        BACKTRACE
        // MyClass *m = dynamic_cast<MyClass *>(ptr);
        // controllerView->setModel(new Controller(
        // Controller::DummyController, nullptr,"dummy controller" ));
    }

    connect(controllerView, SIGNAL(deleteController(Controller*)), this,
            SLOT(deleteController(Controller*)), Qt::QueuedConnection);
