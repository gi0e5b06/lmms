...
SetupDialog::SetupDialog(ConfigTabs _tab_to_open) :
      m_bufferSize(ConfigManager::inst()
      ...
      m_disableBackup(
              !ConfigManager::inst()->value("app", "disablebackup").toInt()),
      m_openLastProject(
              ConfigManager::inst()->value("app", "openlastproject").toInt()),
      m_hqAudioDev(ConfigManager::inst()->value("mixer", "hqaudio").toInt()),
      m_lang(ConfigManager::inst()->value("app", "language")),
      m_workingDir(
              QDir::toNativeSeparators(ConfigManager::inst()->workingDir())),
      m_vstDir(QDir::toNativeSeparators(ConfigManager::inst()->vstDir())),
      m_artworkDir(
              QDir::toNativeSeparators(ConfigManager::inst()->artworkDir())),
      ...
      m_defaultSoundfont(QDir::toNativeSeparators(
              ConfigManager::inst()->defaultSoundfont())),
#endif
#ifdef LMMS_HAVE_STK
      m_stkDir(QDir::toNativeSeparators(ConfigManager::inst()->stkDir())),
#endif
      m_backgroundArtwork(QDir::toNativeSeparators(
              ConfigManager::inst()->backgroundArtwork())),
      m_smoothScroll(
              ConfigManager::inst()->value("ui", "smoothscroll").toInt()),
      m_enableAutoSave(ConfigManager::inst()
                               ->value("ui", "enableautosave", "1")
                               .toInt()),
      m_enableRunningAutoSave(
              ConfigManager::inst()
                      ->value("ui", "enablerunningautosave", "0")
                      .toInt()),
      m_saveInterval(
              ConfigManager::inst()->value("ui", "saveinterval").toInt() < 1
                      ? MainWindow::DEFAULT_SAVE_INTERVAL_MINUTES
                      : ConfigManager::inst()
                                ->value("ui", "saveinterval")
                                .toInt()),
      m_mappedFiles(CONFIG_GET_BOOL("disk.mappedfiles")),
      m_oneInstrumentTrackWindow(
              ConfigManager::inst()
                      ->value("ui", "oneinstrumenttrackwindow")
                      .toInt()),
      m_compactTrackButtons(ConfigManager::inst()
                                    ->value("ui", "compacttrackbuttons")
                                    .toInt()),
      m_syncVSTPlugins(
              ConfigManager::inst()->value("ui", "syncvstplugins").toInt()),
      m_animateAFP(
              ConfigManager::inst()->value("ui", "animateafp", "1").toInt()),
      m_printNoteLabels(
              ConfigManager::inst()->value("ui", "printnotelabels").toInt()),
      m_displayWaveform(ConfigManager::inst()
                                ->value("ui", "displaywaveform", "1")
                                .toInt()),
      m_disableAutoQuit(
      ...
      m_midiMtcVideoFps(CONFIG_GET_INT("midi.mtc_video_fps"))
{
    ...
    connect(baseSRCMB, SIGNAL(currentIndexChanged(int)), this,
            SLOT(setBaseSampleRate(int)));

    bufsize_tw->setFixedHeight(YDelta * labelNumber + HeaderSize + FooterSize
                               + 28);

    TabWidget* ui_tw = new TabWidget(tr("UI"), general);
    labelNumber      = 0;

    QCheckBox* enable_tooltips = new QCheckBox(tr("Enable tooltips"), ui_tw);
    labelNumber++;
    enable_tooltips->move(XDelta, YDelta * labelNumber);
    enable_tooltips->setChecked(m_toolTips);
    connect(enable_tooltips, SIGNAL(toggled(bool)), this,
            SLOT(toggleToolTips(bool)));

    QCheckBox* dbfs = new QCheckBox(tr("Display volume as dBFS "), ui_tw);
    labelNumber++;
    dbfs->move(XDelta, YDelta * labelNumber);
    dbfs->setChecked(m_displaydBFS);
    connect(dbfs, SIGNAL(toggled(bool)), this, SLOT(toggleDisplaydBFS(bool)));

    QCheckBox* compacttracks
            = new QCheckBox(tr("Compact track buttons"), ui_tw);
    labelNumber++;
    ...
    QCheckBox* leftsidebar = new QCheckBox(tr("Side bar on left"), ui_tw);
    labelNumber++;
    leftsidebar->move(XDelta, YDelta * labelNumber);
    leftsidebar->setChecked(m_uiLeftSideBar);
    connect(leftsidebar, SIGNAL(toggled(bool)), this,
            SLOT(toggleLeftSideBar(bool)));

    QCheckBox* oneitw
            = new QCheckBox(tr("One instrument track window mode"), ui_tw);
    labelNumber++;
    ...
    QCheckBox* noteLabels
            = new QCheckBox(tr("Enable note labels in piano roll"), ui_tw);
    labelNumber++;
    ...
    QCheckBox* displayWaveform
            = new QCheckBox(tr("Enable waveform display by default"), ui_tw);
    labelNumber++;
    ...
    QCheckBox* restart_msg = new QCheckBox(
            tr("Show restart warning after changing settings"), ui_tw);
    labelNumber++;
    restart_msg->move(XDelta, YDelta * labelNumber);
    restart_msg->setChecked(m_warnAfterSetup);
    connect(restart_msg, SIGNAL(toggled(bool)), this,
    ...
    QCheckBox* hqaudio
            = new QCheckBox(tr("HQ-mode for output audio-device"), audio_tw);
    labelNumber++;
    ...
    QCheckBox* disableAutoquit = new QCheckBox(
            tr("Keep effects running even without input"), audio_tw);
    labelNumber++;
    ...
    QCheckBox* syncVST = new QCheckBox(
            tr("Sync VST plugins to host playback"), audio_tw);
    labelNumber++;
    ...
    QCheckBox* disableBackup = new QCheckBox(
            tr("Create backup file when saving a project"), disk_tw);
    labelNumber++;
    ...
    QCheckBox* openLastProject
            = new QCheckBox(tr("Reopen last project on start"), disk_tw);
    labelNumber++;
    ...
    QCheckBox* mmpz = new QCheckBox(tr("Compress project files per default"),
                                    disk_tw);
    labelNumber++;
    mmpz->move(XDelta, YDelta * labelNumber);
    mmpz->setChecked(m_MMPZ);
    connect(mmpz, SIGNAL(toggled(bool)), this, SLOT(toggleMMPZ(bool)));
    ...
    */
    ...
    m_wdLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                              YDelta);
    ...
    QPushButton* workingdir_select_btn = new QPushButton(
            embed::getIconPixmap("project_open", 16, 16), "", paths);
    ...
    */
    ...
    m_adLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                              YDelta);
    ...
    TabWidget * backgroundArtwork_tw = new TabWidget( tr(
                                                         "Background artwork"
    ).toUpper(), pathSelectors );//paths backgroundArtwork_tw->setFixedHeight(
    48 );
    */
    labelNumber++;
    labelNumber++;
    QLabel* backgroundArtwork_tw
            = new QLabel(tr("Background artwork"), paths);
    backgroundArtwork_tw->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                                      YDelta);
    labelNumber++;
    m_baLineEdit = new QLineEdit(m_backgroundArtwork, paths);
    m_baLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                              YDelta);
    ...
    backgroundartworkdir_select_btn->move(btnStart,
                                          YDelta * labelNumber - YDelta);
    ...
    */
    ...
    m_vdLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                              YDelta);
    ...
    connect(vstdir_select_btn, SIGNAL(clicked()), this, SLOT(openVSTDir()));
    ...
    */
    ...
    m_gigLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                               YDelta);
    ...
    connect(gigdir_select_btn, SIGNAL(clicked()), this, SLOT(openGIGDir()));
    ...
    */
    ...
    m_sf2LineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                               YDelta);
    ...
    connect(sf2dir_select_btn, SIGNAL(clicked()), this, SLOT(openSF2Dir()));
    ...
    */
    ...
    m_ladLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                               YDelta);
    ...
    QPushButton* laddir_select_btn = new QPushButton(
            embed::getIconPixmap("add_folder", 16, 16), "", paths);
    ...
    */
    ...
    m_stkLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                               YDelta);
    ...
    connect(stkdir_select_btn, SIGNAL(clicked()), this, SLOT(openSTKDir()));
#endif
    ...
    */
    ...
    m_sfLineEdit->setGeometry(XDelta, YDelta * labelNumber, txtLength,
                              YDelta);
    ...
    */
    ...
    TabWidget* auto_save_tw
            = new TabWidget(tr("Auto save").toUpper(), performance);
    labelNumber = 0;

    labelNumber++;
    m_saveIntervalLbl = new QLabel(auto_save_tw);
    m_saveIntervalLbl->setGeometry(XDelta, YDelta * labelNumber, 350 - XDelta,
                                   YDelta);
    ...
    labelNumber++;
    m_autoSave = new QCheckBox(tr("Enable auto-save"), auto_save_tw);
    ...
    labelNumber++;
    m_runningAutoSave = new QCheckBox(tr("Allow auto-save while playing"),
                                      auto_save_tw);
    ...
    // labelNumber++;
    auto_save_tw->setFixedHeight(YDelta * labelNumber + HeaderSize
                                 + FooterSize + 28);
    ...
    labelNumber++;
    QCheckBox* smoothScroll
            = new QCheckBox(tr("Smooth scroll in Song Editor"), ui_perf_tw);
    ...
    QCheckBox* animAFP = new QCheckBox(
            tr("Show playback cursor in AudioFileProcessor"), ui_perf_tw);
    ...
    labelNumber++;
    QCheckBox* mappedFiles
            = new QCheckBox(tr("Mapped sample files"), disk_perf_tw);
    mappedFiles->move(XDelta, YDelta * labelNumber);
    mappedFiles->setChecked(m_mappedFiles);
    connect(mappedFiles, SIGNAL(toggled(bool)), this,
            SLOT(toggleMappedFiles(bool)));

    labelNumber++;
    disk_perf_tw->setFixedHeight(YDelta * labelNumber + HeaderSize
                                 + FooterSize);
    ...
    m_audioInterfaces->setCurrentIndex(
            m_audioInterfaces->findText(audioDevName));
    ...
    labelNumber++;
    QComboBox* midiMtcVideoFps = new QComboBox(midiiface_tw);
    midiMtcVideoFps->addItem(QLocale::system().toString(24));
    midiMtcVideoFps->addItem(QLocale::system().toString(25));
    midiMtcVideoFps->addItem(QLocale::system().toString(29.97));
    midiMtcVideoFps->addItem(QLocale::system().toString(30));
    midiMtcVideoFps->setCurrentIndex(m_midiMtcVideoFps);
    midiMtcVideoFps->setGeometry(2 * XDelta, YDelta * labelNumber, 60,
                                 YDelta);
    ...
    labelNumber++;
    midiiface_tw->setFixedHeight(YDelta * labelNumber + HeaderSize
                                 + FooterSize);
    ...
    m_tabBar->addTab(paths, tr("Paths"), 1, false, true)
            ->setIcon(embed::getIconPixmap("setup_directories"));
    m_tabBar->addTab(performance, tr("Performance settings"), 2, false, true)
            ->setIcon(embed::getIconPixmap("setup_performance"));
...
void SetupDialog::accept()
{
    ...
    ConfigManager::inst()->setWorkingDir(
            QDir::fromNativeSeparators(m_workingDir));
    ...
    ConfigManager::inst()->setArtworkDir(
            QDir::fromNativeSeparators(m_artworkDir));
...
void SetupDialog::setBufferSize(int _value)
{
    ...
    */
    ...
    m_bufSizeLbl->setText(
            tr("Frames: %1, Latency: %2 ms")
                    .arg(m_bufferSize)
                    .arg(1000.0f * m_bufferSize
...
void SetupDialog::toggleMidiMtcEnabled(bool _enabled)
{
    m_midiMtcEnabled = _enabled;
}

void SetupDialog::setMidiMtcVideoFps(int _vfps)
{
    m_midiMtcVideoFps = qBound(0, _vfps, 3);
}

void SetupDialog::openWorkingDir()
{
    QString new_dir = FileDialog::getExistingDirectory(
            this, tr("Choose LMMS working directory"), m_workingDir);
...
void SetupDialog::openBackgroundArtwork()
{
    ...
    QString dir = (m_backgroundArtwork.isEmpty()) ? m_artworkDir
                                                  : m_backgroundArtwork;
    QString new_file = FileDialog::getOpenFileName(
            this, tr("Choose background artwork"), dir,
...
void SetupDialog::displaySaveIntervalHelp()
{
    QWhatsThis::showText(QCursor::pos(),
                         tr("Set the time between automatic backup to %1.\n"
                            ...
                            "something some older systems find difficult.")
                                 .arg(ConfigManager::inst()->recoveryFile()));
}
