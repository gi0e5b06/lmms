...
ExportProjectDialog::ExportProjectDialog(const QString& _file_name,
      ...
      QDialog(_parent),
      Ui::ExportProjectDialog(), m_fileName(_file_name), m_fileExtension(),
      m_multiExport(multi_export), m_renderManager(nullptr)
{
    setupUi(this);
    setWindowTitle(
            tr("Export project to %1").arg(QFileInfo(_file_name).fileName()));
    ...
    for(int i = 0; i <= MAX_LEVEL; ++i)
    {
        ...
        compLevelCB->addItem(QString::number(i) + " " + info,
...
void ExportProjectDialog::startExport()
{
    ...
    {
        double level = compLevelCB->itemData(compLevelCB->currentIndex())
                               .toDouble();
    ...
    if(!(m_multiExport
         || output_name.endsWith(m_fileExtension, Qt::CaseInsensitive)))
    {
    ...
    connect(m_renderManager.get(), SIGNAL(progressChanged(int)), progressBar,
            SLOT(setValue(int)));
    connect(m_renderManager.get(), SIGNAL(progressChanged(int)), this,
...
void ExportProjectDialog::onFileFormatChanged(int index)
{
    ...
    bool sampleRateControlsVisible
