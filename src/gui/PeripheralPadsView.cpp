...
// QPixmap * PeripheralPadsView::s_padPm = nullptr;           /*!< A pad
// released
 ...
 * correct?
 */
PeripheralPadsView::PeripheralPadsView(Piano* _piano, QWidget* _parent) :
      PeripheralView(_piano, _parent), /*!< Our parent */
...
void PeripheralPadsView::modelChanged()
{
        ...
        if( m != nullptr )
        {
                connect( m->instrumentTrack()->baseNoteModel(), SIGNAL(
...
void PeripheralPadsView::mousePressEvent(QMouseEvent* _me)
{
    if(_me->button() == Qt::LeftButton)
    {
        ...
        if(keyNum >= 0)
        {
            ...
            {
                int velocity
                        = m->instrumentTrack()->midiPort()->baseVelocity();
                // set note on
                /*
                ...
                */
                m->handleKeyPress(keyNum, velocity);
                m_lastKey = keyNum;
...
void PeripheralPadsView::mouseMoveEvent(QMouseEvent* _me)
{
    ...
    if( keyNum != m_lastKey )
    {
            if( m_lastKey != -1 )
            {
                    m->midiEventProcessor()->processInEvent( MidiEvent(
    MidiNoteOff, -1, m_lastKey, 0 ) ); m->setKeyState( m_lastKey, false
    ); m_lastKey = -1;
            }
            if( _me->buttons() & Qt::LeftButton )
            {
                    ...
                    {
                            m->midiEventProcessor()->processInEvent(
    MidiEvent( MidiNoteOn, -1, keyNum, velocity ) ); m->setKeyState(
