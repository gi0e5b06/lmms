/*
 * PeripheralPadsView.cpp - implementation of piano-widget used in instrument-track-window
...
//QPixmap * PeripheralPadsView::s_padPressedPm = NULL;    /*!< A pad pressed */
...
void PeripheralPadsView::pianoScrolled( int _newPos )
{
	...
	//		( _new_pos / WhiteKeysPerOctave ) * KeysPerOctave;
        ...
        qInfo("PeripheralPadsView::pianoScrolled np=%d sk=%d",
              _newPos,m_startKey);
	update();
}
...
void PeripheralPadsView::mousePressEvent( QMouseEvent * _me )
{
	if( _me->button() == Qt::LeftButton )
	{
                ...
                if(keyNum>=0)
                {
                        ...
                        {
                                m_piano->instrumentTrack()->baseNoteModel()->
                                        setInitValue( (float) keyNum );
...
void PeripheralPadsView::mouseMoveEvent( QMouseEvent * _me )
{
	...
	if( keyNum != m_lastKey )
	{
		...
		if( _me->buttons() & Qt::LeftButton )
		{
			...
			{
				m_piano->midiEventProcessor()->processInEvent( MidiEvent( MidiNoteOn, -1, keyNum, velocity ) );
				m_piano->setKeyState( keyNum, true );
				m_lastKey = keyNum;
			}
			else
			{
				m_piano->instrumentTrack()->baseNoteModel()->setInitValue( (float) keyNum );
			}
		}
	...
	{
		m_piano->midiEventProcessor()->processInEvent( MidiEvent( MidiKeyPressure, -1, keyNum, velocity ) );
	}
        */
}
...
{
	const int keyNum = getKeyFromKeyEvent( _ke ) +
...
void PeripheralPadsView::paintEvent( QPaintEvent * )
{
	...
	const int baseKey = (m_piano!=NULL)
