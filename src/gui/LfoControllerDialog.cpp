...
const int CD_LFO_AMOUNT_CD_KNOB_X
        = CD_LFO_SPEED_CD_KNOB_X + CD_KNOB_X_SPACING;
const int CD_LFO_PHASE_CD_KNOB_X
        = CD_LFO_AMOUNT_CD_KNOB_X + CD_KNOB_X_SPACING;
const int CD_LFO_MULTIPLIER_X
        = CD_LFO_PHASE_CD_KNOB_X + CD_KNOB_X_SPACING + 3;

LfoControllerDialog::LfoControllerDialog(Controller* _model,
                                         QWidget*    _parent) :
      ControllerDialog(_model, _parent)
{
    ...
    triangle_wave_btn->setActiveGraphic(
            embed::getPixmap("triangle_wave_active"));
    triangle_wave_btn->setInactiveGraphic(
            embed::getPixmap("triangle_wave_inactive"));
    ToolTip::add(triangle_wave_btn, tr("Click here for a triangle-wave."));
    ...
    moog_saw_wave_btn->setActiveGraphic(
            embed::getPixmap("moog_saw_wave_active"));
    moog_saw_wave_btn->setInactiveGraphic(
            embed::getPixmap("moog_saw_wave_inactive"));
    ToolTip::add(moog_saw_wave_btn, tr("Click here for a moog saw-wave."));
    ...
    white_noise_btn->setActiveGraphic(
            embed::getPixmap("white_noise_wave_active"));
    white_noise_btn->setInactiveGraphic(
            embed::getPixmap("white_noise_wave_inactive"));
    ToolTip::add(white_noise_btn, tr("Click here for white-noise."));
    ...
    x1->setActiveGraphic(embed::getPixmap("lfo_x1_active"));
    x1->setInactiveGraphic(embed::getPixmap("lfo_x1_inactive"));

    PixmapButton* x100 = new PixmapButton(this, NULL);
    x100->move(CD_LFO_MULTIPLIER_X, CD_LFO_SHAPES_Y - 8);
    x100->setActiveGraphic(embed::getPixmap("lfo_x100_active"));
    x100->setInactiveGraphic(embed::getPixmap("lfo_x100_inactive"));

    PixmapButton* d100 = new PixmapButton(this, NULL);
    d100->move(CD_LFO_MULTIPLIER_X, CD_LFO_SHAPES_Y + 22);
    d100->setActiveGraphic(embed::getPixmap("lfo_d100_active"));
    d100->setInactiveGraphic(embed::getPixmap("lfo_d100_inactive"));
    ...
    pal.setBrush(backgroundRole(),
                 embed::getPixmap("lfo_controller_artwork"));
    setPalette(pal);
}
...
void LfoControllerDialog::contextMenuEvent(QContextMenuEvent*)
{
    ...
    contextMenu->addAction( embed::getIcon( "arp_up_on" ),
                                            tr( "Move &up" ),
                                            this, SLOT( moveUp() ) );
    contextMenu->addAction( embed::getIcon( "arp_down_on" ),
                                            tr( "Move &down" ),
                                            this, SLOT( moveDown() ) );
    ...
    contextMenu->addSeparator();
    contextMenu->addAction( embed::getIcon( "help" ),
                                            tr( "&Help" ),
