...
AboutDialog::AboutDialog(QWidget* parent) : QDialog(parent), Ui::AboutDialog()
{
    setupUi(this);

    iconLabel->setPixmap(embed::getPixmap("icon", 64, 64));
    ...
    versionLabel->setText(versionLabel->text()
                                  ...
                                  .arg(QT_VERSION_STR)
                                  .arg(GCC_VERSION));
    versionLabel->setTextInteractionFlags(versionLabel->textInteractionFlags()
                                          | Qt::TextSelectableByMouse);

    copyrightLabel->setText(
            copyrightLabel->text().arg(LMMS_PROJECT_COPYRIGHT));
