...
// granularity of managed memory
const int MM_CHUNK_SIZE = 64;

// how many chunks to allocate at startup
// TODO: make configurable
const int MM_INITIAL_CHUNKS = 1024 * 1024;

// min. amount of chunks to increment at a time
const int MM_INCREMENT_CHUNKS = 16 * 1024;
...
struct MemoryPool
{
    ...
    MemoryPool(int chunks) : m_chunks(chunks)
    {
    ...
    MemoryPool(const MemoryPool& mp) :
          m_pool(mp.m_pool), m_free(mp.m_free), m_chunks(mp.m_chunks),
          m_mutex()
    {
    }
...
class EXPORT MemoryManagerLegacy
{
  ...
  protected:
    static int extend(
            int chunks);  // returns index of created pool (for use by alloc)
