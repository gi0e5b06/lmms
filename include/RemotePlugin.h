...
// sometimes we need to exchange bigger messages (e.g. for VST parameter
// dumps) so set a usable value here
...
class shmFifo
{
  ...
  public:
    ...
    shmFifo() :
          m_invalid(false), m_master(true), m_shmKey(0),
#ifdef USE_QT_SHMEM
          m_shmObj(),
#else
          m_shmID(-1),
#endif
          m_data(nullptr), m_dataSem(QString::null),
          m_messageSem(QString::null), m_lockDepth(0)
    {
#ifdef USE_QT_SHMEM
        do
        {
        ...
        m_dataSem.setKey(QString::number(m_data->dataSem.semKey), 1,
                         QSystemSemaphore::Create);
        m_messageSem.setKey(QString::number(m_data->messageSem.semKey), 0,
                            QSystemSemaphore::Create);
    }
    ...
    shmFifo(key_t _shm_key) :
          m_invalid(false), m_master(false), m_shmKey(0),
#ifdef USE_QT_SHMEM
  ...
  private:
    ...
    void write(const void* _buf, int _len)
    {
        ...
        while(_len > SHM_FIFO_SIZE - m_data->endPtr)
        {
            ...
            {
                memmove(m_data->data, m_data->data + m_data->startPtr,
...
class EXPORT RemotePluginBase
{
  public:
    struct message
    {
        message() : id(IdUndefined), data()
        {
        }

        message(const message& _m) : id(_m.id), data(_m.data)
        {
        }

        message(int _id) : id(_id), data()
        {
        }
...
class RemotePluginClient : public RemotePluginBase
{
  public:
    ...
    virtual void process(const sampleFrame* _in_buf, sampleFrame* _out_buf)
            = 0;
...
RemotePluginBase::RemotePluginBase(shmFifo* _in, shmFifo* _out) :
      m_in(_in), m_out(_out)
#else
RemotePluginBase::RemotePluginBase() : m_socket(-1), m_invalid(false)
#endif
{
#ifdef LMMS_HAVE_LOCALE_H
...
RemotePluginBase::message RemotePluginBase::waitForMessage(const message& _wm,
                                                           bool _busy_waiting)
{
#ifndef BUILD_REMOTE_PLUGIN_CLIENT
    if(_busy_waiting)
    ...
    {
        WaitDepthCounter(int& depth, bool busy) : m_depth(depth), m_busy(busy)
        {
            if(m_busy)
            {
                ++m_depth;
...
#endif
#ifdef USE_QT_SHMEM
      m_shmObj(), m_shmQtID("/usr/bin/lmms"),
#endif
      m_vstSyncData(nullptr), m_shm(nullptr), m_inputCount(0),
      m_outputCount(0), m_sampleRate(44100), m_bufferSize(0)
{
#ifndef SYNC_WITH_SHM_FIFO
...
bool RemotePluginClient::processMessage(const message& _m)
{
    ...
    switch(_m.id)
    {
        ...
        case IdMidiEvent:
            processMidiEvent(
                    MidiEvent(static_cast<MidiEventTypes>(_m.getInt(0)),
                              _m.getInt(1), _m.getInt(2), _m.getInt(3)),
                    _m.getInt(4));
            break;
...
void RemotePluginClient::doProcessing()
{
    if(m_shm != nullptr)
    {
        process((sampleFrame*)(m_inputCount > 0 ? m_shm : nullptr),
                (sampleFrame*)(m_shm + (m_inputCount * m_bufferSize)));
    }
    else
    {
