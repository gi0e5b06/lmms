...
class EXPORT BandLimitedWave
{
  public:
    ...
    /*! \brief This method provides interpolated samples of bandlimited
     * waveforms. \param _ph The phase of the sample. \param _wavelen The
     * wavelength (length of one cycle, ie. the inverse of frequency) of the
            ...
            oscillate(real_t _ph, real_t _wavelen, Waveforms _wave)
    {
        ...
        if(_wavelen > TLENS[MAXTBL])
        {
            ...
            const real_t lookupf = ph * real_t(tlen);
            const int    lookup  = lookupf;
            const real_t ip      = fraction(lookupf);

            const sample_t s1 = s_waveforms[_wave].sampleAt(t, lookup);
            const sample_t s2
                    = s_waveforms[_wave].sampleAt(t, (lookup + 1) % tlen);
            const int      lm = lookup == 0 ? tlen - 1 : lookup - 1;
            const sample_t s0 = s_waveforms[_wave].sampleAt(t, lm);
            const sample_t s3
                    = s_waveforms[_wave].sampleAt(t, (lookup + 2) % tlen);
        ...
        /*
          ...
          tlen = tlen << 1;
          t += 1;
          const sample_t s3 = s_waveforms[ _wave ].sampleAt( t,
          lookup ); const sample_t s4 = s_waveforms[ _wave ].sampleAt( t, (
          lookup + 1 ) % tlen ); const sample_t s34 = linearInterpolate( s3,
          s4, ip );
          const real_t ip2 = ( ( tlen - _wavelen ) / tlen - 0.5
          ) * 2.0;
