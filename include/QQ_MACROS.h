...
#define QQ_LOCAL_PROPERTY(type, name, getf, setf)          \
    Q_PROPERTY(type name READ getf WRITE setf STORED true) \
  protected:                                               \
    type m_##name;                                         \
                                                           \
    Q_SIGNAL void name##Changed(type);                     \
    Q_SLOT void   name##Updated();                         \
                                                           \
  public:                                                  \
    type getf() const                                      \
    {                                                      \
    ...
    void setf(type new_val)                                \
    {                                                      \
        if(m_##name != new_val)                            \
        {                                                  \
            m_##name = new_val;                            \
            Q_EMIT name##Changed(new_val);                 \
        }                                                  \
    }

#define QQ_LOCAL_PTR_PROPERTY(type, name, getf, setf)                          \
    Q_PROPERTY(type name READ getf WRITE setf                                  \
                                         STORED true /*NOTIFY name##Changed*/) \
  protected:                                                                   \
    type m_##name;                                                             \
    void name##Updated();                                                      \
                                                                               \
...
#define QQ_LOCAL_PROPERTY_DV(type,name,getf,setf,def_val)\
        Q_PROPERTY(type name READ getf WRITE setf STORED true NOTIFY
name##Changed)\
        protected: type m_##name = def_val;\
        public:    type getf() { return m_##name; }\
        void setf(const type new_val) { if(m_##name!=new_val) {
m_##name=new_val; Q_EMIT name##Changed(new_val); } }\
          Q_SIGNAL void name##Changed(type);\
...
name##Changed)\
        protected: type m_##name = def_val;\
        public:    type getf() { return m_##name; }\
        void setf(type new_val) { if(m_##name!=new_val) { m_##name=new_val;
Q_EMIT name##Changed(new_val); } }\
          Q_SIGNAL void name##Changed(type);\
...
name##Changed)\
        protected: type * m_##name = &def_val;\
        public:    type & getf() { return *m_##name; }\
        void setf(type & new_val) { if(*m_##name!=new_val) {
m_##name=&new_val; Q_EMIT name##Changed(new_val); } }\
          Q_SIGNAL void name##Changed(type &);\
...
#define
QQ_READONLY_PTR_DELEGATED_PTR_PROPERTY(type,name,getf1,delegate,getf2)\
        Q_PROPERTY(type name READ getf1 STORED false)\
        public:   type getf1() { return delegate->getf2(); }\

#define
QQ_READONLY_PTR_DELEGATED_REF_PROPERTY(type,name,getf1,delegate,getf2)\
        Q_PROPERTY(type name READ getf1 STORED false)\
        public:   type & getf1() { return delegate->getf2(); }\


#define QQ_DELEGATED_PROPERTY(type,name,getf1,setf1,delegate,getf2,setf2)\
        Q_PROPERTY(type name READ getf1 WRITE setf1 STORED false NOTIFY
name##Changed)\
        public:   type getf1() { return delegate.getf2(); }\
                  void setf1(const type new_val) {
if(delegate.getf2()!=new_val) { delegate.setf2(new_val); Q_EMIT
name##Changed(new_val); } }\
         Q_SIGNAL void name##Changed(type);\
...
name##Changed)\
        public:   type getf1() { return delegate.getf2(); }\
                  void setf1(type new_val) { if(delegate.getf2()!=new_val) {
delegate.setf2(new_val); Q_EMIT name##Changed(#name,new_val); } }\
         Q_SIGNAL void name##Changed(type);\
...
#define
QQ_PTR_DELEGATED_PTR_PROPERTY(type,name,getf1,setf1,delegate,getf2,setf2)\
        Q_PROPERTY(type name READ getf1 WRITE setf1 STORED false NOTIFY
name##Changed)\
        public:   type getf1() { return delegate->getf2(); }\
                  void setf1(type new_val) { if(delegate->getf2()!=new_val) {
...
#define
QQ_PTR_DELEGATED_REF_PROPERTY(type,name,getf1,setf1,delegate,getf2,setf2)\
        Q_PROPERTY(type name READ getf1 WRITE setf1 STORED false NOTIFY
name##Changed)\
        public:   type & getf1() { return delegate->getf2(); }\
                  void setf1(const type & new_val) {
...
/***
  Usage:
    ...
    - REF_PROPERTY only works on primitive and copy-value types. Use
PTR_PROPERTY for QObject-derived types.
...
class Sandbox : public QObject
{
        ...
        QQ_CONSTANT_REF_PROPERTY(QString,zc,zc,HELLO)
                ...
                //this is a QObject: QQ_CONSTANT_REF_PROPERTY(Field
,bc,bc,BYE)
        ...
        QQ_LOCAL_RESETABLE_REF_PROPERTY(QString,zr,zr,setZr,resetZr,HELLO2)
                //this is a QObject: QQ_LOCAL_RESETABLE_PROPERTY(Field
,zr,zr,setZr,resetZr,BYE)
                //this is a QObject: QQ_LOCAL_RESETABLE_REF_PROPERTY(Field
,br,br,setBr,resetBr,BYE)
        ...
        QQ_DELEGATED_PTR_PROPERTY(QObject*,aa,aa,setAa,OBJET,parent,setParent)
                // not a ref:
QQ_DELEGATED_REF_PROPERTY(QString,za,za,setZa,OBJET,objectName,setObjectName)
